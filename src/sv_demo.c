...
Addional demo data InfinityWards has added that contains players position and
viewangle The only intention is maybe that the player self can create a demo
with higher FPS-rate than server provides
====================
*/
...
void SV_StopRecord(client_t* cl) {
  ...
  Com_sprintf(cmdline, sizeof(cmdline), "\"%s/%s\" \"%s/%s\"",
              fs_homepath->string, sv_demoCompletedCmd->string,
              fs_homepath->string, cl->demoName);
...
void SV_RecordClient(client_t* cl, char* basename) {
  ...
  *(int32_t*)0x13f39080 = *(int32_t*)msg.data;
  compLen = 4 + MSG_WriteBitsCompress(0, msg.data + 4, (byte*)0x13f39084,
                                      msg.cursize - 4);
...
int FS_DemoWrite(const void* buffer, int len, fileHandleData_t* fh) {
  ...
  if (len + fh->bufferPos >
      fh->bufferSize) { // If the buffer is too small to take more data flush it
    if (FS_DemoFlush(fh) != oldBufferPos) { // Fatal file write error
      Com_Printf("Demo file write error. Closing file %s\n", fh->name);
      FS_FCloseDemoFile(fh);
      return 0;
