...
int GScr_LoadScriptAndLabel(const char* scriptName, const char* labelName,
                            qboolean mandatory) {
  ...
  if (!Scr_LoadScript(scriptName, &load_buffer, 0)) {
    if (mandatory) {
      ...
      Com_DPrintf(
          "Notice: Could not find script '%s' - this part will be disabled\n",
          scriptName);
    }
    return 0;
  }
  ...
  if (!fh) {
    if (mandatory) {
      Com_Error(ERR_DROP, "Could not find label '%s' in script '%s'", labelName,
                scriptName);
    } else {
      Com_DPrintf("Notice: Could not find label '%s' in script '%s' - this "
...
void GScr_LoadGameTypeScript(void) {
  ...
  Cvar_RegisterString("g_gametype", "dm", CVAR_LATCH | CVAR_SERVERINFO,
                      "Current game type");

  Com_sprintf(gametype_path, sizeof(gametype_path), "maps/mp/gametypes/%s",
  ...
  g_scr_data.startgametype = GScr_LoadScriptAndLabel(
      "maps/mp/gametypes/_callbacksetup", "CodeCallback_StartGameType", 1);
  g_scr_data.playerconnect = GScr_LoadScriptAndLabel(
      "maps/mp/gametypes/_callbacksetup", "CodeCallback_PlayerConnect", 1);
  g_scr_data.playerdisconnect = GScr_LoadScriptAndLabel(
      "maps/mp/gametypes/_callbacksetup", "CodeCallback_PlayerDisconnect", 1);
  g_scr_data.playerdamage = GScr_LoadScriptAndLabel(
      "maps/mp/gametypes/_callbacksetup", "CodeCallback_PlayerDamage", 1);
  g_scr_data.playerkilled = GScr_LoadScriptAndLabel(
      "maps/mp/gametypes/_callbacksetup", "CodeCallback_PlayerKilled", 1);
  g_scr_data.playerlaststand = GScr_LoadScriptAndLabel(
      "maps/mp/gametypes/_callbacksetup", "CodeCallback_PlayerLastStand", 1);

  /**************** Additional *************************/
  script_CallBacks_new[SCR_CB_SCRIPTCOMMAND] = GScr_LoadScriptAndLabel(
      "maps/mp/gametypes/_callbacksetup", "CodeCallback_ScriptCommand", 0);
}
...
__cdecl void GScr_LoadScripts(void) {
  ...
  g_scr_data.initstruct =
      GScr_LoadScriptAndLabel("codescripts/struct", "initstructs", 1);
  g_scr_data.createstruct =
      GScr_LoadScriptAndLabel("codescripts/struct", "createstruct", 1);

  GScr_LoadGameTypeScript();

  mapname = Cvar_RegisterString("mapname", "", CVAR_LATCH | CVAR_SYSTEMINFO,
...
__cdecl unsigned int Scr_LoadScript(const char* scriptname,
                                    PrecacheEntry* precache, int iarg_02) {
  ...
  Q_strncpyz(&callScriptStackNames[MAX_QPATH * callScriptStackPtr], scriptname,
             MAX_QPATH);
  ...
  if (FindVariable(scrStruct.var_03, handle)) {
    ...
    while it is still possible to fall back to default script if our extended
    functionality is not available.
    */

    Com_sprintf(filepath, sizeof(filepath), "%s.gsx",
                SL_ConvertToString(handle));
    scr_buffer_handle = Scr_AddSourceBuffer(SL_ConvertToString(handle),
                                            filepath, TempMalloc(0), 1);
    if (!scr_buffer_handle) {
      /*
      If no extended script was found just load traditional script (gsc)
      */
      Com_sprintf(filepath, sizeof(filepath), "%s.gsc",
                  SL_ConvertToString(handle));
      scr_buffer_handle = Scr_AddSourceBuffer(SL_ConvertToString(handle),
                                              filepath, TempMalloc(0), 1);
    }
...
void GetVirtualFunctionArray(){
    ...
    for(i = 0, ptr = (v_function_t*)0x821c620; ptr->offset != NULL && i < 25;
ptr++, i++){
...
funaddr, funtype); Q_strcat(buffer, sizeof(buffer), line);
...
void GetVirtualFunctionArray(){
    ...
    for(i = 0, ptr = (v_function_t*)0x826e060; ptr->offset != NULL; ptr++, i++){
        ...
        Com_sprintf(line, sizeof(line), "\tScr_AddFunction(\"%s\", (void*)%p,
%i);\n", funname, funaddr, funtype); Q_strcat(buffer, sizeof(buffer), line);
...
=\n{\n", j); Q_strcat(buffer, sizeof(buffer), line);

        for(i = 0; i < ptr->size; i++, subptr++)
        {
                Com_sprintf(line, sizeof(line), "\t{ NETF( %s ), %i, %i,
...
ptr->offset); Q_strcat(buffer, sizeof(buffer), line); Com_Printf("%s\n", line);
...
void StringIndexArray(){
    ...
    for(j=0, ptr = (stringindexcmd_t*)0x80a396e; ptr->mov3 != 0x90 ; ptr++,
j++){
...
void GetPlayerFieldArray(){
    ...
    for(j=0, ptr = (scrClientFields_t*)0x8215780; ptr->name != NULL; ptr++,
j++){

        Com_sprintf(line, sizeof(line), "\tScr_AddPlayerField(%s, %d, %d, %p,
...
void GetEntityFieldArray() {
  ...
  for (j = 0, ptr = (scrEntityFields_t*)0x82202a0; ptr->name != NULL;
       ptr++, j++) {
...
number[j],j); Q_strcat(buffer, sizeof(buffer), line);
...
/* only for debug */
__regparm3 void VM_Notify_Hook(int entid, int constString,
                               variableValue_t* arguments) {
...
void Scr_ClearArguments() {
  while (scrVmPub.numParams) {
    RemoveRefToValue(scrVmPub.argumentVariables->varType,
                     scrVmPub.argumentVariables->value);
...
void Scr_NotifyNum(int entityNum, unsigned int entType,
                   unsigned int constString, unsigned int numArgs) {
  int entVarNum;
...
void Scr_Notify(gentity_t* ent, unsigned short constString,
...
void RuntimeError_Debug(char* msg, char* a3, int a4) {
  ...
  if (scrVmPub.field_8) {
    for (i = scrVmPub.field_8 - 1; i > 0; --i) {
      ...
      Scr_PrintPrevCodePos(0, scrVmPub.backtrace[i].field_0,
                           scrVmPub.backtrace[i].field_4 == 0);
    }
