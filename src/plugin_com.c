...
P_P_F qboolean Plugin_IsLoaded(char* name) {
  ...
  for (i = 0; i < MAX_PLUGINS; ++i) {
    ...
    if (pluginFunctions.plugins[i].loaded) {
      if (Q_strncmp(pluginFunctions.plugins[i].name, name,
                    sizeof(pluginFunctions.plugins[i].name)) == 0) {
...
P_P_F qboolean Plugin_ExportFunction(char* name, void* (*function)()) {
  ...
  if (pID < 0) {
    Com_Printf(
        "^1Plugin_ExportFunction: Error! Called from an unknown plugin!\n");
    return qfalse;
  }
  if (pluginFunctions.plugins[pID].enabled == qfalse) {
    Com_Printf(
        "^3WARNING^7: Plugin_ExportFunction called from a disabled plugin!\n");
    return qfalse;
  }
  if (name == NULL || function == NULL) {
    Com_DPrintf(
        "Plugin_ExportFunction: Error - NULL argument! Plugin ID: %d.\n", pID);
  ...
  for (i = 0; i < pluginFunctions.plugins[pID].exports; ++i) {
    if (strncmp(pluginFunctions.plugins[pID].exportedFunctions[i].name, name,
                PLUGIN_COM_MAXNAMELEN) != 0) {
      Com_Printf("^3WARNING^7: Plugin of ID %d tried to export function \"%s\" "
                 "twice.\n",
                 pID, name);
      return qfalse;
    }
  ...
  pluginFunctions.plugins[pID]
      .exportedFunctions[pluginFunctions.plugins[pID].exports]
      .function = function;
  Q_strncpyz(pluginFunctions.plugins[pID]
                 .exportedFunctions[pluginFunctions.plugins[pID].exports]
                 .name,
             name, PLUGIN_COM_MAXNAMELEN);
...
P_P_F void* Plugin_ImportFunction(char* pluginName, char* name) {
  ...
  if (pID < 0) {
    Com_Printf(
        "^1Plugin_ImportFunction: Error! Called from an unknown plugin!\n");
    return NULL;
  }
  ...
  for (i = 0; i < MAX_PLUGINS; ++i) {
    if (strncmp(pluginFunctions.plugins[i].name, pluginName, 20) == 0) {
      for (j = 0; j < pluginFunctions.plugins[i].exports; ++j) {
        if (strncmp(pluginFunctions.plugins[i].exportedFunctions[j].name, name,
                    PLUGIN_COM_MAXNAMELEN) == 0) {
          Com_DPrintf(
              "^2Notice:^7 Plugin #%d imported plugin's #%d function \"%s\"\n.",
              pID, i, name);
