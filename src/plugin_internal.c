...
qboolean PHandler_TcpConnect(int pID, const char* remote, int connection) {
  if (pluginFunctions.plugins[pID].sockets[connection].sock < 1) {
    pluginFunctions.plugins[pID].sockets[connection].sock =
        NET_TcpClientConnect(remote);
    ...
    Com_DPrintf("PHandler_TcpConnect: Received socket %d @ %d\n",
                pluginFunctions.plugins[pID].sockets[connection].sock,
                connection);
    NET_StringToAdr(remote,
                    &pluginFunctions.plugins[pID].sockets[connection].remote,
                    NA_UNSPEC);
    return qtrue;
  }
  Com_PrintError(
      "Plugin_TcpConnect: Connection id %d is already in use for plugin #%d!\n",
      connection, pID);
...
int PHandler_TcpGetData(int pID, int connection, void* buf, int size) {
  int len;
  pluginTcpClientSocket_t* ptcs =
      &pluginFunctions.plugins[pID].sockets[connection];

  if (ptcs->sock < 1) {
    Com_PrintWarning(
        "Plugin_TcpGetData: called on a non open socket for plugin ID: #%d\n",
        pID);
    return -1;
  }
...
qboolean PHandler_TcpSendData(int pID, int connection, void* data, int len) {
  ...
  if (ptcs->sock < 1) {
    Com_PrintWarning(
        "Plugin_TcpSendData: called on a non open socket for plugin ID: #%d\n",
        pID);
    return qfalse;
  }
...
void PHandler_TcpCloseConnection(int pID, int connection) {
  ...
  if (ptcs->sock < 1) {
    Com_PrintWarning("Plugin_TcpCloseConnection: Called on a non open socket "
                     "for plugin ID: #%d\n",
                     pID);
    return;
  }
...
void PHandler_CmdExecute_f() {
  ...
  if (!pluginFunctions.enabled) {
    Com_DPrintf("Error! Tried executing a plugin command with plugins being "
                "disabled! Command name: '%s'.\n",
                Cmd_Argv(1));
    return;
  }
  char name[128];
...
void PHandler_RemoveCommand(int pID, char* name) {
  ...
  for (i = 0; i < j; i++) {
    if (strcmp(name, pluginFunctions.plugins[pID].cmd[i].name) == 0) {
      ...
      for (k = i; k < j - 1; k++) {
        pluginFunctions.plugins[pID].cmd[k] =
            pluginFunctions.plugins[pID].cmd[k + 1];
      }
      Com_DPrintf("Command '%s' removed for plugin %s.\n", name,
                  pluginFunctions.plugins[pID].name);
      return;
    }
  }
  Com_DPrintf("Warning: tried removing command '%s', which was not found for "
              "plugin %s.\n",
...
void* PHandler_Malloc(int pID, size_t size) {
  ...
  Com_DPrintf("Attempting to allocate %dB of memory for plugin #%d...\n", size,
              pID);
  // Plugin identified, find the first free spot in it's allocated pointers
  // table
  ...
  Com_Printf(
      "Plugins: Warning! Memory allocations limit reached for plugin #%d!\n",
      pID);
  return NULL;
}
void PHandler_Free(int pID, void* ptr) {
  ...
  int i;
  if (ptr == NULL) {
    Com_DPrintf("Plugins: Warning! Plugin #%d tried freeing a NULL pointer! "
                "Called Plugin_Free() twice?\n",
                pID);
    return;
  }
  // Plugin identified, find the first free spot in it's allocated pointers
  ...
  for (i = 0; i < PLUGIN_MAX_MALLOCS; i++) {
    if (pluginFunctions.plugins[pID].memory[i].ptr == ptr) {
      ...
      pluginFunctions.plugins[pID].usedMem -=
          pluginFunctions.plugins[pID].memory[i].size;
      --pluginFunctions.plugins[pID].mallocs;
      return;
    }
  }
...
void PHandler_FreeAll(int pID) {
  ...
  if (pID < 0) {
    Com_Printf(
        "Plugins: Error! Tried to free all memory of an unknown plugin!\n");
    return;
  }
  for (i = 0; i < PLUGIN_MAX_MALLOCS; ++i) {
    if (pluginFunctions.plugins[pID].memory[i].ptr != NULL) {
...
P_P_F void PHandler_Error(int pID, int code, char* string) {
  if (pluginFunctions.plugins[pID].enabled == qfalse) {
    Com_PrintWarning("An error of ID %d and string \"%s\" occured in a "
                     "disabled plugin with ID %d!\n",
                     code, string, pID);
    return;
  }
  switch (code) {
  case P_ERROR_WARNING:
    Com_Printf("Plugin #%d ('%s') issued a warning: \"%s\"\n", pID,
               pluginFunctions.plugins[pID].name, string);
    break;
  case P_ERROR_DISABLE:
    Com_Printf("Plugin #%d ('%s') returned an error and will be disabled! "
               "Error string: \"%s\".\n",
               pID, pluginFunctions.plugins[pID].name, string);
    pluginFunctions.plugins[pID].enabled = qfalse;
    break;
  case P_ERROR_TERMINATE:
    Com_Printf("Plugin #%d ('%s') reported a critical error, the server will "
               "be terminated. Error string: \"%s\".\n",
               pID, pluginFunctions.plugins[pID].name, string);
    Com_Error(ERR_FATAL, "%s", string);
    break;
  default:
    Com_DPrintf("Plugin #%d ('%s') reported an unknown error! Error string: "
                "\"%s\", error code: %d.\n",
...
void PHandler_PluginList_f() {
  ...
  if (pluginFunctions.loadedPlugins == 0) {
    ...
    Com_Printf("*--------------------------------------------------------------"
               "--------------------*\n");
    Com_Printf("| ID |         name         | enabled? | memory allocations | "
               "total all. mem. in B |\n");
    for (i = 0, j = 0; i < pluginFunctions.loadedPlugins; ++i, ++j) {
      while (j < MAX_PLUGINS) { // ORing might be dangerous when the compiler
                                // uses optimalization...
      ...
      Com_Printf("| %-3d| %-21s| %-9s| %-19d| %-21d|\n", j,
                 pluginFunctions.plugins[j].name,
                 pluginFunctions.plugins[j].enabled == 0 ? "no" : "yes",
                 pluginFunctions.plugins[j].mallocs,
                 pluginFunctions.plugins[j].usedMem);
    }
  ...
  Com_Printf("\nPlugin handler version: %d.%d.\n", PLUGIN_HANDLER_VERSION_MAJOR,
             PLUGIN_HANDLER_VERSION_MINOR);
}
/*
======
 Misc
...
int PHandler_CallerID() // Can now be inlined, why not ^^
{
  ...
  if(j<3){
  Com_Error(ERR_FATAL,"PHandler_CallerID: backtrace failed to return function
  pointers! Possible exploit detected! Terminating the server...\n");
  }
  for(i=0;i<MAX_PLUGINS;++i){
      if(pluginFunctions.plugins[i].lib_start < funcptrs[2] &&
  pluginFunctions.plugins[i].lib_start + pluginFunctions.plugins[i].lib_size >
  funcptrs[2]) return i;
  }
  return -1;*/
}
void PHandler_ScrFncExecute() {
  ...
  for (i = 0; i < MAX_PLUGINS; i++) {
    if (pluginFunctions.plugins[i].loaded &&
        pluginFunctions.plugins[i].enabled) {
      for (j = 0; j < pluginFunctions.plugins[i].cmds; j++)
        if (strcmp(name, pluginFunctions.plugins[i].scr_functions[j].name) ==
            0) {
          Com_DPrintf(
              "Executing plugin command '%s' for plugin '%s', plugin ID: %d.\n",
              name, pluginFunctions.plugins[i].name, i);
          func = (void (*)())(
              pluginFunctions.plugins[i].scr_functions[j].xcommand);
          pluginFunctions.hasControl = i;
          func();
          pluginFunctions.hasControl = PLUGIN_UNKNOWN;
          return;
...
void PHandler_ScrAddFunction(char* name, xfunction_t function, qboolean replace,
                             int pID) {
  int i;

  if (pID >= MAX_PLUGINS) {
    Com_Printf("Error: tried adding a script command for a plugin with non "
               "existent pID. pID supplied: %d.\n",
               pID);
    return;
  } else if (pID < 0) {
    Com_Printf("Plugin Handler error: Plugin_ScrAddFunction or "
               "Plugin_ScrReplaceFunction called from not within a plugin or "
               "from a disabled plugin!\n");
    return;
  }
  if (!pluginFunctions.plugins[pID].loaded) {
    Com_Printf(
        "Error: Tried adding a command for not loaded plugin! PID: %d.\n", pID);
  }
  Com_Printf(
      "Adding a plugin script function for plugin %d, command name: %s.\n", pID,
      name);
  for (i = 0; i < (sizeof(pluginFunctions.plugins[0].scr_functions) /
                   sizeof(pluginCmd_t));
       i++) {
  ...
  if (i == (sizeof(pluginFunctions.plugins[0].scr_functions) /
            sizeof(pluginCmd_t))) {
    Com_PrintError(
        "Exceeded maximum number of scrip-functions (%d) for plugin\n",
        (sizeof(pluginFunctions.plugins[0].scr_functions) /
         sizeof(pluginCmd_t)));
    return;
  }
  if (strlen(name) >=
      sizeof(pluginFunctions.plugins[0].scr_functions[0].name)) {
    Com_PrintError(
        "Exceeded maximum length of name for scrip-method: %s (%d) for plugin "
        "%s\n",
  ...
  if (Scr_AddFunction(name, PHandler_ScrFncExecute, qfalse) == qfalse) {
    Com_PrintError("Failed to add this script function: %s for plugin\n", name);
    return;
  }
  Q_strncpyz(pluginFunctions.plugins[pID].scr_functions[i].name, name,
             sizeof(pluginFunctions.plugins[0].scr_functions[0].name));
      ...
      for (j = 0; j < pluginFunctions.plugins[i].cmds; j++)
        if (strcmp(name, pluginFunctions.plugins[i].scr_methods[j].name) == 0) {
          Com_DPrintf(
              ...
              name, pluginFunctions.plugins[i].name, i);
          func =
              (void (*)())(pluginFunctions.plugins[i].scr_methods[j].xcommand);
          pluginFunctions.hasControl = i;
          func();
          pluginFunctions.hasControl = PLUGIN_UNKNOWN;
          return;
...
void PHandler_ScrAddMethod(char* name, xfunction_t function, qboolean replace,
                           int pID) {
  int i;
  ...
  if (pID >= MAX_PLUGINS) {
    ...
    Com_Printf(
        "Plugin Handler error: Plugin_ScrAddMethod or Plugin_ScrReplaceMethod "
        "called from not within a plugin or from a disabled plugin!\n");
    return;
  }
  if (!pluginFunctions.plugins[pID].loaded) {
    Com_Printf(
  ...
  Com_Printf("Adding a plugin script method for plugin %d, method name: %s.\n",
             pID, name);
  for (i = 0; i < (sizeof(pluginFunctions.plugins[0].scr_methods) /
                   sizeof(pluginCmd_t));
       i++) {
  ...
  if (i ==
    ...
    Com_PrintError(
        "Exceeded maximum number of scrip-methods (%d) for plugin\n",
        (sizeof(pluginFunctions.plugins[0].scr_methods) / sizeof(pluginCmd_t)));
    return;
  }
  if (strlen(name) >= sizeof(pluginFunctions.plugins[0].scr_methods[0].name)) {
    Com_PrintError("Exceeded maximum length of name for scrip-method: %s (%d) "
                   "for plugin %s\n",
                   name, sizeof(pluginFunctions.plugins[0].scr_methods[0].name),
                   name);
    return;
  }
  if (replace == qtrue) {
  ...
  if (Scr_AddMethod(name, PHandler_MetFncExecute, qfalse) == qfalse) {
    Com_PrintError("Failed to add this script function: %s for plugin\n", name);
    return;
  }
  Q_strncpyz(pluginFunctions.plugins[pID].scr_methods[i].name, name,
             sizeof(pluginFunctions.plugins[0].scr_methods[0].name));
