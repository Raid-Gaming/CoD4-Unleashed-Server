...
__optimize3 __regparm1 void SV_GetChallenge(netadr_t* from) {
  ...
  // Drop the authorize stuff if this client is coming in via IPv6 as the auth
  // server does not support ipv6. Drop also for addresses coming in on local
  // LAN and for stand-alone games independent from id's assets.
  if (challenge->adr.type == NA_IP && svse.authorizeAddress.type != NA_DOWN &&
      !Sys_IsLANAddress(from) && sv_authorizemode->integer != -1) {

    // look up the authorize server's IP
    if (svse.authorizeAddress.type == NA_BAD) {
      Com_Printf("Resolving %s\n", AUTHORIZE_SERVER_NAME);
      if (NET_StringToAdr(AUTHORIZE_SERVER_NAME, &svse.authorizeAddress,
                          NA_IP)) {
        svse.authorizeAddress.port = BigShort(PORT_AUTHORIZE);
        Com_Printf("%s resolved to %s\n", AUTHORIZE_SERVER_NAME,
                   NET_AdrToString(&svse.authorizeAddress));
      }
    }
    ...
    if (svse.authorizeAddress.type == NA_IP && challenge->adr.type == NA_IP &&
        NET_CompareBaseAdr(from, &svse.authorizeAddress)) {
      ...
      NET_OutOfBandPrint(NS_SERVER, from, "getIpAuthorize %i %s \"\" 0",
                         challenge->challenge, NET_AdrToStringShort(from));
      return;
    }
    ...
    // let them in, assuming the id server is down
      ...
      if (challenge->ipAuthorize == 0) {
        Com_PrintWarning("Activisions authorize server timed out - Accept "
                         "client without validating him\n");
      }
    } else {
      ...
      // the 0 is for backwards compatibility with obsolete sv_allowanonymous
      // flags getIpAuthorize <challenge> <IP> <game> 0 <auth-flag>
      NET_OutOfBandPrint(NS_SERVER, from, "needcdkey");

      NET_OutOfBandPrint(NS_SERVER, &svse.authorizeAddress,
                         "getIpAuthorize %i %s \"\" 0 PB \"%s\"",
                         challenge->challenge, NET_AdrToStringShort(from),
                         challenge->pbguid);
...
__optimize3 __regparm1 void SV_AuthorizeIpPacket(netadr_t* from) {
  ...
  // This binds the serveraddress to a specific socket - No longer will this
  // server be contacted over all sockets
  if (NET_GetDefaultCommunicationSocket() == NULL) {
    NET_RegisterDefaultCommunicationSocket(from);
  }
  ...
  if (!Q_stricmp(s, "demo")) {
    ...
    NET_OutOfBandPrint(NS_SERVER, &svse.challenges[i].adr,
  ...
  if (!Q_stricmp(s, "accept")) {
    if (Q_stricmp(p, svse.challenges[i].pbguid)) {
      ...
      NET_OutOfBandPrint(NS_SERVER, &svse.challenges[i].adr,
                         "challengeResponse %i", svse.challenges[i].challenge);
  ...
  NET_OutOfBandPrint(NS_SERVER, &svse.challenges[i].adr,
                     "print\n^1Auth Failed");
  NET_OutOfBandPrint(NS_SERVER, &svse.challenges[i].adr, "challengeResponse %i",
                     svse.challenges[i].challenge);
  svse.challenges[i].pingTime = com_frameTime;
  svse.challenges[i].ipAuthorize = -1; // CD-KEY was invalid
}
...
__optimize3 __regparm1 void SV_GetChallenge(netadr_t* from) {
  ...
  if (from->type == NA_IP && svse.authorizeAddress.type != NA_DOWN) {
    ...
    if (svse.authorizeAddress.type == NA_IP && from->type == NA_IP &&
        NET_CompareBaseAdr(from, &svse.authorizeAddress)) {
      // Reset the default socket so that this is forwarded to all sockets
      if (NET_GetDefaultCommunicationSocket() == NULL) {
        NET_RegisterDefaultCommunicationSocket(from);
      ...
      NET_OutOfBandPrint(NS_SERVER, from, "getIpAuthorize %i %s \"\" 0",
                         challenge, NET_AdrToStringShort(from));
      return;
    }
  }
  challenge = NET_CookieHash(from);
  clientChallenge = atoi(SV_Cmd_Argv(1));
  NET_OutOfBandPrint(NS_SERVER, from, "challengeResponse %d %d 0 xproto",
...
__optimize3 __regparm1 void SV_DirectConnect(netadr_t* from) {
  ...
  for (j = 0, cl = svs.clients; j < sv_maxclients->integer; j++, cl++) {
    ...
    } else if (NET_CompareBaseAdr(from, &cl->netchan.remoteAddress) &&
               cl->state == CS_CONNECTED) {
      NET_OutOfBandPrint(
          NS_SERVER, from,
          "error\nConnection refused:\nAn uncompleted connection from %s has "
          "been detected\nPlease try again later\n",
          NET_AdrToString(&cl->netchan.remoteAddress));
      Com_Printf(
          "Rejected connection from %s. This is a Fake-Player-DoS protection\n",
          NET_AdrToString(&cl->netchan.remoteAddress));
#ifdef COD4U
  ...
  if (!newcl && svse.challenges[c].pingTime) {
    ...
    if (sv_minPing->integer && ping < sv_minPing->integer) {
      ...
      NET_OutOfBandPrint(NS_SERVER, from,
                         "error\nServer is for high ping players only minping: "
                         "%i ms but your ping was: %i ms\n",
    ...
    if (sv_maxPing->integer && ping > sv_maxPing->integer) {
      NET_OutOfBandPrint(NS_SERVER, from,
                         "error\nServer is for low ping players only maxping: "
                         "%i ms but your ping was: %i ms\n",
  ...
  if (version != sv_protocol->integer && version != sv_protocol2->integer) {
    if (sv_protocol->integer == 6 && version < 6) {
      NET_OutOfBandPrint(
          ...
          "install the update to Call of Duty 4  v1.7",
          sv_protocol->integer);
    } else {
      NET_OutOfBandPrint(NS_SERVER, from,
                         "error\nServer uses a different protocol version: "
                         "%i\n You use protocol version: %i",
  ...
  if (sv_authorizemode->integer == 1 && svse.challenges[c].ipAuthorize == -1) {
    ...
    PHandler_Event(PLUGINS_ONPLAYERCONNECTAUTHFAIL, from,
                   &svse.challenges[c].pbguid, userinfo,
                   &svse.challenges[c].ipAuthorize, &pluginreject);
    if (pluginreject) {
      NET_OutOfBandPrint(NS_SERVER, from,
  ...
  if (version != sv_protocol->integer) {
    ...
    if (version < 7) {
      ...
      NET_OutOfBandPrint(
          ...
          "CoD4 and see on the main-menu if a new update is available\n",
          sv_protocol->integer);
      Com_Printf("rejected connect from version %i\n", version);
      return;
#ifdef COD4X18UPDATE
  ...
  if (version < 7) {
    ...
    Com_Printf("Rejected a connection: No or invalid GUID found/provided. "
               "Length: %i\n",
               strlen(pbguid));
#ifdef COD4U
    Com_Memset(&svse.challenges[c], 0, sizeof(svse.challenges[c]));
...
#ifdef COD4U
    PHandler_Event(PLUGINS_ONPLAYERWANTRESERVEDSLOT, from, pbguid, userinfo,
                   svse.challenges[c].ipAuthorize, &canreserved);
#else
    PHandler_Event(PLUGINS_ONPLAYERWANTRESERVEDSLOT, from, pbguid, userinfo, 0,
                   &canreserved);
#endif
  ...
  for (i = 0; i < 10;
       i++) { // Move waiting players up in queue if there is a purged slot
    if (svse.connectqueue[i].lasttime != 0) {
      if (svse.connectqueue[i + 1].challengeslot ==
          svse.connectqueue[i].challengeslot) {
      ...
      Com_Memcpy(&svse.connectqueue[i], &svse.connectqueue[i + 1],
                 (9 - i) * sizeof(connectqueue_t));
    }
  }
  for (i = 0; i < 10; i++) { // Find highest slot or the one which is already
...
#ifdef COD4X18UPDATE
  if (version == sv_protocol->integer || version == sv_protocol2->integer ||
      newcl->challenge != challenge || newcl->state != CS_CONNECTED) {
    Com_Memset(newcl, 0x00, sizeof(client_t));
  }
#else
  Com_Memset(newcl, 0x00, sizeof(client_t));
  ...
  //    Q_strncpyz(newcl->pbguid, ret, sizeof(newcl->pbguid));	// save the
  //    pbguid
  ...
  PHandler_Event(PLUGINS_ONPLAYERCONNECT, clientNum, from, newcl->originguid,
                 userinfo, newcl->authentication, denied, sizeof(denied));
  ...
  if (strstr(Cvar_GetVariantString("noPbGuids"), newcl->originguid) &&
      32 == strlen(newcl->originguid) && newcl->authentication == 1) {
    newcl->noPb = qtrue;
  }
  if (newcl->noPb == qfalse) {
    PunkBusterStatus = PbAuthClient(
        NET_AdrToString(from),
        atoi(Info_ValueForKey(userinfo, "cl_punkbuster")), newcl->pbguid);
    if (PunkBusterStatus) {
  ...
  if (newcl->protocol != sv_protocol->integer &&
      newcl->protocol != sv_protocol2->integer) {
  ...
  Netchan_Setup(NS_SERVER, &newcl->netchan, *from, qport, newcl->unsentBuffer,
                sizeof(newcl->unsentBuffer), newcl->fragmentBuffer,
                sizeof(newcl->fragmentBuffer));
...
#endif
  Com_Printf(
      "Going from CS_FREE to CS_CONNECTED for %s num %i guid %s from: %s\n",
      nick, clientNum, newcl->pbguid, NET_AdrToConnectionString(from));

  Q_strncpyz(newcl->xversion, Info_ValueForKey(userinfo, "xver"),
             sizeof(newcl->xversion));
  ...
  if (sv_modStats->boolean) {
    NET_OutOfBandPrint(NS_SERVER, from, "connectResponse %s",
  ...
  for (j = 0, cl = svs.clients, count = 0; j < sv_maxclients->integer;
       j++, cl++) {
...
__optimize3 __regparm2 void SV_ReceiveStats(netadr_t* from, msg_t* msg) {
  ...
  if (cl == NULL) {
    Com_DPrintf("SV_ReceiveStats: Received statspacket from disconnected "
                "remote client: %s qport: %d\n",
...
void SV_UserinfoChanged(client_t* cl) {
  ...
  if (!Q_isprintstring(cl->name) || strstr(cl->name, "ID_") ||
      strstr(cl->name, "&&") || strstr(cl->name, "///") ||
      Q_PrintStrlen(cl->name) < 3) {
    if (cl->state == CS_ACTIVE) {
      if (!Q_isprintstring(cl->name))
        SV_SendServerCommand(
            cl,
            "c \"^5Playernames can not contain advanced ASCII-characters\"");
      if (strlen(cl->name) < 3)
        SV_SendServerCommand(
            cl, "c \"^5Playernames can not be shorter than 3 characters\"");
    }
    if (cl->uid <= 0) {
      Com_sprintf(cl->name, 16, "CID_%i", cl - svs.clients);
      cl->usernamechanged = UN_NEEDUID;
    } else {
      Com_sprintf(cl->name, 16, "ID_%i:%i", cl->uid / 100000000,
                  cl->uid % 100000000);
...
__cdecl void SV_DropClient(client_t* drop, const char* reason) {
  ...
  if (drop->uid > 0 && strlen(drop->pbguid) == 32)
    SV_EnterLeaveLog(
        "^4Client %s %s ^4left this server from slot %d with uid %d guid %s",
        drop->name, NET_AdrToString(&drop->netchan.remoteAddress), clientnum,
        drop->uid, drop->pbguid);
  else if (drop->uid > 0)
    SV_EnterLeaveLog(
        "^4Client %s %s ^4left this server from slot %d with uid %d",
        drop->name, NET_AdrToString(&drop->netchan.remoteAddress), clientnum,
        drop->uid);
  else if (strlen(drop->pbguid) == 32)
    SV_EnterLeaveLog(
        "^4Client %s %s ^4left this server from slot %d with guid %s",
        drop->name, NET_AdrToString(&drop->netchan.remoteAddress), clientnum,
        drop->pbguid);
  ...
  SV_SendServerCommand(NULL, "%c \"\x15%s^7 %s%s\"\0", 0x65, clientName, var_01,
                       dropreason);
  ...
  drop->reliableSent = drop->reliableSequence =
      drop->reliableAcknowledge; // Reset unsentBuffer and Sequence to ommit the
                                 // outstanding junk from beeing transmitted
...
==================
*/
__optimize3 __regparm3 void SV_UserMove(client_t* cl, msg_t* msg,
                                        qboolean delta) {
  int i, key, clientNum;
  ...
  //	key ^= Com_HashKey( extcl->reliableCommands[ cl->reliableAcknowledge & (
  // MAX_RELIABLE_COMMANDS - 1 ) ].command, 32 );
...
void SV_ClientEnterWorld(client_t* client, usercmd_t* cmd) {
  ...
  /*
          if(client->netchan.remoteAddress.type != NA_BOT && ((sv_pure->integer
     != 0 && client->pureAuthentic == 0) || !psvs.serverAuth)) return;
  */
  ...
  if (client->netchan.remoteAddress.type != NA_BOT && sv_pure->integer != 0 &&
  ...
  if (sv_autodemorecord->boolean && !client->demorecording &&
      (client->netchan.remoteAddress.type == NA_IP ||
       client->netchan.remoteAddress.type == NA_IP6)) {
    if (psvs.useuids) {
  ...
  if (!client->enteredWorldForFirstTime &&
    ...
    if (client->uid > 0) {
      SV_EnterLeaveLog(
          "^5Client %s %s ^5entered this server in slot %d with uid %d",
          client->name, NET_AdrToString(&client->netchan.remoteAddress),
          clientNum, client->uid);
      client->power = Auth_GetClPowerByUID(client->uid);
    } else {
      SV_EnterLeaveLog(
          "^5Client %s %s ^5entered this server in slot %d with guid %s",
          client->name, NET_AdrToString(&client->netchan.remoteAddress),
          clientNum, client->pbguid);
    }
  }
...
sharedEntity_t* SV_AddBotClient() {
  ...
  *userinfo = 0;
  ...
  // init the netchan queue
  Netchan_Setup(NS_SERVER, &cl->netchan, botnet, qport, cl->unsentBuffer,
                sizeof(cl->unsentBuffer), cl->fragmentBuffer,
                sizeof(cl->fragmentBuffer));

  /*	for(index = 0; index < MAX_RELIABLE_COMMANDS; index++ ){
                  if(index < MAX_RELIABLE_COMMANDS / 2){
                          cl->reliableCommands[index] =
     &cl->lowReliableCommands[index & (MAX_RELIABLE_COMMANDS - 1)]; } else {
                          cl->reliableCommands[index] =
     &svse.extclients[i].highReliableCommands[index & (MAX_RELIABLE_COMMANDS -
     1)];
                  }
          }
  */
...
char* SV_IsGUID(char* GUID) {
  ...
  j = 0;
  while (j < k) {
    if (GUID[j] < 0x30 || GUID[j] > 0x66 ||
        (GUID[j] < 0x41 && GUID[j] > 0x39) ||
        (GUID[j] < 0x61 && GUID[j] > 0x46)) {
      return NULL;
    }
    j++;
  }
...
void SV_WWWRedirect(client_t* cl, msg_t* msg) {
  ...
  Com_sprintf(cl->wwwDownloadURL, sizeof(cl->wwwDownloadURL), "%s/%s",
              sv_wwwBaseURL->string, cl->downloadName);
...
__cdecl void SV_WriteDownloadToClient(client_t* cl, msg_t* msg) {
  ...
  if (!cl->download) {
    ...
    if (!FS_VerifyPak(cl->downloadName)) {
      // will drop the client and leave it hanging on the other side. good for
      // him
      ...
      if (!sv_allowDownload->integer) {
        ...
        if (sv_pure->integer) {
          Com_sprintf(errorMessage, sizeof(errorMessage),
                      "EXE_AUTODL_SERVERDISABLED_PURE\x15%s", cl->downloadName);
        } else {
          Com_sprintf(errorMessage, sizeof(errorMessage),
                      "EXE_AUTODL_SERVERDISABLED\x15%s", cl->downloadName);
        }
  ...
  while (cl->downloadCurrentBlock - cl->downloadClientBlock <
    ...
    if (!cl->downloadBlocks[curindex]) {
      ...
      if (!cl->downloadBlocks[curindex]) { // Crash fix for download subsystem
        SV_DropClient(cl, "Failed to allocate a new chunk of memory for the "
                          "serverdownloadsystem");
        return;
      }
    }
  ...
  if (cl->downloadBlockSize[curindex]) {
    if (!cl->downloadBlocks[curindex]) { // Crash evaluation for download
                                         // subsystem
      Com_PrintError(
          "FATAL Server error in SV_WriteDownloadToClient.\nClient: %i, Name: "
          "%s, File: %s, CLDlBlock: %i, DlSize: %i, DlBlkSize: %i, DlSendTime: "
          "%i, ServerTime: %i, XmitBlock: %i, ClientState: %i, CurIndex: %i",
          cl - svs.clients, cl->name, cl->downloadName, cl->downloadClientBlock,
          cl->downloadSize, cl->downloadBlockSize[curindex],
          cl->downloadSendTime, svs.time, cl->downloadXmitBlock, cl->state,
          curindex);
...
__optimize3 __regparm2 void SV_ExecuteClientMessage(client_t* cl, msg_t* msg) {
  int c, clnum;
  int serverId;
  static const char* clc_strings[256] = {
      "clc_move", "clc_moveNoDelta", "clc_clientCommand", "clc_EOF", "clc_nop"};
  ...
  decompressMsg.cursize = MSG_ReadBitsCompress(
      msg->data + msg->readcount, msg->cursize - msg->readcount,
      decompressMsg.data, decompressMsg.maxsize);
  ...
  if (serverId != sv_serverId && !cl->wwwDl_var01 && !cl->wwwDownloadStarted &&
      !cl->wwwDl_var02) {
    if ((serverId & 0xf0) != (sv_serverId & 0xf0)) {
      while (qtrue) {
        ...
        if (c == clc_clientCommand) {
          if (!SV_ClientCommand(cl, &decompressMsg, 1) ||
              cl->state == CS_ZOMBIE)
...
=================
*/
static void SV_ResetPureClient_f(client_t* cl) { cl->pureAuthentic = 0; }
...
void SV_WWWDownload_f(client_t* cl) {
  ...
  if (!Q_stricmp(download, "ack")) {
    ...
    *cl->downloadName = 0;
    ...
    Com_PrintWarning("Client '%s' reported that the http download of '%s' "
                     "failed, falling back to a server download\n",
    ...
    *cl->downloadName = 0;
    ...
    Com_PrintWarning(
        "Client '%s' reports that the redirect download for '%s' had wrong "
        "checksum.\n        You should make sure that your files on your "
        "redirect are the same files you have on your server\n",
        cl->name, cl->downloadName);
    ...
    *cl->downloadName = 0;
    ...
    Com_PrintWarning(
        "SV_WWWDownload: Unknown wwwdl subcommand '%s' for client '%s'\n",
        download, cl->name);
    SV_DropClient(cl, "Unexpected www download message.");
  }
}
...
void SV_Stats_f(client_t* cl) {
  ...
  MSG_ReadBase64(
      &msg, &cl->stats[STATS_PART_SIZE * (cl->receivedstats & NUM_STATS_MASK)],
      STATS_PART_SIZE);
...
static ucmd_t ucmds[] = {{"userinfo", SV_UpdateUserinfo_f, 0},
...
qboolean SV_ClientCommand(client_t* cl, msg_t* msg, qboolean inDl) {
  ...
  if (!sv_floodProtect->boolean || cl->state == CS_PRIMED ||
      cl->netchan.remoteAddress.type == NA_LOOPBACK
      // NERVE - SMF - some server game-only commands we cannot have flood
      // protect
      || !Q_strncmp("team", s, 4) || !Q_strncmp("mr", s, 2) ||
      !Q_strncmp("score", s, 5)) {
  ...
  Q_strncpyz(cl->lastClientCommandString, s,
...
void SV_SendClientVoiceData(client_t* client) {
  ...
  NET_OutOfBandData(NS_SERVER, &client->netchan.remoteAddress, msg.data,
                    msg.cursize);
