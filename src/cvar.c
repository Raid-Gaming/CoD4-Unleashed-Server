...
void Cvar_ValueToStr(cvar_t const* cvar, char* bufvalue, int sizevalue,
                     char* bufreset, int sizereset, char* buflatch,
                     int sizelatch);
...
static qboolean Cvar_ValidateNewVar(const char* var_name, cvarType_t type,
                                    CvarValue* value, CvarLimits* limits) {
  ...
  case CVAR_BOOL:
    ...
    return retval;
  case CVAR_FLOAT:
    if (isnan((*limits).fmax) || isnan((*limits).fmin) ||
        isnan((*value).floatval)) {
      (*value).floatval = 0.0;
      (*limits).fmax = 0.0;
      (*limits).fmin = 0.0;
      return qfalse;
  ...
  case CVAR_ENUM:
    ...
    for (i = 0;
         (*value).enumval.strings[i] != NULL && i < (*value).enumval.integer;
         i++)
      ;
...
void Cvar_Deallocate( cvar_t *var )
{
        ...
        while ( 1 ) {
                ...
                if ( !othervar ) {
                        ...
                        Com_Error(ERR_FATAL, "Cvar_Deallocate: Tried to unlink a
cvar which does not exist"); return;
                }
                if ( var == othervar ) {
                        *back = var->next;
                        return;
...
static cvar_t* Cvar_Register(const char* var_name, cvarType_t type,
                             unsigned short flags, CvarValue value,
                             CvarLimits limits, const char* description) {
  cvar_t* var;
  qboolean isvalid;
  long hash;
  cvar_t* safenext;
  ...
  if (var &&
      (((flags & CVAR_ROM) || (flags & CVAR_CHEAT && !cheating_enabled)) &&
    ...
    */
    if (var->type != type && !(var->flags & CVAR_USER_CREATED)) {
      Com_Error(ERR_FATAL,
                "Cvar_Register: Tried to reregister an already registered Cvar "
                "\'%s\' as a different type\n",
                var_name);
      return NULL;
    }
    /* Get the old value from string var */
    /* We change the type of a user created Cvar. Free the strings and keep the
     * current value for later eval */
...
static int Cvar_SetVariant(cvar_t* var, CvarValue value, qboolean force) {
  ...
  // note what types of cvars have been modified (userinfo, archive, serverinfo,
  // systeminfo)
  cvar_modifiedFlags |= var->flags;
  var->modified = qtrue;
  return 1;
...
============
*/
void Cvar_SetWithType(const char* var_name, CvarValue value, cvarType_t type,
                      qboolean force) {
  cvar_t* var;
  ...
  if (var) {
    if (type == var->type) {
      ...
      Com_Error(ERR_FATAL,
                "Cvar_SetWithType: Existing Cvar %s has a different type. Old "
                "type: %d New type: %d",
  ...
  for (var = cvar_vars; var; var = var->next) {
    if (var->flags & CVAR_CHEAT) {
      ...
      if (var->flags & CVAR_LATCH)
        Cvar_SetVariant(
            var, cval,
            qfalse); /* Don't force latched Cvars as this can be dangerous */
      else
        Cvar_SetVariant(var, cval, qtrue);
    }
  }
...
void Cvar_ValueToStr(cvar_t const* cvar, char* bufvalue, int sizevalue,
  ...
  case CVAR_BOOL:
    if (bufvalue) {
      if (cvar->boolean)
        //					Com_sprintf(bufvalue, sizevalue,
        //"true");
    ...
    if (bufreset) {
      if (cvar->resetBoolean)
        //					Com_sprintf(bufreset, sizereset,
        //"true");
    ...
    if (buflatch) {
      if (cvar->latchedBoolean)
        //					Com_sprintf(buflatch, sizelatch,
        //"true");
  ...
  case CVAR_FLOAT:
    if (bufvalue)
      Com_sprintf(bufvalue, sizevalue, "%g", cvar->floatval);
    if (bufreset)
      Com_sprintf(bufreset, sizereset, "%g", cvar->resetFloatval);
    if (buflatch)
      Com_sprintf(buflatch, sizelatch, "%g", cvar->latchedFloatval);
    return;
  case CVAR_VEC2:
    if (bufvalue)
      Com_sprintf(bufvalue, sizevalue, "%g %g", cvar->vec2[0], cvar->vec2[1]);
    if (bufreset)
      Com_sprintf(bufreset, sizereset, "%g %g", cvar->resetVec2[0],
                  cvar->resetVec2[1]);
    if (buflatch)
      Com_sprintf(buflatch, sizelatch, "%g %g", cvar->latchedVec2[0],
                  cvar->latchedVec2[1]);
    return;
  case CVAR_VEC3:
    if (bufvalue)
      Com_sprintf(bufvalue, sizevalue, "%g %g %g", cvar->vec3[0], cvar->vec3[1],
                  cvar->vec3[2]);
    if (bufreset)
      Com_sprintf(bufreset, sizereset, "%g %g %g", cvar->resetVec3[0],
                  cvar->resetVec3[1], cvar->resetVec3[2]);
    if (buflatch)
      Com_sprintf(buflatch, sizelatch, "%g %g %g", cvar->latchedVec3[0],
                  cvar->latchedVec3[1], cvar->latchedVec3[2]);
    return;
  case CVAR_VEC4:
    if (bufvalue)
      Com_sprintf(bufvalue, sizevalue, "%g %g %g %g", cvar->vec4[0],
                  cvar->vec4[1], cvar->vec4[2], cvar->vec4[3]);
    if (bufreset)
      Com_sprintf(bufreset, sizereset, "%g %g %g %g", cvar->resetVec4[0],
                  cvar->resetVec4[1], cvar->resetVec4[2], cvar->resetVec4[3]);
    if (buflatch)
      Com_sprintf(buflatch, sizelatch, "%g %g %g %g", cvar->latchedVec4[0],
                  cvar->latchedVec4[1], cvar->latchedVec4[2],
                  cvar->latchedVec4[3]);
    return;
  case CVAR_COLOR:
    if (bufvalue)
      Com_sprintf(bufvalue, sizevalue, "%.3g %.3g %.3g %.3g",
                  (float)cvar->color.red / (float)0xff,
                  (float)cvar->color.green / (float)0xff,
                  (float)cvar->color.blue / (float)0xff,
                  (float)cvar->color.alpha / (float)0xff);
    if (bufreset)
      Com_sprintf(bufreset, sizereset, "%.3g %.3g %.3g %.3g",
                  (float)cvar->resetColor.red / (float)0xff,
                  (float)cvar->resetColor.green / (float)0xff,
                  (float)cvar->resetColor.blue / (float)0xff,
                  (float)cvar->resetColor.alpha / (float)0xff);
    if (buflatch)
      Com_sprintf(buflatch, sizelatch, "%.3g %.3g %.3g %.3g",
                  (float)cvar->latchedColor.red / (float)0xff,
                  (float)cvar->latchedColor.green / (float)0xff,
                  (float)cvar->latchedColor.blue / (float)0xff,
                  (float)cvar->latchedColor.alpha / (float)0xff);
    return;
  case CVAR_INT:
    if (bufvalue)
      Com_sprintf(bufvalue, sizevalue, "%d", cvar->integer);
    if (bufreset)
      Com_sprintf(bufreset, sizereset, "%d", cvar->resetInteger);
    if (buflatch)
      Com_sprintf(buflatch, sizelatch, "%d", cvar->latchedInteger);
    return;
  case CVAR_STRING:
    if (bufvalue)
      Com_sprintf(bufvalue, sizevalue, "%s", cvar->string);
    if (bufreset)
      Com_sprintf(bufreset, sizereset, "%s", cvar->resetString);
    if (buflatch)
      Com_sprintf(buflatch, sizelatch, "%s", cvar->latchedString);
    return;
  case CVAR_ENUM:
    if (cvar->enumStr) {
      if (bufvalue) {
...
qboolean Cvar_Command(void) {
  ...
  if (Cmd_Argc() == 1) {
    Cvar_ValueToStr(v, value, sizeof(value), reset, sizeof(reset), latch,
                    sizeof(latch));

    if (v->description != NULL)
      Com_Printf("\"%s\" is: \"%s" S_COLOR_WHITE
                 "\" default: \"%s" S_COLOR_WHITE "\" info: \"%s" S_COLOR_WHITE
                 "\"\n",
                 v->name, value, reset, v->description);
    else
      Com_Printf("\"%s\" is: \"%s" S_COLOR_WHITE
                 "\" default: \"%s" S_COLOR_WHITE "\"\n",
                 v->name, value, reset);
...
void Cvar_WriteVariables(fileHandle_t f) {
  ...
  for (var = cvar_vars; var; var = var->next) {
    if (var->flags & CVAR_ARCHIVE) {
      ...
      Cvar_ValueToStr(var, bufferval, sizeof(bufferval), NULL, 0, bufferlatch,
                      sizeof(bufferlatch));
...
cvar_t* Cvar_RegisterString(const char* name, const char* string,
...
cvar_t* Cvar_RegisterBool(const char* name, qboolean boolean,
                          unsigned short flags, const char* description) {
...
cvar_t* Cvar_RegisterInt(const char* name, int integer, int min, int max,
                         unsigned short flags, const char* description) {
...
cvar_t* Cvar_RegisterFloat(const char* name, float val, float min, float max,
                           unsigned short flags, const char* description) {
...
cvar_t* Cvar_RegisterVec2(const char* name, float x, float y, float min,
...
cvar_t* Cvar_RegisterVec3(const char* name, float x, float y, float z,
                          float min, float max, unsigned short flags,
                          const char* description) {
...
cvar_t* Cvar_RegisterVec4(const char* name, float x, float y, float z,
                          float imag, float min, float max,
                          unsigned short flags, const char* description) {
...
cvar_t* Cvar_RegisterColor(const char* name, float r, float g, float b,
                           float alpha, unsigned short flags,
                           const char* description) {
...
cvar_t* Cvar_RegisterEnum(const char* name, const char** strings, int integer,
                          unsigned short flags, const char* description) {
...
void Cvar_ClearModified(cvar_t* cvar) { cvar->modified = 0; }
...
void Cvar_Init(void) {
  ...
  cvar_cheats =
      Cvar_RegisterBool("cvar_cheats", qfalse, CVAR_INIT, "Enable cheating");
...
void Cvar_ChangeResetValue(cvar_t* var, CvarValue value) {
  ...
  switch (var->type) {
  case CVAR_BOOL:
    if ((var->resetBoolean && value.boolean) ||
        (!var->resetBoolean && !value.boolean))
      return;
...
void Cvar_AddFlags(cvar_t* var, unsigned short flags) { var->flags |= flags; }
