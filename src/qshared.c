...
Same like strcpy but it does always a byte wise copying so source and
destiantion can overlap Use is to fix up a problem with Info_SetValueForKey()
which happened on some newer machines
=============
*/
  ...
  while (1) {
    ...
    *o = 0;
    ...
    if (!strcmp(key, pkey)) {
      Q_bstrcpy(
          start,
          s); // remove this part - Bugfix using Q_bstrcpy() instaed strcpy()
      return;
    }
...
void Info_SetValueForKey(char* s, const char* key, const char* value) {
  ...
  if (strlen(s) >= MAX_INFO_STRING) {
    Com_PrintWarning(
        "Unexpected error - Info_SetValueForKey: oversize infostring");
  }
...
static void Info_Encode(const char* inurl, int encodelen, char* outencodedurl,
                        int len) {
  int i, y;
...
int BigInfo_DecodedValueForKey(const char* s, const char* key, char* out,
                               int outbuflen) {
  const char* value;
...
char* Com_ParseGetToken(char* line) {
  ...
  if (parse_lastpos == line) {
    ...
    if (parse_inquotes) { // In case we are inside quotes step until the end
                          // quote forward
  ...
  if (*line ==
      '"') { // Check if the next token is the beginning of a quoted string
...
qboolean isNumeric(const char* string, int size) {
  ...
  if (size >
      0) { // If we have given a length compare the string limited by the length
...
void stack_init(void* array[], size_t size) {
  array[0] =
      (void*)((size_t)array +
              size); // Moving the stackpointer in array[0] to top of stack
}
...
void XML_Init(xml_t* base, char* s, int size, char* encoding) {
  ...
  Com_sprintf(version, sizeof(version),
...
qboolean QDECL XML_OpenTag(xml_t* base, char* root, int count, ...) {
  ...
  if (!stack_push(base->stack, sizeof(base->stack),
                  base->buffer + base->bufposition + 1)) {
...
void XML_CloseTag(xml_t* base) {
  ...
  if (base->parents == 0) {
    Com_PrintError("XML_CloseTag: Invalid close attempt in XML. Attempt to "
                   "close more elements than you have opened.\n");
  ...
  for (i = 0; *root != ' ' && *root != 0 && *root != '>' && i < sizeof(buffer);
       stringptr++, root++, i++)
    *stringptr = *root;
  *stringptr = 0;
  if (base->last) {
