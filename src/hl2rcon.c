...
void HL2Rcon_SourceRconDisconnect(netadr_t* from, int connectionId) {
  ...
  if (connectionId < 0 || connectionId >= MAX_RCONUSERS) {
    Com_Error(ERR_FATAL, "HL2Rcon_SourceRconDisconnect: bad connectionId: %i",
              connectionId);
    return;
  }
...
tcpclientstate_t HL2Rcon_SourceRconAuth(netadr_t* from, msg_t* msg,
                                        int* connectionId) {
  ...
  for (i = 0, user = sourceRcon.activeRconUsers; i < MAX_RCONUSERS;
       i++, user++) {
...
void HL2Rcon_SourceRconSendConsole(const char* data, int msglen) {
  HL2Rcon_SourceRconSendDataToEachClient((const byte*)data, msglen,
                                         SERVERDATA_CONLOG);
}
...
void HL2Rcon_SourceRconSendDataToEachClient(const byte* data, int msglen,
                                            int type) {
...
void HL2Rcon_SourceRconSendChatToEachClient(const char* text, rconUser_t* self,
                                            int cid, qboolean onlyme) {
  ...
  if ((power = Auth_GetClPowerByUID(uid)) < 100) {
    ...
    if (powercmd > power) {
      Com_BeginRedirect(sv_outputbuf, SV_OUTPUTBUF_LENGTH,
                        HL2Rcon_SourceRconFlushRedirect);
...
qboolean HL2Rcon_SourceRconEvent(netadr_t* from, msg_t* msg, int connectionId) {
  ...
  while (msg->readcount < msg->cursize) {
    ...
    case SERVERDATA_TURNONSTREAM:
      ...
      sourceRcon.redirectUser = connectionId + 1;
      Com_BeginRedirect(sv_outputbuf, sizeof(sv_outputbuf),
                        HL2Rcon_SourceRconFlushRedirect);
    ...
    case SERVERDATA_SAY:
      ...
      team = MSG_ReadByte(
          msg); // teamnumber or -1 if it is for all team or clientnum is set
    ...
    default:
      ...
      Com_Printf("Not a valid source rcon packet from: %s received. Type: %d - "
                 "Closing connection\n",
...
void HL2Rcon_Init() {
  ...
  NET_TCPAddEventType(HL2Rcon_SourceRconEvent, HL2Rcon_SourceRconAuth,
                      HL2Rcon_SourceRconDisconnect, 9038723);
