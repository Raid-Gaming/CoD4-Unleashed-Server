...
qboolean Scr_FS_AlreadyOpened(char* qpath) {
  ...
  for (i = 0; i < MAX_SCRIPT_FILEHANDLES; i++) {
    ...
    if (!Q_stricmp(qpath, scr_fsh[i].filename)) {
      Com_PrintScriptRuntimeWarning("Script_FileOpen: Tried to open a file "
                                    "with the same name two times: %s\n",
                                    qpath);
      return qtrue;
    }
  }
...
fileHandle_t Scr_OpenScriptFile(char* qpath, scr_fileHandleType_t ft,
  ...
  Com_PrintScriptRuntimeWarning(
      "Scr_OpenScriptFile: Exceeded limit of %i opened files\n",
