...
int ReliableMessagesGetAcknowledge(framedata_t* frame) {
  ...
  for (i = 0; i < frame->windowsize; ++i) {
    if (frame->fragments[(i + frame->sequence) % frame->bufferlen].ack !=
        i + frame->sequence) {
      break;
    }
  }
...
void ReliableMessagesTransmitNextFragment(netreliablemsg_t* chan) {
  ...
  if (chan->txwindow.acknowledge == chan->txwindow.sequence) {
    // Let the remote end still know about the current acknowledge state even
    // when nothing is going to be sent
  ...
  if (chan->txwindow.fragments[sequence % chan->txwindow.bufferlen].ack ==
      sequence) {
    ...
    MSG_WriteLong(&buf,
                  chan->rxwindow.sequence); // Acknowledge for the other end
    ReliableMessageWriteSelectiveAcklist(&chan->rxwindow, &buf);
    MSG_WriteShort(&buf, chan->txwindow.windowsize);
    MSG_WriteShort(&buf,
                   chan->txwindow.fragments[sequence % chan->txwindow.bufferlen]
                       .len); // Fragment size
    MSG_WriteData(
        &buf,
        chan->txwindow.fragments[sequence % chan->txwindow.bufferlen].data,
  ...
  ++chan->txwindow.frame;
  if (chan->txwindow.frame >=
      chan->txwindow.acknowledge + chan->txwindow.windowsize) {
    chan->txwindow.frame = chan->txwindow.acknowledge;
  }
}
...
void ReliableMessagesReceiveNextFragment(netreliablemsg_t* chan, msg_t* buf) {
  ...
  if (sequence >= chan->rxwindow.sequence + chan->rxwindow.windowsize) {
    Com_PrintError("Illegible sequence - got: %d max: %d\n", sequence,
                   chan->rxwindow.sequence + chan->rxwindow.windowsize);
    return;
  }
  ...
  if (acknowledge > chan->txwindow.sequence) {
    Com_PrintError(
        "Invalid reliable acknowledge. acknowledge(%d) > sequence(%d)\n",
  ...
  for (i = 0; i < numselectiveack; ++i) {
    ...
    if (startack + length > acknowledge + chan->txwindow.windowsize) {
      Com_PrintError(
          "Selective acknowledge %d is out of windowsize acknowledge %d\n",
          startack + length, acknowledge);
      return;
    }
    Com_Printf("SACK: %d %d\n", startack, length);
    for (j = 0; j < length; ++j) {
      chan->txwindow.fragments[(startack + j) % chan->txwindow.bufferlen].ack =
          startack + j;
    }
  }
  ...
  if (chan->txwindow.acknowledge < acknowledge) {
    ...
    chan->txwindow.acknowledge = acknowledge;
    Com_Printf(
        "^5Acknowledge is now %d Top is now: %d Remaining fragments are %d\n",
        chan->txwindow.acknowledge, chan->txwindow.sequence,
        chan->txwindow.sequence - chan->txwindow.acknowledge);
  }
  ...
  chan->rxwindow.fragments[sequence % chan->rxwindow.bufferlen].len =
      fragmentsize;
  MSG_ReadData(
      buf, chan->rxwindow.fragments[sequence % chan->rxwindow.bufferlen].data,
      chan->rxwindow.fragments[sequence % chan->rxwindow.bufferlen].len);
...
int ReliableMessageReceive(netreliablemsg_t* chan, byte* outdata, int len) {
  ...
  for (i = 0; i < numfragments; ++i) {
    ...
    memcpy(outdata + writepos, chan->rxwindow.fragments[index].data,
           chan->rxwindow.fragments[index].len);
...
int ReliableMessageSend(netreliablemsg_t* chan, byte* indata, int len) {
  ...
  for (i = 0; i < freefragmentcnt; ++i) {
    ...
    memcpy(chan->txwindow.fragments[index].data, indata + i * MAX_FRAGMENT_SIZE,
           slen);
...
void ReliableMessageSetup(netreliablemsg_t* chan, int qport, int netsrc,
                          netadr_t* remote) {
  fragment_t* dynrxmem;
  fragment_t* dyntxmem;
  ...
  memset(chan->txwindow.fragments, -1,
         chan->txwindow.bufferlen * sizeof(fragment_t));
  memset(chan->rxwindow.fragments, -1,
         chan->rxwindow.bufferlen * sizeof(fragment_t));
...
void Net_TestingFunction(netreliablemsg_t* chan) {
  ...
  numbytes = ReliableMessageSend(chan, ((byte*)testdata) + sendpos,
                                 sizeof(testdata) - sendpos);
  sendpos += numbytes;
  if (numbytes > 0)
