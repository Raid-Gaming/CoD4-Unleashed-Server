...
typedef struct {    //(0x2146c);
  ...
  unsigned int
      messageSent; // (0x243e0 | 0x2f74) time the message was transmitted
...
typedef enum { UN_VERIFYNAME, UN_NEEDUID, UN_OK } username_t;
#pragma pack(1)
...
typedef struct client_s { // 90b4f8c
  clientState_t state;
  int unksnapshotvar; // must timeout a few frames in a row so debugging doesn't
                      // break
  ...
  int reliableSequence; // (0x20e50)last added reliable message, not necesarily
                        // sent or acknowledged yet
  ...
  int nextReliableTime; // (0x21454) svs.time when another reliable command will
                        // be allowed
  ...
  int nextSnapshotTime; // (0x21464) send another snapshot when svs.time >=
                        // nextSnapshotTime
  int timeoutCount;
  clientSnapshot_t
      frames[PACKET_BACKUP]; // (0x2146c) updates can be delta'd from here
  int ping;                  //(0x804ec)
...
typedef struct { // 0x8c51780
  ...
  int numSnapshotEntities; // 0xba0de8c
...
typedef struct { // 0x13e78d00
  ...
  int frameusec; // Frameusec set every Level-startup to the desired value from
                 // sv_fps
  qboolean restarting; // if true, send configstring changes during SS_LOADING
  int serverId;        // restartedServerId;	serverId before a map_restart
  int checksumFeed;    // 0x14 the feed key that we use to compute the pure
                       // checksum strings
  // https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=475
  // the serverId associated with the current checksumFeed (always <= serverId)
  /*	int			checksumFeedServerId;
          int			snapshotCounter;	// incremented for each
     snapshot built

          int			nextFrameTime;		// when time >
     nextFrameTime, process world struct cmodel_s
     *models[MAX_MODELS];*/
  byte unk[0x800];
  ...
  int gameClientSize; // 0x5fb40 (13ed8840)will be > sizeof(playerState_t) due
                      // to game private data
  /*
          int				restartTime;
          int				time;*/
...
typedef struct { // 13F18F80
  ...
  int time;
  int snapFlagServerBit;   // ^= SNAPFLAG_SERVERCOUNT every SV_SpawnServer()
  int numSnapshotEntities; // 0x13f18f8c
                           // sv_maxclients->integer*PACKET_BACKUP*MAX_PACKET_ENTITIES
  int numSnapshotClients;
...
void __cdecl SV_GameSendServerCommand(int clientnum, int svscmd_type,
                                      const char* text);
void __cdecl SV_SetConfigstring(int index, const char* text);
...
__cdecl void SV_ClipMoveToEntity(struct moveclip_s* clip, svEntity_t* entity,
