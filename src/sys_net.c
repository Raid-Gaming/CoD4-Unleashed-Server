...
void NET_TCPConnectionClosed(netadr_t* adr, int sock, int connectionId,
                             int serviceId) {}
#endif
#ifndef __NET_GAME_H__
...
qboolean NET_TCPPacketEvent(netadr_t* remote, byte* bufData, int cursize,
                            int* sock, int connectionId, int serviceId) {
  return qfalse;
}
#endif
...
#define MAX_TCPCONNECTEDTIMEOUT                                                \
  1800000 // 30 minutes - close this if we have too many waiting connections
...
typedef struct {
  fd_set fdr;
  int highestfd;
  int activeConnectionCount; // Connections that have been successfully
                             // authentificated
...
__optimize3 __regparm3 static void
SockadrToNetadr(struct sockaddr* s, netadr_t* a, qboolean tcp, int socket) {
...
static qboolean Sys_StringToSockaddr(const char* s, struct sockaddr* sadr,
  ...
  if (!retval) {
    ...
    if (search) {
      ...
      Com_PrintError("Sys_StringToSockaddr: Error resolving %s: No address of "
                     "required type found.\n",
                     s);
  } else
...
const char* NET_AdrToStringShort(netadr_t* a) {
  ...
  else if (a->type == NA_IP || a->type == NA_IP6 || a->type == NA_TCP ||
           a->type == NA_TCP6) {
    struct sockaddr_storage sadr;
...
const char* NET_AdrMaskToString(netadr_t* a) {
  ...
  if (a->type == NA_LOOPBACK) {
    ...
    Com_sprintf(s, sizeof(s), "%i.%i.%i.x:%hu", a->ip[0], a->ip[1], a->ip[2],
                BigShort(a->port));
  } else if (a->type == NA_IP6 || a->type == NA_TCP6) {
    Com_sprintf(s, sizeof(s), "[%02x%02x:%02x%02x:%02x%02x:x:x:x:x:x]:%hu",
                a->ip6[0], a->ip6[1], a->ip6[2], a->ip6[3], a->ip6[4],
                a->ip6[5], BigShort(a->port));
  }
  return s;
}

const char*
NET_AdrToConnectionStringCommon(netadr_t* a,
                                const char*(NET_AdrToString_a)(netadr_t*),
                                const char*(NET_AdrToString_b)(netadr_t*)) {
  if (a == NULL)
    return "(null)";
...
const char* NET_AdrToConnectionStringShort(netadr_t* a) {
  return NET_AdrToConnectionStringCommon(a, NET_AdrToStringShort,
                                         NET_AdrToStringShort);
}

const char* NET_AdrToConnectionStringMask(netadr_t* a) {
  return NET_AdrToConnectionStringCommon(a, NET_AdrMaskToString,
                                         NET_AdrToString);
}
...
qboolean NET_CompareAdr(netadr_t* a, netadr_t* b) {
  ...
  if (a->type == NA_IP || a->type == NA_IP6 || a->type == NA_TCP ||
      a->type == NA_TCP6) {
    if (a->port == b->port)
      return qtrue;
  } else
...
__optimize3 __regparm3 int NET_GetPacket(netadr_t* net_from, void* net_message,
                                         int maxsize, int socket) {
  int ret;
  ...
  if (socket != INVALID_SOCKET) {
    fromlen = sizeof(from);
    ret = recvfrom(socket, net_message, maxsize, 0, (struct sockaddr*)&from,
                   &fromlen);
    ...
    if (ret == SOCKET_ERROR) {
      ...
      if (err != EAGAIN && err != ECONNRESET) {
        Com_PrintWarningNoRedirect("NET_GetPacket on (%s - %d): %s\n",
                                   NET_AdrToString(NET_SockToAdr(socket)),
                                   socket, NET_ErrorString());
      }
    } else {

      /*			memset( ((struct sockaddr_in *)&from)->sin_zero,
         0, 8 );

                              if ( usingSocks && memcmp( &from, &socksRelayAddr,
         fromlen ) == 0 ) { if ( ret < 10 || net_message->data[0] != 0 ||
         net_message->data[1] != 0 || net_message->data[2] != 0 ||
         net_message->data[3] != 1 ) { return qfalse;
                                      }
                                      net_from->type = NA_IP;
                                      ...
                                      net_from->port = *(short
         *)&net_message->data[8]; net_message->readcount = 10;
                              }
                              else {*/
...
qboolean Sys_SendPacket(int length, const void* data, netadr_t* to) {
  ...
  if ((to->type == NA_IP && to->sock == INVALID_SOCKET) ||
      (to->type == NA_IP6 && to->sock == INVALID_SOCKET) ||
      (to->sock == INVALID_SOCKET && to->type == NA_MULTICAST6))
    return qfalse;
  ...
  /*
          if( usingSocks && to->type == NA_IP ) {
                  ...
                  *(int *)&socksBuf[4] = ((struct sockaddr_in
     *)&addr)->sin_addr.s_addr;
                  *(short *)&socksBuf[8] = ((struct sockaddr_in
     *)&addr)->sin_port; memcpy( &socksBuf[10], data, length ); ret = sendto(
     to->socket, socksBuf, length+10, 0, &socksRelayAddr, sizeof(socksRelayAddr)
     );
  ...
  if (to->sock != 0) {
    if (to->type == NA_IP || to->type == NA_BROADCAST)
      ret = sendto(to->sock, data, length, 0, (struct sockaddr*)&addr,
    ...
    if (ret == SOCKET_ERROR) {
      err2 = socketError;
      if (err2 != EAGAIN) {
        Q_strncpyz(tmpstr, NET_AdrToString(NET_SockToAdr(to->sock)),
                   sizeof(tmpstr));
        Com_PrintError(
            "Sys_SendPacket: socket %d Conn: %s ==> %s failed with: %s\n",
    ...
    for (i = 0; i < numIP; i++) {
      ...
      if (to->type == NA_IP) {
        ...
        ret = sendto(ip_socket[i].sock, data, length, 0,
      ...
      if (ret == SOCKET_ERROR) {
        ...
        Q_strncpyz(tmpstr, NET_AdrToString(&ip_socket[i]), sizeof(tmpstr));
        Com_Printf(
            "^2Sys_SendPacket: socket %d Conn: %s ==> %s successfully sent\n",
            ip_socket[i].sock, tmpstr, NET_AdrToString(to));
      }
#endif
    }
  ...
  for (index = 0; index < numIP; index++) {
    if (localIP[index].type == adr->type) {
      if (adr->type == NA_IP || adr->type == NA_TCP) {
        ...
        comparemask =
            (byte*)&((struct sockaddr_in6*)&localIP[index].netmask)->sin6_addr;
        compareadr = adr->ip6;
...
int NET_IPSocket(char* net_interface, int port, int* err, qboolean tcp) {
  ...
  *err = 0;
  ...
  if (!tcp) {
    ...
    if (setsockopt(newsocket, IPPROTO_IP, IP_TOS, (char*)&tos, sizeof(tos)) ==
        SOCKET_ERROR) {
    ...
    /*
            // Short WAIT_STATE time
                    ...
                    if( setsockopt( newsocket, SOL_SOCKET, SO_LINGER, (char *)
       &so_linger, sizeof(so_linger) ) == SOCKET_ERROR ) { Com_PrintWarning(
       "NET_IPSocket: setsockopt SO_LINGER: %s\n", NET_ErrorString() );
                    }
    */
    reuse = 1;

    if (setsockopt(newsocket, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse,
  ...
  if (!tcp) {
    ...
    if (ret == SOCKET_ERROR && err2 != EAGAIN) {
      Com_PrintError(
          "NET_IPSocket reading from open socket %d failed with: %s\n",
...
====================
*/
int NET_IP6Socket(char* net_interface, int port, struct sockaddr_in6* bindto,
                  int* err, qboolean tcp) {
  SOCKET newsocket;
  ...
  *err = 0;
  ...
  if (tcp) {
    /*
            // Short WAIT_STATE time
                    ...
                    if( setsockopt( newsocket, SOL_SOCKET, SO_LINGER, (char *)
       &so_linger, sizeof(so_linger) ) == SOCKET_ERROR ) { Com_PrintWarning(
       "NET_IP6Socket: setsockopt SO_LINGER: %s\n", NET_ErrorString() );
                    }
    */
  ...
  if (!tcp) {
    ...
    if (ret == SOCKET_ERROR && err2 != EAGAIN) {
      Com_PrintError(
          "NET_IP6Socket reading from open socket %d failed with: %s\n",
          newsocket, NET_ErrorString());
    } else {
...
sizeof(addr), AF_INET6))
        {
                Com_Printf("WARNING: NET_JoinMulticast6: Incorrect multicast
address given, " "please set cvar %s to a sane value.\n", net_mcast6addr->name);

                Cvar_SetInt(net_enabled, net_enabled->integer |
NET_DISABLEMCAST);
...
#else
                curgroup.ipv6mr_interface =
if_nametoindex(net_mcast6iface->string); #endif
        }
        else
...
|| (net_enabled->integer & NET_DISABLEMCAST)) return;

        if(IN6_IS_ADDR_MULTICAST(&boundto.sin6_addr) ||
IN6_IS_ADDR_UNSPECIFIED(&boundto.sin6_addr))
        {
                // The way the socket was bound does not prohibit receiving
multi-cast packets. So we don't need to open a new one. multicast6_socket =
ip6_socket;
        }
        else
        {
                if((multicast6_socket = NET_IP6Socket(net_mcast6addr->string,
ntohs(boundto.sin6_port), NULL, &err, qfalse)) == INVALID_SOCKET)
                {
                        // If the OS does not support binding to multicast
addresses, like WinXP, at least try with the normal file descriptor.
        ...
        {
                if (setsockopt(multicast6_socket, IPPROTO_IPV6,
IPV6_MULTICAST_IF, (char *) &curgroup.ipv6mr_interface,
sizeof(curgroup.ipv6mr_interface)) < 0)
                {
                        Com_Printf("NET_JoinMulticast6: Couldn't set scope on
multicast socket: %s\n", NET_ErrorString());
        ...
        if (setsockopt(multicast6_socket, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char
*) &curgroup, sizeof(curgroup)))
        {
                Com_Printf("NET_JoinMulticast6: Couldn't join multicast group:
%s\n", NET_ErrorString());
...
void NET_LeaveMulticast6()
{
        if(multicast6_socket != INVALID_SOCKET)
        {
                if(multicast6_socket != ip6_socket)
                        ...
                        setsockopt(multicast6_socket, IPPROTO_IPV6,
IPV6_LEAVE_GROUP, (char *) &curgroup, sizeof(curgroup));
...
INVALID_SOCKET ) { Com_Printf( "WARNING: NET_OpenSocks: socket: %s\n",
NET_ErrorString() ); return;
        }
...
NET_ErrorString() ); return;
        ...
        if ( h->h_addrtype != AF_INET ) {
                Com_Printf( "WARNING: NET_OpenSocks: gethostbyname: address type
was not AF_INET\n" ); return;
        }
        address.sin_family = AF_INET;
...
) ) == SOCKET_ERROR ) { Com_Printf( "NET_OpenSocks: connect: %s\n",
NET_ErrorString() ); return;
        }
...
username/password
        ...
        if ( buf[1] == 2 ) {
                ...
                buf[0] = 1;		// username/password authentication
version buf[1] = ulen; if ( ulen ) { memcpy( &buf[2], net_socksUsername->string,
ulen );
                }
                buf[2 + ulen] = plen;
                if ( plen ) {
...
SOCKET_ERROR ) { Com_Printf( "NET_OpenSocks: send: %s\n", NET_ErrorString() );
                        return;
                }
...
buf[3] ); return;
        ...
        ((struct sockaddr_in *)&socksRelayAddr)->sin_addr.s_addr = *(int
*)&buf[4];
...
static void NET_GetLocalAddress(void) {
  ...
  if (!getaddrinfo(hostname, NULL, &hint, &res)) {
     ...
     * interfaces, we'll just assume a netmask with all bits set. */
    ...
    /* Windows doesn't seem to add the loopback interface to its list of
     * available interfaces. We have to assume they are there. */
    if (has_ip4 && !has_loopback4 &&
        Sys_StringToSockaddr("localhost", (struct sockaddr*)&localhostadr,
                             sizeof(localhostadr), AF_INET)) {
      NET_AddLocalAddress("localhost", (struct sockaddr*)&localhostadr,
                          (struct sockaddr*)&mask4);
    }

    if (has_ip6 && !has_loopback6 &&
        Sys_StringToSockaddr("localhost", (struct sockaddr*)&localhostadr,
                             sizeof(localhostadr), AF_INET6)) {
      NET_AddLocalAddress("localhost", (struct sockaddr*)&localhostadr,
  ...
  if (net_enabled->integer & NET_ENABLEV6) {
    ...
    for (i = 0; i < limit6; i++) {
      ...
      if (Q_stricmp(net_ip6->string, "::")) {
        ...
        Com_sprintf(addrbuf2, sizeof(addrbuf2), "[%s]:%d", net_ip6->string,
                    port6 + i);
        NET_StringToAdr(addrbuf2, &ip_socket[0], NA_IP6);

        ip_socket[0].sock =
            NET_IP6Socket(net_ip6->string, port6 + i, &boundto, &err, qfalse);
        ...
        for (j = 0; j < numIP; j++) {
          ...
          Sys_SockaddrToString(addrbuf, sizeof(addrbuf),
                               (struct sockaddr*)&localIP[j].addr);
          ...
          Com_sprintf(addrbuf2, sizeof(addrbuf2), "[%s]:%d", addrbuf,
                      port6 + i);
          NET_StringToAdr(addrbuf2, &ip_socket[socketindex6], NA_IP6);

          ip_socket[socketindex6].sock =
              NET_IP6Socket(addrbuf, port6 + i, &boundto, &err, qfalse);
      ...
      if (validsock6) // && tcp6_socket != INVALID_SOCKET)
      {
        ...
        tcp6_socket =
            NET_IP6Socket(net_ip6->string, port6 + i, &boundto, &err, qtrue);
  ...
  if (net_enabled->integer & NET_ENABLEV4) {
    ...
    for (i = 0; i < limit; i++) {
      ...
      if (Q_stricmp(net_ip->string, "0.0.0.0")) {
        ...
        Com_sprintf(addrbuf2, sizeof(addrbuf2), "%s:%d", net_ip->string,
                    port + i);
        NET_StringToAdr(addrbuf2, &ip_socket[socketindex], NA_IP);

        ip_socket[socketindex].sock =
            NET_IPSocket(net_ip->string, port + i, &err, qfalse);
        ...
        for (j = 0; j < numIP; j++) {
          ...
          ip_socket[socketindex].sock =
              NET_IPSocket(addrbuf, port + i, &err, qfalse);
...
static qboolean NET_GetCvars(void) {
  ...
  net_enabled =
      Cvar_RegisterInt("net_enabled", 3, 0, 8, CVAR_LATCH | CVAR_ARCHIVE,
                       "Enables / Disables Network");

  modified = net_enabled->modified;
  net_enabled->modified = qfalse;

  net_ip = Cvar_RegisterString("net_ip", "0.0.0.0", CVAR_LATCH,
                               "Default Network Address");
  modified += net_ip->modified;
  net_ip->modified = qfalse;

  net_ip6 = Cvar_RegisterString("net_ip6", "::", CVAR_LATCH,
                                "Default IPv6 Network Address");
  modified += net_ip6->modified;
  net_ip6->modified = qfalse;

  net_port = Cvar_RegisterInt("net_port", 0, 0, 65535, CVAR_LATCH,
                              "Network Port Server will listen on");
  modified += net_port->modified;
  net_port->modified = qfalse;

  net_port6 = Cvar_RegisterInt("net_port6", 0, 0, 65535, CVAR_LATCH,
                               "IPv6 Network Port Server will listen on");
  modified += net_port6->modified;
  net_port6->modified = qfalse;
  /*
          // Some cvars for configuring multicast options which facilitates
  scanning for servers on local subnets. net_mcast6addr = Cvar_RegisterString(
  "net_mcast6addr", NET_MULTICAST_IP6, CVAR_LATCH | CVAR_ARCHIVE,  "IPv6 Network
  multicast address"); modified += net_mcast6addr->modified;
          net_mcast6addr->modified = qfalse;

  #ifdef _WIN32
          net_mcast6iface = Cvar_RegisterInt( "net_mcast6iface", 0, 0, 65535,
  CVAR_LATCH | CVAR_ARCHIVE ,  "IPv6 Network multicast interface"); #else
          net_mcast6iface = Cvar_RegisterString( "net_mcast6iface", "",
  CVAR_LATCH | CVAR_ARCHIVE ,  "IPv6 Network multicast interface"); #endif

          modified += net_mcast6iface->modified;
          net_mcast6iface->modified = qfalse;

          net_socksEnabled = Cvar_RegisterInt( "net_socksEnabled", 0, 0, 1,
  CVAR_LATCH | CVAR_ARCHIVE, "Net enable socks proxy support"); modified +=
  net_socksEnabled->modified; net_socksEnabled->modified = qfalse;

          net_socksServer = Cvar_RegisterString( "net_socksServer", "",
  CVAR_LATCH | CVAR_ARCHIVE, "Net socks proxyserver address"); modified +=
  net_socksServer->modified; net_socksServer->modified = qfalse;

          net_socksPort = Cvar_RegisterInt( "net_socksPort", 1080, 0, 65535,
  CVAR_LATCH | CVAR_ARCHIVE, "Net socks proxyserver port"); modified +=
  net_socksPort->modified; net_socksPort->modified = qfalse;

          net_socksUsername = Cvar_RegisterString( "net_socksUsername", "",
  CVAR_LATCH | CVAR_ARCHIVE , "Net socks proxyserver username"); modified +=
  net_socksUsername->modified; net_socksUsername->modified = qfalse;

          net_socksPassword = Cvar_RegisterString( "net_socksPassword", "",
  CVAR_LATCH | CVAR_ARCHIVE , "Net socks proxyserver password"); modified +=
  net_socksPassword->modified; net_socksPassword->modified = qfalse;
  */
  net_dropsim = Cvar_RegisterInt("net_dropsim", 0, 0, 100, CVAR_TEMP,
                                 "Net enable packetloss simulation");
...
void NET_Config(qboolean enableNetworking) {
  ...
  if (stop) {
    ...
    for (i = 0, con = tcpServer.connections; i < MAX_TCPCONNECTIONS;
         i++, con++) {
...
void NET_RegisterDefaultCommunicationSocket(netadr_t* adr) {
  ...
  if (socketadr != NULL && socketadr->type == NA_IP) {
    ...
    Com_Printf(
        "NET_Notice: Default address selected for outgoing networkdata is %s\n",
        NET_AdrToString(&ip_defaultSock));
  } else {
    Com_PrintWarning("NET_RegisterDefaultCommunicationSocket, got packet from "
                     "bad socket %d at %s\n",
...
void NET_TcpCloseSocket(int socket) {
  ...
  for (i = 0, conn = tcpServer.connections; i < MAX_TCPCONNECTIONS;
       i++, conn++) {
    if (conn->remote.sock == socket) {
      ...
      if (conn->state >= TCP_AUTHSUCCESSFULL) {
        ...
        NET_TCPConnectionClosed(&conn->remote, conn->connectionId,
                                conn->serviceId);
      }
...
int NET_TcpSendData(int sock, const void* data, int length) {
  ...
  state = send(sock, data, numbytes,
               NET_NOSIGNAL); // FIX: flag NOSIGNAL prevents SIGPIPE in case of
                              // connection problems
  ...
  if (state == SOCKET_ERROR) {
    ...
    if (err == EAGAIN) {
      ...
      Com_PrintWarningNoRedirect(
          "NET_SendTCPPacket: Couldn't send data to remote host: %s\n",
          NET_ErrorString());
    }
...
int NET_TcpServerGetPacket(tcpConnections_t* conn, void* netmsg, int maxsize,
                           qboolean warn) {
  ...
  if (ret == SOCKET_ERROR) {
    ...
    if (ret == ECONNRESET) {
      ...
      Com_PrintWarningNoRedirect(
          "NET_GetTcpPacket recv() syscall failed: %s\n",
          NET_ErrorString()); // BUGFIX: this causes SIGSEGV in case of an error
                              // during console stream
...
void NET_TcpServerPacketEventLoop() {
  ...
  if (activefd < 0) {
    Com_PrintWarningNoRedirect(
        "NET_TcpServerPacketEventLoop: select() syscall failed: %s\n",
        NET_ErrorString());
    return;
  }
    ...
    if (FD_ISSET(conn->remote.sock, &fdr)) {
      ...
      case TCP_AUTHAGAIN:
        ...
        if (conn->lastMsgTime == 0 || conn->remote.sock < 1) {
          ...
          // Close connection, we don't want to process huge messages as
          // auth-packet or want to quit if the login was bad
        } else if ((conn->state = NET_TCPAuthPacketEvent(
                        &conn->remote, bufData, cursize, &conn->connectionId,
      ...
      case TCP_AUTHSUCCESSFULL:
        ...
        if (cursize > sizeof(bufData)) {
          Com_PrintWarningNoRedirect("NET_TcpServerPacketEventLoop: Oversize "
                                     "packet from %s. Must not happen!\n",
                                     NET_AdrToString(&conn->remote));
          cursize = sizeof(bufData);
        }
        NET_TCPPacketEvent(&conn->remote, bufData, cursize, conn->connectionId,
                           conn->serviceId);
        break;
      }

    } else if (conn->lastMsgTime && conn->state < TCP_AUTHSUCCESSFULL &&
               conn->lastMsgTime + MAX_TCPAUTHWAITTIME < NET_TimeGetTime()) {
      NET_TcpCloseSocket(conn->remote.sock);
    }
  }
}
...
void NET_TcpServerOpenConnection(netadr_t* from) {
  ...
  for (i = 0, conn = tcpServer.connections; i < MAX_TCPCONNECTIONS;
       i++, conn++) {
    if ((NET_CompareBaseAdr(from, &conn->remote) &&
         conn->state < TCP_AUTHSUCCESSFULL) ||
        conn->remote.sock < 1) { // Net request from same address - Close the
                                 // old not confirmed connection
      break;
    }
    if (conn->state < TCP_AUTHSUCCESSFULL) {
  ...
  if (i == MAX_TCPCONNECTIONS) {
    if (tcpServer.activeConnectionCount > MAX_TCPCONNECTIONS / 3 &&
        oldestTimeAccepted + MAX_TCPCONNECTEDTIMEOUT < NET_TimeGetTime()) {
      conn = &tcpServer.connections[oldestAccepted];
      tcpServer.activeConnectionCount--; // As this connection is going to be
                                         // closed decrease the counter
      NET_TCPConnectionClosed(&conn->remote, conn->connectionId,
                              conn->serviceId);
      ...
      closesocket(from->sock); // We have already too many open connections. Not
                               // possible to open more. Possible attack

      if (tcpServer.lastAttackWarnTime + MIN_TCPAUTHWAITTIME <
          NET_TimeGetTime()) {
        tcpServer.lastAttackWarnTime = NET_TimeGetTime();
        Com_PrintWarning("Possible Denial of Service Attack, Dropping "
                         "connectrequest from: %s\n",
  ...
  Com_DPrintf(
      "Opening a new TCP server connection. Sock: %d Index: %d From:%s\n",
...
__optimize3 __regparm3 qboolean NET_TcpServerConnectRequest(netadr_t* net_from,
                                                            fd_set* fdr) {
...
__optimize3 __regparm1 qboolean NET_TcpServerConnectEvent(fd_set* fdr) {
  ...
  // Give the system a possibility to abort processing network packets so it
  // won't block execution of frames if the network getting flooded
    ...
    if (ret == SOCKET_ERROR) {
      ...
      *buflen = readcount;
      ...
      if (ret > *buflen - readcount) {
        ...
        Com_PrintWarning("Oversize packet on socket %d Remaining bytes are: "
                         "%d, received bytes are %d\n",
...
int NET_TcpClientConnect(const char* remoteAdr) {
  ...
  if ((newsocket = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) ==
      INVALID_SOCKET) {
  ...
  if (connect(newsocket, (void*)&address, sizeof(address)) == SOCKET_ERROR) {
    ...
    if (err == EINPROGRESS
      ...
      if (retval < 0) {
        ...
        if (getsockopt(newsocket, SOL_SOCKET, SO_ERROR, (char*)&err, &so_len) ==
            0) {
          return newsocket;
        }
...
__optimize3 __regparm1 qboolean NET_Sleep(unsigned int usec) {
  ...
  int i;
  qboolean netabort =
      qfalse; // This will be true if we had to process more than 666 packets on
              // one single interface Usually this marks an ongoing floodattack
              // onto this CoD4 server
  ...
  if (retval < 0) {
    ...
    if ((tcp_socket != INVALID_SOCKET && FD_ISSET(tcp_socket, &fdr)) ||
        (tcp6_socket != INVALID_SOCKET && FD_ISSET(tcp6_socket, &fdr))) {
...
====================
*/
void NET_Restart_f(void) { NET_Config(networkingEnabled); }
...
int NET_GetHostPort() { return net_port->integer; }
