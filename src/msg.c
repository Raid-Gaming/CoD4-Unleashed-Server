...
void MSG_InitReadOnlySplit(msg_t* buf, byte* data, int length, byte* arg4,
                           int arg5) {
...
int MSG_SkipToString( msg_t *msg, const char* string ) {
        ...
        do{
                c = MSG_ReadByte( msg );      // use ReadByte so -1 is out of
bounds if ( c == -1 )
                {
                        return qfalse;
                }
                if(c == string[0] && !Q_strncmp(msg->data + msg->readcount,
string, msg->cursize - msg->readcount))
                {
                        return qtrue;
                }
        }
...
void MSG_ClearLastReferencedEntity(msg_t* msg) { msg->lastRefEntity = -1; }
...
netFieldList_t netFieldList[] = {{NETFE(entityStateFields)},
...
void MSG_WriteDeltaEntity(snapshotInfo_t* snap, msg_t* msg, int time,
                          entityState_t* from, entityState_t* to, int arg_6) {
    ...
    if (g_entities[snap->clnum].client->sess.sessionTeam == TEAM_FREE) {
      if (!SV_FFAPlayerCanBlock()) {
        to->solid &= ~PLAYER_SOLIDMASK;
               ...
               OnSameTeam(&g_entities[to->number], &g_entities[snap->clnum]))
      to->solid &= ~PLAYER_SOLIDMASK;
  }
  ...
  for (i = 0, lc = 0, field = fieldtype->field; i < fieldtype->numFields;
       i++, field++) {
  ...
  if (lc > 0) {
    for (i = 0, field = fieldtype->field; i != lc; i++, field++) {
      MSG_WriteDeltaField(snap, msg, time, (unsigned const char*)from,
                          (unsigned const char*)to, field, i, 0);
    }
  }
  //	MSG_GetUsedBitCount(msg);
}
...
void MSG_WriteReliableCommandToBuffer(const char* source, char* destination,
                                      int length) {
  int i;
  int requiredlen;
  ...
  if (i == length - 1) {
    ...
    if (requiredlen > length) {
      Com_PrintWarning("WARNING: Reliable command is too long (%i/%i) and will "
                       "be truncated: '%s'\n",
