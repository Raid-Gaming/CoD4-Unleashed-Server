...
typedef enum { SAY_CHAT, SAY_CONSOLE, SAY_SCREEN } consaytype_t;
...
static void SV_GetPlayerByHandleInternal(const char* s, clanduid_t* cl) {
  ...
  if ((s[0] == '@' || s[0] == 'u') && isNumeric(&s[1], 0)) {
    ...
    if (!cl->cl) {
      ...
      for (i = 0, cl->cl = svs.clients; i < sv_maxclients->integer;
           i++, cl->cl++) {
        if (!cl->cl->state) {
          continue;
        }
      ...
      if (!lastfound) { // No exact playermatch found - Now search for partial
                        // name matches
        for (i = 0, cl->cl = svs.clients; i < sv_maxclients->integer;
             i++, cl->cl++) {
          if (!cl->cl->state) {
  ...
  if (!cl->cl &&
      cl->uid > 0) { // See whether this player is currently onto server
    for (i = 0, cl->cl = svs.clients; i < sv_maxclients->integer;
         i++, cl->cl++) {
...
static void SV_ConSayChat_f(void) { SV_ConSay(NULL, SAY_CHAT); }
...
static void SV_ConSayConsole_f(void) { SV_ConSay(NULL, SAY_CONSOLE); }
...
static void SV_ConSayScreen_f(void) { SV_ConSay(NULL, SAY_SCREEN); }
...
static void SV_ConTell(consaytype_t contype) {
  ...
  if (Cmd_Argc() < 3) {
    Com_Printf("1. Usage: tellcommand clientnumber text... \n2. Usage: "
               "tellcommand \"client by name\" text...\n");
    return;
  }
...
==================
*/
static void SV_ConTellScreen_f(void) { SV_ConTell(SAY_SCREEN); }
...
==================
*/
static void SV_ConTellConsole_f(void) { SV_ConTell(SAY_CONSOLE); }
...
==================
*/
static void SV_ConTellChat_f(void) { SV_ConTell(SAY_CHAT); }
...
static void SV_DumpUser_f(void) {
  ...
  /*
          ...
          if(cl.uid || guid){
                  ...
                  *infostring = 0;
                  if(cl.uid)
                          Info_SetValueForKey( infostring, "UID",
     va("%i",cl.uid));
                  ...
                  if(svse.cmdInvoker.currentCmdInvoker)
                          CL_AddReliableCommand(&svse.authserver,
     va("getUserinfo Ticket %i \"%s\"",
     svs.clients[SV_RemoteCmdGetInvokerClnum()].challenge+QT_CMDQUERY,
     infostring)); else CL_AddReliableCommand(&svse.authserver, va("getUserinfo
     Ticket %i \"%s\"", -1, infostring));
...
static void Cmd_BanPlayer_f() {
  ...
  if (!Q_stricmp(Cmd_Argv(0), "banUser") ||
      !Q_stricmp(Cmd_Argv(0), "banClient")) {
    if (Cmd_Argc() < 2) {
      Com_Printf("Usage: banUser <user>\n");
      Com_Printf("Where user is one of the following: online-playername | "
                 "online-playerslot | guid | uid\n");
      Com_Printf(
          "online-playername can be a fraction of the playername. uid is a "
          "number > 0 and gets written with a leading \"@\" character\n");
    ...
    if (Cmd_Argc() < 3) {
      Com_Printf(
          "Usage: permban <user> <Reason for this ban (max 126 chars)>\n");
      Com_Printf("Where user is one of the following: online-playername | "
                 "online-playerslot | guid | uid\n");
      Com_Printf(
          "online-playername can be a fraction of the playername. uid is a "
...
gothandle:
  ...
  if (cl.cl && strlen(cl.cl->pbguid) == 32) {
    ...
    SV_PlayerAddBanByip(&cl.cl->netchan.remoteAddress, "INVALID USER", 0,
                        "INVALID", 0, -1);
    return;
  }
  ...
  if (strlen(banreason) > 126) {
    Com_Printf("Error: You have exceeded the maximum allowed length of 126 "
               "characters for the reason\n");
    return;
  }
  ...
  if (cl.cl) {
    ...
    // Banning
    SV_AddBan(cl.uid, Cmd_GetInvokerUID(), guid, cl.cl->name, (time_t)-1,
              banreason);
    // Messages and kick
    if (cl.uid > 0) {
      Com_Printf("Banrecord added for player: %s uid: %i\n", cl.cl->name,
                 cl.uid);
      SV_PrintAdministrativeLog(
          "banned player: %s uid: %i IP: %s with the following reason: %s",
          cl.cl->name, cl.uid, NET_AdrToString(&cl.cl->netchan.remoteAddress),
          banreason);
      Com_sprintf(dropmsg, sizeof(dropmsg),
                  "You have been permanently banned on this server\nYour ban "
                  "will %s expire\nYour UID is: %i    Banning admin UID is: "
                  "%i\nReason for this ban:\n%s",
      ...
      Com_Printf("Banrecord added for player: %s guid: %s\n", cl.cl->name,
                 cl.cl->pbguid);
      SV_PrintAdministrativeLog(
          "banned player: %s guid: %s IP: %s with the following reason: %s",
          cl.cl->name, cl.cl->pbguid,
          NET_AdrToString(&cl.cl->netchan.remoteAddress), banreason);
      Com_sprintf(
          dropmsg, sizeof(dropmsg),
          "You have been permanently banned on this server\nYour GUID is: %s   "
          " Banning admin UID is: %i\nReason for this ban:\n%s",
          cl.cl->pbguid, Cmd_GetInvokerUID(), banreason);

      if (cl.cl->authentication < 1)
        SV_PlayerAddBanByip(&cl.cl->netchan.remoteAddress, banreason, 0,
                            cl.cl->pbguid, 0, -1);
    }
    ...
    if (cl.uid > 0) {
      ...
      SV_PrintAdministrativeLog(
          "banned player uid: %i with the following reason: %s", cl.uid,
          banreason);
    } else {
      Com_Printf("Banrecord added for guid: %s\n", guid);
      SV_PrintAdministrativeLog(
          "banned player guid: %s with the following reason: %s", guid,
          banreason);
    }
  }
  // InsertPluginEvent
...
static void Cmd_TempBanPlayer_f() {
  ...
  if (Cmd_Argc() < 4) {
    Com_Printf("Usage: tempban <user> <time> <Reason for this ban (max. 126 "
               "chars)>\n");
    Com_Printf("Where user is one of the following: online-playername | "
               "online-playerslot | guid | @uid\n");
    Com_Printf(
        "Where time is one of the following: ^2XX^7m | ^2XX^7h | ^2XX^7d\n");
    Com_Printf("Where reason for this ban is contains a description without "
               "the letters: \" ; %% / \\ \n");
    Com_Printf(
        "online-playername can be a fraction of the playername. uid is a "
        "number > 0 and gets written with a leading \"@\" character\n");
    Com_Printf("guid is a hexadecimal string with length of 8 characters\n");
    Com_Printf("XX is an integer representing the time of ban in minutes, "
               "hours or days\n");
    return;
  }
...
gothandle:
  ...
  if (duration > 60 * 24 * 30) {
    Com_Printf("Error: Can not issue a temporary ban that lasts longer than 30 "
               "days\n");
    return;
  }
  ...
  if (strlen(banreason) > 126) {
    Com_Printf("Error: You have exceeded the maximum allowed length of 126 for "
               "the reason\n");
    return;
  }
  ...
  if (cl.cl) {
    ...
    SV_AddBan(cl.uid, Cmd_GetInvokerUID(), guid, cl.cl->name, expire,
              banreason);
    ...
    if (cl.uid > 0) {
      ...
      Com_Printf("Banrecord added for player: %s uid: %i\n", cl.cl->name,
                 cl.uid);
      SV_PrintAdministrativeLog("temporarily banned player: %s uid: %i IP: %s "
                                "until %s with the following reason: %s",
                                cl.cl->name, cl.uid,
                                NET_AdrToString(&cl.cl->netchan.remoteAddress),
                                endtime, banreason);
      Com_sprintf(dropmsg, sizeof(dropmsg),
                  "You have been temporarily banned from this server\nYour ban "
                  "will expire on: %s UTC\nYour UID is: %i    Banning admin "
                  "UID is: %i\nReason for this ban:\n%s",
      ...
      Com_Printf("Banrecord added for player: %s guid: %s\n", cl.cl->name,
                 cl.cl->pbguid);
      SV_PrintAdministrativeLog("temporarily banned player: %s guid: %s IP: %s "
                                "until %s with the following reason: %s",
                                cl.cl->name, cl.cl->pbguid,
                                NET_AdrToString(&cl.cl->netchan.remoteAddress),
                                endtime, banreason);
      Com_sprintf(dropmsg, sizeof(dropmsg),
                  "You have been temporarily banned from this server\nYour ban "
                  "will expire on: %s UTC\nYour GUID is: %s    Banning admin "
                  "UID is: %i\nReason for this ban:\n%s",
                  endtime, cl.cl->pbguid, Cmd_GetInvokerUID(), banreason);

      if (cl.cl->authentication < 1)
        SV_PlayerAddBanByip(&cl.cl->netchan.remoteAddress, banreason, 0,
                            cl.cl->pbguid, 0, expire);
    }
    SV_DropClient(cl.cl, dropmsg);
  } else {
    ...
    if (cl.uid > 0) {
      Com_Printf("Banrecord added for uid: %i\n", cl.uid);
      SV_PrintAdministrativeLog("temporarily banned player uid: %i until %s "
                                "with the following reason: %s",
                                cl.uid, endtime, banreason);

    } else {
      Com_Printf("Banrecord added for guid: %s\n", guid);
      SV_PrintAdministrativeLog("temporarily banned player guid: %s until %s "
                                "with the following reason: %s",
                                guid, endtime, banreason);
    }
  }
  // InsertPluginEvent
}
...
static void Cmd_KickPlayer_f() {
  ...
  if (cl.uid > 0) {
    Com_Printf("Player kicked: %s ^7uid: %i\nReason: %s\n", cl.cl->name, cl.uid,
               kickreason);
    SV_PrintAdministrativeLog(
        "kicked player: %s^7 uid: %i with the following reason: %s",
        cl.cl->name, cl.uid, kickreason);
  } else {
    Com_Printf("Player kicked: %s ^7guid: %s\nReason: %s\n", cl.cl->name,
               cl.cl->pbguid, kickreason);
    SV_PrintAdministrativeLog(
        "kicked player: %s^7 guid: %s with the following reason: %s",
...
static void SV_DumpBanlist_f() { SV_DumpBanlist(); }
...
static void SV_MiniStatus_f(void) {
  ...
  Com_Printf("num score ping uid/guid   name                             "
             "address                                             FPS XVer\n");
...
==================
*/
void SV_Heartbeat_f(void) { svse.nextHeartbeatTime = 0; }
  ...
  while (*cmdstring) {
    if (*cmdstring == '$') {
      if (!Q_strncmp(cmdstring, "$uid", 4)) {
        Com_sprintf(tmp, sizeof(outstr) - (tmp - outstr), "%i",
                    Cmd_GetInvokerUID());
        tmp += strlen(tmp);
        cmdstring += 4;
        ...
        if (!*Cmd_Argv(i)) {
          cmdstring += 4;
          if (*cmdstring == ':' &&
              *(cmdstring + 1) != ' ') { // Default argument in place!
            ++cmdstring; // Just advance the pointer and read in the argument as
...
static void Cmd_AddTranslatedCommand_f() {
  ...
  if (Cmd_Argc() != 3) {
    Com_Printf("Usage: addCommand <commandname> <\"string to execute\"> String "
               "can contain: $uid $clnum $pow $arg $arg:default\n");
    return;
  }
  ...
  Q_strncpyz(psvs.translatedCmd[free].cmdname, cmdname,
             sizeof(psvs.translatedCmd[free].cmdname));
  Q_strncpyz(psvs.translatedCmd[free].cmdargument, string,
             sizeof(psvs.translatedCmd[free].cmdargument));

  Cmd_AddPCommand(psvs.translatedCmd[free].cmdname,
                  Cmd_ExecuteTranslatedCommand_f, 100);
  Com_Printf("Added custom command: %s -> %s\n",
             psvs.translatedCmd[free].cmdname,
             psvs.translatedCmd[free].cmdargument);
}
...
static void SV_ClearAllMessages_f() { G_ClearAllMessages(); }
...
static void SV_MapRotate_f(void) {
  ...
  Q_strncpyz(map_rotationbuf, sv_mapRotationCurrent->string,
             sizeof(map_rotationbuf));
  Com_ParseReset();
  ...
  while (qtrue) {
    ...
    if (!Q_stricmpn(maplist, "gametype ", 9)) {
      ...
      Cvar_SetString(sv_mapRotationCurrent,
                     maplist); // Set the cvar with one map less
...
static void SV_MapRestart_f(void) { SV_MapRestart(qfalse); }

static void SV_FastRestart_f(void) { SV_MapRestart(qtrue); }
...
qboolean SV_RunDownload(const char* url, const char* filename) {
  ...
  if (curfileobj == NULL) {
    Com_Printf("Couldn't connect to download-server for downloading. Failed to "
               "download %s\n",
               filename);
    return qfalse;
  }
  ...
  if (curfileobj->code != 200) {
    Com_Printf("Downloading of file: \"%s\" has failed with the following "
               "message: %d %s\n",
  ...
  len = FS_SV_BaseWriteFile(filename,
                            curfileobj->recvmsg.data + curfileobj->headerLength,
                            curfileobj->contentLength);
  if (len != curfileobj->contentLength) {

    len = FS_SV_HomeWriteFile(
        filename, curfileobj->recvmsg.data + curfileobj->headerLength,
        curfileobj->contentLength);
    if (len != curfileobj->contentLength) {
      Com_PrintError(
          "Opening of \"%s\" for writing has failed! Download aborted.\n",
          filename);
      FileDownloadFreeRequest(curfileobj);
      return qfalse;
    }
...
void SV_DemoCompletedExec(const char* mapname) {
  ...
  Com_sprintf(cmdline, sizeof(cmdline), "\"%s/%s\" \"%s/usermaps/%s\"",
              fs_homepath->string, sv_mapDownloadCompletedCmd->string,
              fs_homepath->string, mapname);
...
void SV_DownloadMapThread(char* inurl) {
  ...
  if (downloadActive) {
    Com_Printf(
        "There is already a map download running. Won't download this.\n");
  ...
  Com_sprintf(filename, sizeof(filename), "usermaps/%s/%s%s", mapname, mapname,
              ".ff");
...
void SV_DownloadMap_f() {
  ...
  if (Cmd_Argc() != 2) {
    Com_Printf("Usage: downloadmap <\"url\">\nFor example: downloadmap "
               "\"http://somehost/cod4/usermaps/mapname\"\n");
    return;
  }
