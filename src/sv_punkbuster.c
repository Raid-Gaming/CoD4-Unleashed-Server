...
typedef struct pbsv_object_s {
  ...
  int(__cdecl* sb)(struct pbsv_object_s* this, int event, int clnum,
                   int maxstringlen, char* string, int);
  void(__cdecl* sa)(struct pbsv_object_s* this, int);
  int(__cdecl* PbSvSendToAddrPort)(char* netdest, unsigned short port,
                                   int msgsize, char* message);
  int(__cdecl* ConnectStringTrap)(struct pbsv_object_s* this,
                                  const char* netadr,
                                  const char* connectString);
  const char*(__cdecl* AuthClient)(struct pbsv_object_s* this, const char*, int,
                                   const char*);
  void(__cdecl* CaptureConsole)(struct pbsv_object_s* this, const char* msg,
                                int buflen);
  int dword_D8B2ECC;
} pbsv_object_t;
...
__cdecl int PbSvSendToAddrPort(char* netdest, unsigned short port, int msgsize,
                               char* message) {
...
__cdecl char* PbSvGameQuery(int para_01, char* string) {
  ...
  case 0x66:
    ...
    if (var_01 < 0 || var_01 > maxclients)
      return "PB_Error: Query Failed";
    cl = &svs.clients[var_01];

    if (cl->state < CS_ACTIVE || cl->noPb == qtrue)
      return "PB_Error: Query Failed";
  ...
  case 0x72:
    ...
    *string = 0;
    ...
    if (cl->state < CS_ACTIVE || cl->noPb)
      return "PB_Error: Query Failed";
    ;

    Com_sprintf(string, 255, "ping=%d score=%d", cl->ping,
                gclient->pers.scoreboard.score);
    return NULL;
...
void PB_SystemDummy(const char* cmdline) {
  Com_PrintWarning(
      "system( %s ) of PunkBuster is blocked while running in securemode\n",
      cmdline);
}
...
int PbSVLoadModule() {
  ...
  if (FS_FileExistsOSPath(PbSv6makefn(modname, "pbsvnew" DLL_EXT)) == qtrue) {
    ...
    rename(PbSv6makefn(modname, "pbsv" DLL_EXT),
           PbSv6makefn(modname_dest, "pbsvold" DLL_EXT));
    Sys_Chmod(PbSv6makefn(modname, "pbsv" DLL_EXT), 0777);
    FS_RemoveOSPath(PbSv6makefn(modname, "pbsv" DLL_EXT));
    rename(PbSv6makefn(modname, "pbsvnew" DLL_EXT),
           PbSv6makefn(modname_dest, "pbsv" DLL_EXT));
  }
  ...
  if (com_securemode) {
    ...
    if (Q_stricmp(hash, PBAG_HASH)) {
      Com_Printf("Tiger = %s\n", hash);
      Com_PrintError("%s checksum missmatch! PunkBuster will not startup in "
                     "securemode when the checksum is invalid.\n",
                     dlname2);
      return 1;
    }
  ...
  if (com_securemode) {
    ...
    if (Sys_MemoryProtectWrite((void*)(imagebase + 0x4c6e0), 0xdfdf7) ==
        qfalse) {
      return 1;
    }
    ...
    if (Sys_MemoryProtectExec((void*)(imagebase + 0x4c6e0), 0xdfdf7) ==
        qfalse) {
      return 1;
    }
  }
...
void PbSv10AddPbEvent(pbsv_object_t* this, int eventType, int clnum,
                      int maxstringsize, char* string, int a6) {
...
void PbServerForceProcess() { PbServerProcessEvents(-1); }
...
const char* PbAuthClient(const char* netadr, qboolean clhaspunkbuster,
                         const char* guid) {
  const char* errormsg;
...
void DisablePbSv() { PbSv10AddPbEvent(&pbsv, 118, -1, 0, 0, 0); }
