...
void SV_DumpCommands( client_t *client, byte *cmd, int size, qboolean
cl_to_servercmd) {
...
%i, T: %i, NA: %i, Len %i, Msg: ", client - svs.clients, svs.time
,client->reliableSequence - client->reliableAcknowledge, size); }else{
                Com_sprintf((char*)buffer, sizeof(buffer), "\nDir: Sv -> Cl, Cl:
%i, T: %i, NA: %i, Len %i, Msg: ", client - svs.clients, svs.time
,client->reliableSequence - client->reliableAcknowledge, size);
        }
...
on) if ( !commanddump && FS_Initialized()) { struct tm *newtime; time_t aclock;

                        time( &aclock );
                        newtime = localtime( &aclock );

                        commanddump = FS_FOpenFileWrite(
"reliableupdateCmds.log" );
...
void SV_Netchan_Decode(client_t* client, byte* data, int remaining) {
  ...
  //	string = (byte *)extcl->reliableCommands[ client->reliableAcknowledge &
...
================
*/
qboolean SV_Netchan_Transmit(client_t* client, byte* data,
                             int cursize) { // int length, const byte *data ) {
  qboolean nt_ret;
  int i;
...
================
*/
__cdecl qboolean SV_Netchan_TransmitNextFragment(
    client_t* client) { // int length, const byte *data ) {
  int i;
