...
qboolean Sec_SignHash(const char* in, size_t inSize, rsa_key* key, char* out,
  ...
  int res = rsa_sign_hash_ex((const unsigned char*)in, is, (unsigned char*)out,
                             &os, LTC_LTC_PKCS_1_V1_5, NULL, 0, 0, 16, key);
  *outSize = os;
  return res;
}
qboolean Sec_VerifyHash(const char* sig, size_t sigSize, const rsa_key* key,
  ...
  int n = 0;
  int res = rsa_verify_hash_ex((const unsigned char*)sig, sigSize,
                               (const unsigned char*)hash, hashSize,
                               LTC_LTC_PKCS_1_V1_5, 0, 16, &n, (rsa_key*)key);
...
qboolean Sec_MakeCertificate(rsa_key* key, const char* commonName,
  ...
  desc->init(&md);
  desc->process(&md, (const unsigned char*)&(out->certificate),
                sizeof(sec_cert_t));
...
qboolean Sec_WriteCertificateToFile(sec_certificate_t* certificate,
  ...
  cert = certificate;
  do {
    len = 16 + strlen(cert->certificate.commonName) +
          strlen(cert->certificate.companyName) + cert->sigSize;

    keysize = sizeof(buf);

    if (rsa_export((unsigned char*)buf, &keysize, PK_PUBLIC,
                   &(cert->certificate.pubKey)) != CRYPT_OK) {
      fclose(fp);
      return qfalse;
    }
    len += keysize;
