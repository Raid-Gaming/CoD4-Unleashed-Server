...
are created (demos, screenshots, etc) will be created reletive to the base path,
...
on the cd.  Files are never writen to the cd path.  It defaults to a value set
by the installer, like "e:\quake3", but it can be overridden with "+set
ds_cdpath g:\quake3".
...
After the paths are initialized, quake will look for the product.txt file.  If
not found and verified, the game will run in restricted mode.  In restricted
mode, only files contained in demoq3/pak0.pk3 will be available for loading, and
only if the zip header is verified to not have been modified.  A single
exception is made for q3config.cfg.  Files can still be written out in
restricted mode, so screenshots and demos are allowed. Restricted mode can be
tested by setting "+set fs_restrict 1" on the command line, even if there is a
valid product.txt under the basepath or cdpath.

If not running in restricted mode, and a file is not found in any local
filesystem, an attempt will be made to download it and save it under the base
path.

If the "fs_copyfiles" cvar is set to 1, then every time a file is sourced from
the cd path, it will be copied over to the base path.  This is a development aid
to help build test releases and to copy working sets over slow network links.
...
#define FS_ListFiles(dir, extension, nfiles)                                   \
  Sys_ListFiles(dir, extension, 0, nfiles, 0)
#define FS_FreeFileList Sys_FreeFileList

/*
typedef void (__cdecl *tFS_WriteFile)(const char* qpath, const void *buffer, int
size); tFS_WriteFile FS_WriteFile = (tFS_WriteFile)(0x818a58c);
...
qboolean FS_Initialized() { return (fs_searchpaths != NULL); }
  ...
  if (!FsThread) {
    for (i = 1; i < MAX_FILE_HANDLES; i++) {
      Com_Printf("FILE %2i: '%s' 0x%x\n", i, fsh[i].name,
                 fsh[i].handleFiles.file.o);
    }
    Com_Error(2, "FS_HandleForFile: none free");
  }
  Com_PrintWarning("FILE %2i: '%s' 0x%x\n", startIndex, fsh[startIndex].name,
                   fsh[startIndex].handleFiles.file.o);
...
void FS_BuildOSPathForThread(const char* base, const char* game,
                             const char* qpath, char* fs_path, int fs_thread) {
  ...
  if (Com_sprintf(fs_path, MAX_OSPATH, "%s/%s/%s", basename, gamename, qpath) >=
      MAX_OSPATH) {
    if (fs_thread) {
      fs_path[0] = 0;
      return;
...
===========
*/
void FS_RemoveOSPath(const char* osPath) { remove(osPath); }
...
char* FS_SV_GetFilepath(const char* file, char* testpath, int lenpath) {
  ...
  if (lenpath < MAX_OSPATH)
    Com_Error(ERR_FATAL,
              "FS_SV_GetFilepath: Given buffer has less than %d bytes length\n",
              MAX_OSPATH);
...
long FS_FOpenFileReadDir(const char* filename, searchpath_t* search,
  ...
  mvabuf;
  char* noReferenceExts[] = {
      "cod4_lnxded-bin", ".so",   ".dll",   ".hlsl", ".txt", ".cfg",
      ".levelshots",     ".menu", ".arena", ".str",  NULL};
  char** testExt;
  ...
  if (file == NULL) {
    ...
    if (search->localized && !fs_ignoreLocalized->boolean &&
        search->langIndex != loc_language->integer)
      return 0;
    ...
    if (search->pack) {
      ...
      if (search->pack->hashTable[hash]) {
        ...
        do {
          ...
          if (!FS_FilenameCompare(pakFile->name, filename)) {
            ...
            err = unzGetCurrentFileInfo(pak->handle, &file_info, netpath,
                                        sizeof(netpath), NULL, 0, NULL, 0);
            if (err != UNZ_OK) {
              Com_PrintWarning("Read error in Zip-file: %s\n",
                               pak->pakFilename);
              return 1;
      ...
      FS_BuildOSPathForThread(dir->path, dir->gamedir, filename, netpath,
                              FsThread);
    ...
    if (search->pack->hashTable[hash]) {
      ...
      do {
        ...
        if (!FS_FilenameCompare(pakFile->name, filename)) {
          ...
          // mark the pak as having been referenced and mark specifics on cgame
          // and ui shaders, txt, arena files  by themselves do not count as a
          // reference as these are loaded from all pk3s from every pk3 file..
          len = strlen(filename);
          ...
          // set the file position in the zip file (also sets the current file
          // info)
          ...
          err =
              unzGetCurrentFileInfo(fsh[*file].handleFiles.file.z, &file_info,
                                    netpath, sizeof(netpath), NULL, 0, NULL, 0);
          if (err != UNZ_OK) {
            Com_PrintWarning("Read error in Zip-file: %s\n", pak->pakFilename);
            return 1;
          }
...
===========
*/
long FS_FOpenFileReadForThread(const char* filename, fileHandle_t* file,
                               int fsThread) {
  searchpath_t* search;
  long len;
...
qboolean FS_Which(const char* filename, void* searchPath) {
  ...
  if (FS_FOpenFileReadDir(filename, search, NULL, qfalse, qfalse, 0) > 0) {
    if (search->pack) {
      ...
      FS_BuildOSPathForThread(search->dir->path, search->dir->gamedir, filename,
                              netpath, 0);
...
/*
 ...
 =============
 */
void FS_FreeFileKeepBuf() { FS_LoadStackDec(); }
...
int FS_SV_FOpenFileRead(const char* filename, fileHandle_t* fp) {
  ...
  if (!fsh[f].handleFiles.file.o) {
    // NOTE TTimo on non *nix systems, fs_homepath == fs_basepath, might want to
    // avoid
    ...
    while (remaining) {
      ...
      if (read == 0) {
        ...
        if (!tries) {
          ...
          return len -
                 remaining; // Com_Error (ERR_FATAL, "FS_Read: 0 bytes read");
        }
      }
...
static fileHandle_t FS_SV_FOpenFileWriteGeneric(const char* filename,
                                                const char* basepath) {
  char ospath[MAX_OSPATH];
  fileHandle_t f;
  mvabuf;
...
static int FS_SV_WriteFileGeneric(const char* qpath, const void* buffer,
                                  int size, const char* basepath) {
  fileHandle_t f;
  int len;
...
// CVE-2006-2082
// compared requested pak against the names as we built them in
...
qboolean FS_VerifyPak(const char* pak) {
  ...
  Com_sprintf(teststring, sizeof(teststring), "%s/mod.ff",
...
static pack_t* FS_LoadZipFile(char* zipfile, const char* basename) {
  ...
  if (strlen(pack->pakBasename) > 4 &&
      !Q_stricmp(pack->pakBasename + strlen(pack->pakBasename) - 4, ".iwd")) {
  ...
  pack->checksum = Com_BlockChecksumKey32(fs_headerLongs, 4 * fs_numHeaderLongs,
                                          LittleLong(0));
  if (fs_checksumFeed)
    pack->pure_checksum = Com_BlockChecksumKey32(
        fs_headerLongs, 4 * fs_numHeaderLongs, LittleLong(fs_checksumFeed));
  else
...
void SEH_InitLanguage() {
  ...
  loc_forceEnglish = Cvar_RegisterBool("loc_forceEnglish", 0, 0x21u,
                                       "Force english localized strings");
  loc_translate =
      Cvar_RegisterBool("loc_translate", qtrue, 0x20u, "Enable translations");
  loc_warnings =
      Cvar_RegisterBool("loc_warnings", 0, 0, "Enable localization warnings");
  loc_warningsAsErrors =
      Cvar_RegisterBool("loc_warningsAsErrors", 0, 0,
...
void FS_InitFilesystem() {
  ...
  //    Com_Error(ERR_DROP, "Couldn't load %s.  Make sure Call of Duty is run
  //    from the correct folder.", "fileSysCheck.cfg");
  Q_strncpyz(lastValidBase, fs_basepath->string, sizeof(lastValidBase));
  Q_strncpyz(lastValidGame, fs_gameDirVar->string, sizeof(lastValidGame));
}

qboolean FS_GameDirDomainFunc(const char* cvar_name,
                              const char* fs_gamestring) {
  ...
  if (Q_stricmpn(fs_gamestring, "mods", 4) || strlen(fs_gamestring) < 6 ||
      ...
      strstr(fs_gamestring, "..") || strstr(fs_gamestring, "::")) {
...
void FS_AddGameDirectory_Single(const char* path, const char* dir_nolocal,
                                qboolean last, int index);
...
int SEH_GetCurrentLanguage(void) { return loc_language->integer; }
...
void FS_Startup(const char* gameName) {
  ...
  fs_debug = Cvar_RegisterInt("fs_debug", 0, 0, 2, 0,
                              "Enable file system debugging information");
  fs_copyfiles = Cvar_RegisterBool("fs_copyfiles", 0, 16,
                                   "Copy all used files to another location");
  fs_cdpath =
      Cvar_RegisterString("fs_cdpath", Sys_DefaultCDPath(), 16, "CD path");
  fs_basepath = Cvar_RegisterString("fs_basepath", Sys_DefaultInstallPath(),
                                    528, "Base game path");
  fs_basegame = Cvar_RegisterString("fs_basegame", "", 16, "Base game name");
  fs_gameDirVar = Cvar_RegisterString(
      "fs_game", "", 28,
      "Game data directory. Must be \"\" or a sub directory of 'mods/'.");
  fs_ignoreLocalized = Cvar_RegisterBool("fs_ignoreLocalized", qfalse, 160,
  ...
  fs_homepath =
      Cvar_RegisterString("fs_homepath", homePath, 528, "Game home path");
  fs_restrict = Cvar_RegisterBool("fs_restrict", qfalse, 16,
                                  "Restrict file access for demos etc.");
  fs_usedevdir = Cvar_RegisterBool("fs_usedevdir", qfalse, 16,
  ...
  if (fs_homepath->string[0] &&
      Q_stricmp(fs_basepath->string, fs_homepath->string) &&
      fs_usedevdir->string) {
  ...
  if (fs_cdpath->string[0] &&
      Q_stricmp(fs_basepath->string, fs_cdpath->string)) {
    if (fs_usedevdir->string) {
  ...
  if (fs_gameDirVar->string[0] && !Q_stricmp(gameName, BASEGAME) &&
      Q_stricmp(fs_gameDirVar->string, gameName) && levelname &&
      levelname->string[0]) {
    if (fs_cdpath->string[0])
      FS_AddGameDirectory(fs_cdpath->string,
                          va("usermaps/%s", levelname->string));
    if (fs_basepath->string[0])
  ...
  for (sp = fs_searchpaths; sp; sp = sp->next) {
    if (sp->dir && !Q_stricmp(sp->dir->path, path) &&
      ...
      if (localized != sp->localized) {
        ...
        Com_PrintWarning("WARNING: game folder %s/%s added as both localized & "
                         "non-localized. Using folder as %s\n",
                         path, dir, localization);
      }
      if (sp->localized && index != sp->localized)
        Com_PrintWarning("WARNING: game folder %s/%s re-added as localized "
...
void FS_AddIwdFilesForGameDirectory(const char* path, const char* dir) {
  ...
  if (numfiles > MAX_PAKFILES) {
    Com_PrintWarning(
        ...
        dir, numfiles, MAX_PAKFILES);
    numfiles = MAX_PAKFILES;
  }
  if (!numfiles && !Q_stricmp(dir, BASEGAME) &&
      !Q_stricmp(path, fs_basepath->string))
  ...
  for (i = 0; i < numfiles; i++) {
    if (!Q_strncmp(sorted[i], "          ", 10)) {
      ...
      if (!language[0]) {
        Com_PrintWarning("WARNING: Localized assets iwd file %s/%s/%s has "
                         "invalid name (no language specified). Proper naming "
                         "convention is: localized_[language]_iwd#.iwd\n",
                         path, dir, sorted[i]);
        continue;
      }
      if (!SEH_GetLanguageIndexForName(language, &langindex)) {
        Com_PrintWarning(
            "WARNING: Localized assets iwd file %s/%s/%s has invalid name (bad "
            "language name specified). Proper naming convention is: "
      ...
      if (!Q_stricmp(dir, BASEGAME) && !Q_stricmp(path, fs_basepath->string) &&
          Q_stricmpn(sorted[i], "iw_", 3) &&
          Q_stricmpn(sorted[i], "xiceops_", 7)) {
    ...
    for (sp = fs_searchpaths; sp != NULL; sp = sp->next) {
      if (sp->pack != NULL &&
          !Q_stricmp(sp->pack->pakBasename, pak->pakBasename)) {
...
void FS_Restart(int checksumFeed) {
  ...
  /*	if ( FS_ReadFile( "default.cfg", NULL ) <= 0 ) {
                  // this might happen when connecting to a pure server not
     using BASEGAME/pak0.pk3
...
// void DB_BuildOSPath(const char *filename<eax>, int ffdir<edx>, int len<ecx>,
// char *buff)
__regparm3 void DB_BuildOSPath(const char* filename, int ffdir, int len,
                               char* buff) {
  ...
  case 1:
    ...
    Com_sprintf(ospath, sizeof(ospath), "%s/%s.ff", fs_gamedirvar->string,
                filename);
    FS_SV_GetFilepath(ospath, buff, len);
    return;
  ...
  case 2:
    ...
    Com_sprintf(ospath, sizeof(ospath), "%s/%s/%s.ff", "usermaps", mapname,
                filename);
    FS_SV_GetFilepath(ospath, buff, len);
    return;
  }
...
void __cdecl FS_AddIwdPureCheckReference(searchpath_t* search) {
  ...
  Q_strncpyz(newCheck->baseName, search->pack->pakBasename,
             sizeof(newCheck->baseName));
  Q_strncpyz(newCheck->gameName, search->pack->pakGamename,
             sizeof(newCheck->gameName));
    ...
    if (search->pack && !search->localized) {
      if (!(search->pack->referenced & FS_GENERAL_REF) &&
          (!Q_stricmp(search->pack->pakGamename, fs_gameDirVar->string) ||
           !Q_stricmpn(search->pack->pakGamename, "usermaps", 8))) {
...
int FS_PureServerSetLoadedIwds(const char* paksums, const char* paknames) {
  ...
  if (numPakSums) {
    ...
    for (pureSums = fs_iwdPureChecks; pureSums; pureSums = pureSums->next) {
      ...
      for (i = 0; i < numPakSums; i++) {
        if (lpakSums[i] == pureSums->checksum &&
            !Q_stricmp(lpakNames[i], pureSums->baseName)) {
          break;
        }
      }
      if (i == numPakSums) {
  ...
  if (numPakSums == fs_numServerIwds && rt == 0) {
    for (i = 0, k = 0; i < fs_numServerIwds;) {
      if (lpakSums[k] == fs_serverIwds[i] &&
          !Q_stricmp(lpakNames[k], fs_serverIwdNames[i])) {
        ++k;
        if (k < numPakSums) {
          i = 0;
          continue;
