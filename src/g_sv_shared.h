...
#define playerStates 0x084BD120
#define sizeOfPlayer 0x3184
...
void __regparm3 G_SayTo(gentity_t* ent, gentity_t* other, int mode, int color,
                        const char* teamname, const char* name,
                        const char* message);
...
void __cdecl SV_GameSendServerCommand(int clientnum, int svscmd_type,
                                      const char* text);
// tSV_GameSendServerCommand SV_GameSendServerCommand =
// (tSV_GameSendServerCommand)(0x817ce42);
...
__cdecl __optimize3 qboolean Jump_GetStepHeight(playerState_t* ps,
                                                const vec3_t vec1, float* val2);
...
const char* __cdecl ClientConnect(
    int clnum, short clscriptid); // Something simular to VM_Call
