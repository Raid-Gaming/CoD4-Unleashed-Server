...
void Init_CallVote(void) {
  ...
  g_votedMapName = Cvar_RegisterString("g_votedMapName", "", 0,
                                       "Contains the voted mapname");
  g_votedGametype = Cvar_RegisterString("g_votedGametype", "", 0,
                                        "Contains the voted gametype");
  g_voteTime = Cvar_RegisterInt("g_voteTime", 30, 10, 90, 0,
                                "Duration a called vote is active");
  g_voteBanTime =
      Cvar_RegisterInt("g_voteBanTime", 15, 1, 240, 0,
                       "Duration a player is banned after successful votekick");
  g_voteMaxVotes = Cvar_RegisterInt("g_voteMaxVotes", 2, 1, 10, 0,
                                    "How many votes a player can call");
  g_voteVoteGametypes =
      Cvar_RegisterString("g_voteVoteGametypes", "", 0,
                          "Contains a list of gametypes that are allowed to "
                          "vote. Empty list = all");
  g_voteKickMinPlayers =
      Cvar_RegisterInt("g_voteKickMinPlayers", 5, 0, 14, 0,
                       "How many active players are needed on server to allow "
                       "calling a kickvote");
  g_voteAllowMaprotate = Cvar_RegisterBool("g_voteAllowMaprotate", qtrue, 0,
                                           "Allow calling map_rotate votes");
  g_voteAllowKick = Cvar_RegisterBool("g_voteAllowKick", qtrue, 0,
                                      "Allow calling kick votes");
  g_voteAllowGametype = Cvar_RegisterBool("g_voteAllowGametype", qtrue, 0,
                                          "Allow calling gametype votes");
  g_voteAllowMap =
      Cvar_RegisterInt("g_voteAllowMap", 1, 0, 2, 0,
                       "Allow calling next map setting votes - 0=disabled, "
                       "1=only from rotation, 2=Any map");
  g_voteAllowRestart = Cvar_RegisterBool("g_voteAllowRestart", qtrue, 0,
                                         "Allow calling map restart votes");
  ...
  g_voteFlags |= g_voteAllowMap->boolean && g_voteAllowGametype->boolean
                     ? VOTEFLAGS_TYPE
                     : 0;
...
__cdecl void Cmd_CallVote_f(gentity_t* ent) {
  ...
  if (!g_allowVote->boolean) {
    SV_GameSendServerCommand(ent - g_entities, 0,
                             va("%c \"GAME_VOTINGNOTENABLED\"\0", 0x65));
    return;
  }
  if (level.voteTime) {
    SV_GameSendServerCommand(ent - g_entities, 0,
                             va("%c \"GAME_VOTEALLREADYINPROGRESS\"\0", 0x65));
    return;
  }
  if (ent->client->pers.voteCount >= g_voteMaxVotes->integer) {
    SV_GameSendServerCommand(
        ent - g_entities, 0,
        va("%c \"You have called too many votes\"\0", 0x65));
    return;
  }
  if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) {
    SV_GameSendServerCommand(ent - g_entities, 0,
                             va("%c \"GAME_NOSPECTATORCALLVOTE\"\0", 0x65));
    return;
  }
  ...
  if (strchr(arg1, ';') || strchr(arg2, ';') || strchr(arg3, ';')) {
    SV_GameSendServerCommand(ent - g_entities, 0,
                             va("%c \"GAME_INVALIDVOTESTRING\"\0", 0x65));
    return;
  }
  ...
  if (!Q_stricmp(arg1, "map_restart")) {
    ...
    SV_GameSendServerCommand(
        ent - g_entities, 0,
        va("%c \"GAME_VOTECOMMANDSARE\x15 map_restart, map_rotate, map "
           "<mapname>, g_gametype <gametype>, kick <player or clientnum>, "
           "typemap <gametype> <map>\"\0",
           0x65));
    return;
  }
  ...
  if (!(g_voteFlags & mask)) {
    SV_GameSendServerCommand(
        ent - g_entities, 0,
        va("%c \"Voting for %s is disabled on this server\"\0", 0x65, arg1));
    return;
  }
  ...
  if (!Q_stricmp(arg1, "typemap")) {
    ...
    if (*g_voteVoteGametypes->string) {
      if (!strstr(g_voteVoteGametypes->string, arg2)) {
        SV_GameSendServerCommand(
            ent - g_entities, 0,
            va("%c \"Voting for gametype %s is disabled on this server\"\0",
               0x65, arg2));
        return;
      }
    }
    ...
    if (i == g_gametypes->numGametypes) {
      SV_GameSendServerCommand(ent - g_entities, 0,
                               va("%c \"GAME_INVALIDGAMETYPE\"\0", 0x65));
      return;
    }
    ...
    if (!(g_voteFlags & VOTEFLAGS_ANYMAP)) {
      if (!strstr(SV_GetMapRotation(), va("map %s", arg3))) {
        SV_GameSendServerCommand(
            ent - g_entities, 0,
            va("%c \"Voting for map %s is disabled on this server\"\0", 0x65,
               arg3));
        return;
      }
    }
    Com_sprintf(level.voteString, sizeof(level.voteString),
                "set g_votedGametype %s; set g_votedMapName %s\n", arg2, arg3);
    Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
                "Set next map to: %s and gametype to: \x14%s", arg3,
                g_gametypes->gametype[i].gametypereadable);
    ...
    Com_sprintf(level.voteString, sizeof(level.voteString),
                "set g_gametype %s; map_restart\n", arg2);
    Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
                "Set gametype to: \x15%s and restart",
                g_gametypes->gametype[i].gametypename);
    ...
    if (!(g_voteFlags & VOTEFLAGS_ANYMAP)) {
      if (!strstr(SV_GetMapRotation(), va("map %s", arg2))) {
        SV_GameSendServerCommand(
            ent - g_entities, 0,
            va("%c \"This server does not allow voting for maps which aren't "
               "part of map-rotation\"\0",
               0x65));
        return;
      }
    }
    Com_sprintf(level.voteString, sizeof(level.voteString),
                "set g_votedMapName %s\n", arg2);
    Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
                "Set next map to: %s", arg2);
  } else if (!Q_stricmp(arg1, "map_rotate")) {

    if (*g_votedMapName->string) {
      if (*g_votedGametype->string)
        Com_sprintf(level.voteString, sizeof(level.voteString),
                    "set g_gametype %s; map %s; set g_votedGametype \"\"; set "
                    "g_votedMapName \"\"\n",
                    g_votedGametype->string, g_votedMapName->string);
      else
        Com_sprintf(level.voteString, sizeof(level.voteString),
                    "map %s; set g_votedMapName \"\"\n",
    ...
    if (activePlayers < g_voteKickMinPlayers->integer) {
      SV_GameSendServerCommand(
          ent - g_entities, 0,
          va("%c \"GAME_VOTINGNOTENOUGHPLAYERS\"\0", 0x65));
      return;
    }
    ...
    if (kicknum != MAX_CLIENTS) { // found a client # to kick, so override
                                  // votestring with better one

      Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
                  "%s wants to kick and tempban: %s", ent->client->pers.netname,
                  level.clients[kicknum].pers.netname);

      if (svs.clients[kicknum].power > 5) {
        SV_GameSendServerCommand(
            ent - g_entities, 0,
            va("%c \"Can not call a vote against a protected player\"\0",
               0x65));
        level.voteDisplayString[0] = 0;
        return;
      } else {
        Com_sprintf(level.voteString, sizeof(level.voteString),
                    "tempban \"%d\" %im Vote kick; kick \"%d\" Vote kick\n",
                    kicknum, g_voteBanTime->integer, kicknum);
      }

    } else { // if it can't do a name match, don't allow kick (to prevent
             // votekick text spam wars)
      SV_GameSendServerCommand(ent - g_entities, 0,
                               va("%c \"GAME_CLIENTNOTONSERVER\"\0", 0x65));
      level.voteDisplayString[0] = 0;
      return;
    }
    // jpw
    ...
    //\"%s\"", arg1, arg2 ); 		Com_sprintf( level.voteDisplayString,
    //sizeof( level.voteDisplayString ), "%s", level.voteString );
  }

  SV_GameSendServerCommand(
      -1, 0,
      va("%c \"GAME_CALLEDAVOTE\x15%s\"\0", 0x65, ent->client->pers.netname));
  ...
  SV_SetConfigstring(CS_VOTE_TIME,
                     va("%i %i", level.voteTime, sv_serverid->integer));
...
qboolean Cmd_FollowClient_f(gentity_t* ent, int clientnum) {
  ...
  ent->client->sess.lastFollowedClient =
      -1; // Reset this to prevent strange things from happening
  ...
  if (ClientCanSpectateTeam(ent->client,
...
__cdecl void StopFollowingOnDeath(gentity_t* ent) {
  ...
  if (ent->client->sess.spectatorClient != -1)
    ent->client->sess.lastFollowedClient =
        ent->client->sess.spectatorClient; // saving the last followed player
...
__cdecl void G_Say(gentity_t* ent, gentity_t* target, int mode,
                   const char* chatText) {
  int j;
  gentity_t* other;
  int color;
  ...
  if (textptr[0] == '/' || textptr[0] == '$' ||
      (textptr[0] == '!' &&
       !g_disabledefcmdprefix->boolean)) { // Check for Command-Prefix
    textptr++;
  ...
  */
  ...
  default:
  case SAY_ALL:
    G_LogPrintf("say;%s;%d;%s;%s\n", SV_GetGuid(ent->s.number), ent->s.number,
                name, text);
    teamname = "";
    color = COLOR_WHITE;
    break;
  case SAY_TEAM:
    G_LogPrintf("sayteam;%s;%d;%s;%s\n", SV_GetGuid(ent->s.number),
                ent->s.number, name, text);
    if (ent->client->sess.sessionTeam == TEAM_RED) {
...
void G_AddChatRedirect(void (*rd_dest)(const char*, int, int)) {
  ...
  for (i = 0; i < MAX_REDIRECTDESTINATIONS; i++) {
    if (rd_destinations[i] == rd_dest) {
      Com_Error(ERR_FATAL, "G_AddChatRedirect: Attempt to add an already "
  ...
  Com_Error(ERR_FATAL,
            "G_AddChatRedirect: Out of redirect handles. Increase "
