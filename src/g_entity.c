...
qboolean __cdecl G_SpawnString(SpawnVar_t* spawnVar, const char* key,
                               const char* defaultString, const char** out) {
  int i;
...
qboolean __cdecl G_LevelSpawnString(const char* key, const char* defaultString,
                                    const char** out) {
...
void __cdecl G_VehSpawner(gentity_t* ent) {
  ...
  if (level.spawnVars.isGameLoadingSpawn) {
    ...
    if (Scr_GetNumParam() == 3) {
      // Scr_Error("Usage: spawn(\"script_vehicle\", <origin>, <spawnflags>,
      // <vehicletype>)");
