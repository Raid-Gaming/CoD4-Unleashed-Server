...
#define XO3(x, x1, x2, y1, y2, z1, z2)                                         \
  XML_OpenTag(xmlobj, x, 3, x1, x2, y1, y2, z1, z2)
#define XO4(x, x1, x2, y1, y2, z1, z2, xx1, xx2)                               \
  XML_OpenTag(xmlobj, x, 3, x1, x2, y1, y2, z1, z2, xx1, xx2)
#define XO5(x, x1, x2, y1, y2, z1, z2, xx1, xx2, yy1, yy2)                     \
  XML_OpenTag(xmlobj, x, 3, x1, x2, y1, y2, z1, z2, xx1, xx2, yy1, yy2)
#define XC XML_CloseTag(xmlobj)
#define XOLINK(x) XO1(TOSTRING(a), TOSTRING(href), x)

char* htmlq3colors[] = {"#000000", "#FF0000", "#00FF00", "#FFFF00", "#0000FF",
                        "#00FFFF", "#FF00FF", "#A0A0A0", "#7B8FB0", "#A69169"};

char* Webadmin_ConvertToHTMLColor(char* qcolorstr, char* hcolorstr,
                                  int maxlen) {
  ...
  for (i = 0, y = 0; qcolorstr[i] && y < maxlen; ++i) {
    if (qcolorstr[i] == '^' && qcolorstr[i + 1] <= '9' &&
        qcolorstr[i + 1] >= '0') {
      if (incolor) {
      ...
      len = Com_sprintf(htmlcolor, sizeof(htmlcolor),
                        "<span style=\"color: %s\">", htmlq3colors[colorindex]);
...
const char* Webadmin_GetUrlVal(const char* url, const char* search,
...
void Webadmin_BuildAdminList(xml_t* xmlobj, int uid) {
  ...
  for (i = 0, cnt = 0, badmin = Auth_GetAdminFromIndex(i); badmin != NULL;
       i++, badmin = Auth_GetAdminFromIndex(i)) {
    ...
    XO("td"); // Name
    XA(Webadmin_ConvertToHTMLColor(badmin->username, colorbuf,
                                   sizeof(colorbuf)));
    XC;
...
void Webadmin_AddAdmin(xml_t* xmlobj, int uid, int adminuid, char* password,
                       int power) {
  /*
          mvabuf;

          if(Auth_GetClPowerByUID(uid) <
     Cmd_GetPower("adminaddadminwithpassword"))
          {
                  XA("Insufficient permissions");
                  return;
          }
...
void Webadmin_BuildServerStatus(xml_t* xmlobj, qboolean showfullguid) {
  ...
  for (cl = svs.clients, gclient = level.clients, i = 0;
    ...
    if (gclient->sess.sessionTeam == TEAM_RED ||
        gclient->sess.sessionTeam == TEAM_BLUE) {
      teambased = qtrue;
      break;
    }
  ...
  XO1("table", "class", "table table-striped table-bordered cod4xtable");
  XA("<th>CID</th><th>Name</th><th>UID/GUID</th><th>Power</th><th>Score</"
     "th><th>Ping</th>");
  ...
  if (teambased) {
    ...
    for (cl = svs.clients, gclient = level.clients, i = 0;
      ...
      Com_sprintf(strbuf, sizeof(strbuf), "<td>%d</td>",
                  gclient->pers.scoreboard.score); // Score
      XA(strbuf);
    ...
    if ((arg1 = Webadmin_GetPostVal(values, "cid"))) {
      ...
      if (cid >= 0 && cid < sv_maxclients->integer) {
        if (uid > 0 || strlen(svs.clients[cid].pbguid) == 32) {
          SV_AddBan(svs.clients[cid].uid, uid, svs.clients[cid].pbguid,
                    svs.clients[cid].name, (time_t)-1, (char*)reason);
        }
        SV_PlayerAddBanByip(&svs.clients[cid].netchan.remoteAddress,
                            (char*)reason, svs.clients[cid].uid,
                            svs.clients[cid].pbguid, uid, -1);
        XA("Banned player");
...
void Webadmin_ConsoleCommand(xml_t* xmlobj, const char* command, int uid) {
  ...
  if ((power = Auth_GetClPowerByUID(uid)) < 100) {
    ...
    // Prevent buffer overflow as well as prevent the execution of priveleged
    // commands by using seperator characters
    ...
    Com_BeginRedirect(sv_outputbuf, SV_OUTPUTBUF_LENGTH,
                      Webadmin_FlushRedirect);
...
#ifdef PUNKBUSTER
    if (!Q_stricmpn(command, "pb_sv_", 6))
...
void Webadmin_BuildLoginForm(xml_t* xmlobj, qboolean badtry,
  ...
  XA("<label for=\"username\">Username</label> <input class=\"form-control\" "
     "type=\"text\" id=\"username\" name=\"username\">");
  XA("<label for=\"password\">Password</label> <input class=\"form-control\" "
...
void Webadmin_BuildMessage(msg_t* msg, const char* username,
                           qboolean invalidloginattempt, const char* banmsg,
                           const char* url, httpPostVals_t* values) {
  ...
  XC;
  XO("h3");
  XA(Webadmin_ConvertToHTMLColor(sv_hostname->string, colorbuf,
                                 sizeof(colorbuf)));
  XA(" ");
  ...
  if (!Q_strncmp(url, "/webadmin", 9)) {
    ...
    if (username == NULL || username[0] == '\0') {
      ...
      if (!Q_strncmp(url + 9, "/listadmins", 11)) {
        ...
        XO("hr");
        XC;
        if (Webadmin_GetUrlVal(url, "action", actionval,
                               sizeof(actionval))) // nnjpls
        {
        ...
        XO5("form", "name", "input", "action", "webadmin?action=sendcmd",
            "method", "post", "class", "form-control", "id", "con_form");
        XA("<label for=\"consolecommand\">Send Command</label> <input "
           "type=\"text\" name=\"consolecommand\" id=\"consolecommand\">");
        XA("<button class=\"givesomespace btn btn-primary btn-xs\" "
    ...
    XA("<a href=\"/webadmin\" class=\"btn btn-primary givesomespace\">Web "
       "Admin</a>");
    XA("<a href=\"/status\" class=\"btn btn-primary givesomespace\">Server "
       "Status</a>");
    XC;
  }
  XO("p");
...
qboolean HTTPCreateWebadminMessage(ftRequest_t* request, msg_t* msg,
  ...
  if (username == NULL) {
    ...
    if (username && password) {
      ...
      if (session == NULL) {
        ...
        SV_PlayerAddBanByip(
            &request->remote,
            "Invalid login attempt. You have to wait 20 seconds", 0, NULL, 0,
            Com_GetRealtime() + 10);
        username = NULL;
      } else {
  ...
  Webadmin_BuildMessage(msg, username, invalidlogin, NULL, request->url,
                        values);
