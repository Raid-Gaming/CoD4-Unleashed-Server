...
char* HStorage_ValueToString(varType_t type, vsValue_t* value, char* buf,
                             int buflen) {
  switch (type) {
  default:
    return "NULL";
  ...
  case VSVAR_FLOAT:
    ...
    return buf;
  case VSVAR_VECTOR:
    Com_sprintf(buf, buflen, "%g %g %g", value->vector[0], value->vector[1],
                value->vector[2]);
    return buf;
  }
}

qboolean HStorage_StringToValue(varType_t type, char* string,
                                vsValue_t* value) {
  switch (type) {
  default:
...
void HStorage_ObjectAllocTables(vsMemObj_t* obj) {
  ...
  memset(&HStorage_GetMemoryStart(obj)[obj->store.hashtableOffset], -1,
         obj->store.hashtableLength);
  memset(&HStorage_GetMemoryStart(obj)[obj->store.memUnitsOffset], 0,
         obj->store.memUnitsLength);
}
...
qboolean HStorage_BeginDataInternal(vsMemObj_t* obj, varType_t type,
                                    const char* name) {
...
qboolean HStorage_AddDataInternal(vsMemObj_t* obj, vsValue_t* value) {
  ...
  if (header->numelements >= MAX_ARRAY_SIZE) {
    obj->lastError =
        "HStorage_AddData: Exceeded number of MAX_ARRAY_SIZE elements";
    return qfalse;
  }
  ...
  case VSVAR_STRING:
    ...
    if (header->dataSize + HStorage_GetHeaderSize() + datalen >=
        sizeof(obj->iter.tempunits)) {
      obj->lastError = "HStorage_AddData: Exceed limited of bytes which can be "
...
int HStorage_GetTableDataIndex(vsMemObj_t* obj, char* name) {
  ...
  units =
      (vsMemUnit_t*)&HStorage_GetMemoryStart(obj)[obj->store.memUnitsOffset];
  ...
  for (i = 0; i < obj->table.numFields; ++i) {
    ...
    if (hashtable[tableindex] == -1 ||
        strcmp(name, (char*)&(units[hashtable[tableindex]].header.dataStart)) ==
            0) {
  ...
  if (i == obj->table.numFields) {
    ...
    obj->lastError = "HStorage_GetTableDataIndex: Hashtable has overflowed. "
                     "Should never happen!";
    return -1;
  }
...
int HStorage_EndDataInternal(vsMemObj_t* obj) {
  ...
  if (index != -1) {
    ...
    if (HStorage_GetAvailableBytesFromDatasize(units[index].header.dataSize) <
        tmpheader->dataSize) {
  ...
  if (index + numUnits >= obj->table.numUnits) {
    obj->table.overflowed = qtrue;
    obj->lastError =
        "HStorage_EndData: Available variable storage has exceeded. Requires "
        "to relocate 1st on a larger amount of memory";
    return 0;
  }
  ...
  memcpy(&(units[index]), &(obj->iter.tempunits[0]),
         tmpheader->dataSize + HStorage_GetHeaderSize());
  obj->lastError = "HStorage_EndData: Success";
  return 1;
}

/* Functions to retrieve an object */
qboolean HStorage_GetBeginDataSetupIterInternal(vsMemObj_t* obj,
                                                int tableindex) {
  int index;
  int* hashtable;
  vsMemHeader_t* header;
  ...
  if (tableindex == -1) {
    obj->lastError =
        "HStorage_GetBeginDataSetupIter: Invalid hashtable index given";
    return qfalse;
  }
  index = hashtable[tableindex];
  /* Not found */
  if (index == -1) {
    obj->lastError = "HStorage_GetBeginDataSetupIter: The element which is "
...
int HStorage_GetBeginDataInternal(vsMemObj_t* obj, char* name,
                                  varType_t* type) {
  int tableindex;
  vsMemUnit_t* units;
  vsMemHeader_t* header;
...
int HStorage_GetDataInternal(vsMemObj_t* obj, vsValue_t* value) {
  ...
  if (obj->iter.elemCount >= header->numelements) {
    obj->lastError = "HStorage_GetData: All elements have been already "
                     "retrieved. No more are available.";
    return 0;
  }
  ...
  case VSVAR_BOOLEAN:
    datalen = sizeof(byte);
    value->boolean =
        ((byte*)(((byte*)(&header->dataStart) + obj->iter.dataOffset)))[0];
    break;

  case VSVAR_INTEGER:
    datalen = sizeof(int32_t);
    value->integer =
        ((int32_t*)(((byte*)(&header->dataStart) + obj->iter.dataOffset)))[0];
    break;

  case VSVAR_FLOAT:
    datalen = sizeof(float);
    value->floatVar =
        ((float*)(((byte*)(&header->dataStart) + obj->iter.dataOffset)))[0];
    break;
  ...
  case VSVAR_VECTOR:
    ...
    value->vector[0] =
        ((float*)(((byte*)(&header->dataStart) + obj->iter.dataOffset)))[0];
    value->vector[1] =
        ((float*)(((byte*)(&header->dataStart) + obj->iter.dataOffset)))[1];
    value->vector[2] =
        ((float*)(((byte*)(&header->dataStart) + obj->iter.dataOffset)))[2];
    break;
  }
...
int HStorage_IterGetNextInfo(vsMemObj_t* obj, char* name, varType_t* type) {
  ...
  while (obj->iter.tableindex < obj->table.numFields) {
    ...
    if (HStorage_GetBeginDataSetupIterInternal(obj, obj->iter.tableindex) ==
        qfalse) {
      ++obj->iter.tableindex;
      continue;
    }
    ...
    if (obj->iter.unit < 0) {
      obj->lastError =
          "HStorage_IterGetNextInfo: Iterator has invalid unit index";
      return 0;
    }
    header = &units[obj->iter.unit].header;
    Q_strncpyz(name, (char*)&header->dataStart, MAX_VARNAME);
...
void HStorage_IterInit(vsMemObj_t* obj) { obj->iter.tableindex = 0; }
...
vsMemObj_t* HStorage_Relocate(vsMemObj_t* obj) {
  ...
  while (HStorage_IterHasNext(obj)) {
    ...
    case 0:
      obj->lastError = "HStorage_Relocate: Not enough space after relocation. "
                       "This is considered as fatal";
      free(newobj);
      return obj;
    case 1:
      /* Usual success case */
  ...
  memcpy(newobj->iter.tempunits, obj->iter.tempunits,
         sizeof(newobj->iter.tempunits));
    ...
    *infostring = 0;
    ...
    for (i = 0; i < count; i++) {
      ...
      if (type == VSVAR_STRING) {
        ...
        BigInfo_SetEncodedValueForKey(infostring, va("v%d", i), string,
                                      strlen(string));
      } else {
        BigInfo_SetValueForKey(
            infostring, va("v%d", i),
            HStorage_ValueToString(type, &value, buf, sizeof(buf)));
      }
    }
...
qboolean HStorage_ParseLine(varStorage_t* vobj, char* line, int linenumber) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (varType == VSVAR_STRING) {
      outlen =
          BigInfo_DecodedValueForKey(line, queryString, outbuf, sizeof(outbuf));
...
qboolean HStorage_LoadDataFromFile(varStorage_t* vobj, const char* filename) {
  ...
  for (i = 0, error = 0; error < 32; i++) {
    ...
    if (!HStorage_ParseLine(vobj, buf, i + 1))
...
/* Preparing the tempdata object */
qboolean HStorage_BeginData(varStorage_t* obj, varType_t type,
                            const char* name) {
