...
void Sys_ReplaceProcess(char* cmdline) {
  ...
  if (Cmd_Argc() < 256) {
    ...
    Com_PrintError("Sys_ReplaceProcess: Exceeded limit of 256 commandline "
                   "arguments.\nCommandline is: %s\n",
                   cmdline);
  }
...
==================
*/
const char* Sys_Dirname(char* path) { return dirname(path); }
...
qboolean Sys_MemoryProtectWrite(void* startoffset, int len) {
  ...
  if (mprotect(startoffset - ((int)startoffset % getpagesize()),
               len + (len % getpagesize()), PROT_READ | PROT_WRITE) != 0) {
...
qboolean Sys_MemoryProtectExec(void* startoffset, int len) {
  ...
  if (mprotect(startoffset - ((int)startoffset % getpagesize()),
               len + (len % getpagesize()), PROT_READ | PROT_EXEC) != 0) {
    perror("Sys_MemoryProtectExec: mprotect change memory to readonly/execute "
           "error");
    return qfalse;
  }
...
qboolean Sys_MemoryProtectReadonly(void* startoffset, int len) {
  ...
  if (mprotect(startoffset - ((int)startoffset % getpagesize()),
               len + (len % getpagesize()), PROT_READ) != 0) {
    perror(
        "Sys_MemoryProtectReadonly: mprotect change memory to readonly error");
    return qfalse;
  }
...
int main(int argc, char* argv[]) {
  ...
  if (uid == 0 ||
      uid !=
          geteuid()) { // warn user that he/she's operating as a privliged user
    Com_Printf("********************************************************\n");
...
qboolean Sys_SetPermissionsExec(const char* ospath) {
  if (chmod(ospath, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH |
                        S_IXOTH) != 0) {
    return qfalse;
  }
  return qtrue;
...
void Sys_SleepSec(int seconds) { sleep(seconds); }
...
int Sys_Backtrace(void** buffer, int size) { return backtrace(buffer, size); }
...
void* Sys_GetProcedure(const char* lpProcName) {
  if (currentLibHandle == NULL) {
    Com_Error(ERR_FATAL,
              "Attempt to get ProcAddress from invalid or not loaded library");
    return NULL;
  }
...
qboolean Sys_CreateNewThread(void* (*ThreadMain)(void*), threadid_t* tid,
                             void* arg) {
  int err;
...
qboolean __cdecl Sys_IsMainThread(void) { return Sys_ThreadisSame(mainthread); }
...
void Sys_ExitThread(int code) { pthread_exit(&code); }

void __attribute__((noreturn)) Sys_ExitForOS(int exitCode) { exit(exitCode); }

int Sys_Chmod(const char* file, int mode) { return chmod(file, mode); }
