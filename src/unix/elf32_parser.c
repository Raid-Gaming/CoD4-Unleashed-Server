...
char** ELF32_GetStrTable(void* buff, int imagelen, sharedlib_data_t* text) {
  ...
  if ((shdr = (Elf32_Shdr*)(buff + hdr->e_shoff)) >=
      (Elf32_Shdr*)(buff + imagelen - sizeof(Elf32_Shdr))) {
    return NULL;
  }
  if (hdr->e_shstrndx != SHN_UNDEF) {
    if (hdr->e_shstrndx !=
        SHN_XINDEX) { // Typical case, standard elf addressing
      if (hdr->e_shstrndx <= hdr->e_shnum)
        strtable = buff + shdr[hdr->e_shstrndx].sh_offset;
      else {
        Com_Printf("Error: the string table index is too big! String table "
                   "index: %d, section headers: %d.\n",
                   hdr->e_shstrndx, hdr->e_shnum);
        return NULL;
      }
    } else { // So called 'elf extended addressing'. Because 'simple' is too
             // mainstream.
  ...
  for (j = 0; j < hdr->e_shnum; ++j) {
    if (strcmp(&strtable[shdr[j].sh_name], ".text") == 0) {
      ...
      // Com_Printf("Debug: .text section found! Size: %d, address:
