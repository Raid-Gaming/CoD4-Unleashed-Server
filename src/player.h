...
typedef struct playerState_s {
  ...
  /*The ground entity's rotation will be added onto the player's view.  In
   * particular, this will cause the player's yaw to rotate around the entity's
   * z-axis instead of the world z-axis. Any rotation that the reference entity
   * undergoes will affect the player.
  ...
  // Animations as in mp/playeranim.script and animtrees/multiplayer.atr, it
  // also depends on mp/playeranimtypes.txt (the currently used weapon)
  int legsTimer;  // 136
  int legsAnim;   // 140
  int torsoTimer; // 144
  int torsoAnim;  // 148
...
typedef struct {
  ...
  //	int spawnObjectiveIndex;         // JPW NERVE index of objective to
  //spawn nearest to (returned from UI)
...
#define PICKUP_FORCE                                                           \
  2 // pickup the next item when touched (and reset to PICKUP_ACTIVATE when
    // done)
...
typedef struct usercmd_s { // Not Known
  ...
  byte weapon;
  byte offHandIndex;
  byte buttonsVertical;
  byte buttonsHorizontal;
  int field_18;
  int field_1C;
} usercmd_t;
...
typedef struct {
  ...
  int connectTime; // DHM - Nerve :: level.time the client first connected to
                   // the server  // N/A
...
#define LT_SPECIAL_PICKUP_MOD                                                  \
  3 // JPW NERVE # of times (minus one for modulo) LT must drop ammo before
    // scoring a point
...
struct gclient_s {
  ...
  // timers
  //	int respawnTime;                // can respawn when time > this, force
  //after g_forcerespwan
  int inactivityTime;         // 0x30cc kick players when time > this
  qboolean inactivityWarning; // 0x30d0 qtrue if the five second warning has
                              // been given
  ...
  /*	int timeResidual;
     ...
     inserted after this point
          ...
          int dropWeaponTime;         // JPW NERVE last time a weapon was
     dropped int limboDropWeapon;         // JPW NERVE weapon to drop in limbo
          int deployQueueNumber;         // JPW NERVE player order in
     reinforcement FIFO queue int sniperRifleFiredTime;         // JPW NERVE
     last time a sniper rifle was fired (for muzzle flip effects) float
     sniperRifleMuzzleYaw;       // JPW NERVE for time-dependent muzzle flip in
     multiplayer int lastBurnTime;         // JPW NERVE last time index for
     flamethrower burn int PCSpecialPickedUpCount;         // JPW NERVE used to
     count # of times somebody's picked up this LTs ammo (or medic health) (for
     scoring) int saved_persistant[MAX_PERSISTANT];           // DHM - Nerve ::
     Save ps->persistant here during Limbo
          ...
          gentity_t       *tempHead;  // Gordon: storing a temporary head for
     bullet head shot detection
