...
================
*/
void Com_MakeTimedEventArgCached(unsigned int index, unsigned int arg,
                                 unsigned int size) {
  ...
  if (arg >= MAX_TIMEDEVENTARGS)
    Com_Error(ERR_FATAL,
              "Com_MakeTimedEventArgCached: Bad function argument number. "
              "Allowed range is 0 - %d arguments",
              MAX_TIMEDEVENTARGS);
...
int QDECL Com_AddTimedEvent(int delay, void* function, unsigned int argcount,
  ...
  if (argcount > MAX_TIMEDEVENTARGS) {
    Com_Error(ERR_FATAL,
              "Com_AddTimedEvent: Bad number of function arguments. Allowed "
              "range is 0 - %d arguments",
              MAX_TIMEDEVENTARGS);
    return -1;
  }
...
void Com_TimedEventLoop(void) {
  ...
  while (qtrue) {
    ...
    if (evt->evFunction)
      evt->evFunction(evt->evArguments[0].arg, evt->evArguments[1].arg,
                      evt->evArguments[2].arg, evt->evArguments[3].arg,
                      evt->evArguments[4].arg, evt->evArguments[5].arg,
                      evt->evArguments[6].arg, evt->evArguments[7].arg);
...
=================
*/
static void Com_Crash_f(void) { *(int*)0 = 0x12345678; }
...
static void Com_InitCvars(void) {
  static const char* dedicatedEnum[] = {"listen server", "dedicated LAN server",
                                        "dedicated internet server", NULL};
  static const char* logfileEnum[] = {"disabled", "async file write",
                                      "sync file write", NULL};
  mvabuf;

  char* s;

  com_dedicated = Cvar_RegisterEnum("dedicated", dedicatedEnum, 2, CVAR_INIT,
                                    "True if this is a dedicated server");
  com_timescale = Cvar_RegisterFloat("timescale", 1.0, 0.0, 1000.0,
                                     CVAR_CHEAT | CVAR_SYSTEMINFO,
                                     "Scale time of each frame");
  com_fixedtime = Cvar_RegisterInt("fixedtime", 0, 0, 1000, 0x80,
                                   "Use a fixed time rate for each frame");
  com_maxFrameTime = Cvar_RegisterInt(
      "com_maxFrameTime", 100, 50, 1000, 0,
      "Time slows down if a frame takes longer than this many milliseconds");
  com_animCheck =
      Cvar_RegisterBool("com_animCheck", qfalse, 0, "Check anim tree");
  s = va("%s %s %s build %i %s", GAME_STRING, Q3_VERSION, PLATFORM_STRING,
         BUILD_NUMBER, __DATE__);

  com_version = Cvar_RegisterString("version", s, CVAR_ROM | CVAR_SERVERINFO,
                                    "Game version");
  com_shortversion =
      Cvar_RegisterString("shortversion", Q3_VERSION,
                          CVAR_ROM | CVAR_SERVERINFO, "Short game version");

  Cvar_RegisterString("build", va("%i", BUILD_NUMBER),
                      CVAR_ROM | CVAR_SERVERINFO, "");
  com_useFastfiles = Cvar_RegisterBool("useFastFiles", qtrue, 16,
  ...
  // AuthServerPort
  sv_webadmin =
      Cvar_RegisterBool("sv_webadmin", qtrue, 0, "Enable HTTP Web Admin");
  com_developer =
      Cvar_RegisterInt("developer", 0, 0, 2, 0, "Enable development options");
  com_developer_script = Cvar_RegisterBool("developer_script", qfalse, 16,
                                           "Enable developer script comments");
  com_logfile =
      Cvar_RegisterEnum("logfile", logfileEnum, 0, 0, "Write to logfile");
  com_sv_running =
      Cvar_RegisterBool("sv_running", qfalse, 64, "Server is running");
  com_securemodevar = Cvar_RegisterBool(
      "securemode", qfalse, CVAR_INIT,
      "CoD4 runs in secure mode which restricts execution of external "
      "scripts/programs and loading of unauthorized shared libraries/plugins. "
      "This is recommended in a shared hosting environment");
...
void Com_InitGamefunctions() {
  ...
  Scr_Settings(com_logfile->integer || com_developer->integer,
               com_developer_script->integer, com_developer->integer);
  ...
  *msg_dumpEnts = Cvar_RegisterBool("msg_dumpEnts", qfalse, CVAR_TEMP,
                                    "Print snapshot entity info");
  *msg_printEntityNums = Cvar_RegisterBool("msg_printEntityNums", qfalse,
                                           CVAR_TEMP, "Print entity numbers");
...
void Com_Init(char* commandLine) {
  ...
  Com_Printf("%s %s %s build %i %s\n", GAME_STRING, Q3_VERSION, PLATFORM_STRING,
             BUILD_NUMBER, __DATE__);
  ...
  Cbuf_Execute(
      0, 0); // Always execute after exec to prevent text buffer overflowing
...
time_t Com_GetRealtime() { return realtime; }

void Com_UpdateRealtime() { time(&realtime); }
...
__optimize3 void Com_Frame(void) {
  ...
  /*	else
     ...
     > 0) minMsec = 1000 / com_maxfpsUnfocused->integer; else
     if(com_maxfps->integer > 0) minMsec = 1000 / com_maxfps->integer; else
                          minMsec = 1;
...
qboolean Com_InError() { return com_errorEntered; }
...
void QDECL Com_Error(int code, const char* fmt, ...) {
  ...
  Cvar_RegisterInt("com_errorCode", code, code, code, CVAR_ROM,
                   "The last calling error code");
  ...
  if (code != ERR_DISCONNECT)
    Cvar_RegisterString("com_errorMessage", com_errorMessage, CVAR_ROM,
                        "The last calling error message");
