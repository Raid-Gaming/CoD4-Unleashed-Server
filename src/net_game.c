...
unsigned int NET_TimeGetTime() { return (unsigned int)com_frameTime; }
...
tcpclientstate_t NET_TCPAuthPacketEvent(netadr_t* from, byte* bufData, int len,
...
void NET_TCPPacketEvent(netadr_t* from, byte* bufData, int len,
                        int connectionId, int serviceId) {
  int i;
  msg_t msg;
...
void NET_TCPAddEventType(
  ...
  if (tcpevent == NULL || tcpauthevent == NULL) {
    Com_Error(ERR_FATAL, "NET_TCPAddEventType: NULL tcpevent handler or NULL "
                         "tcpauthevent handler");
    return;
  }

  for (i = 0; i < MAX_TCPEVENTS; i++) {
    if (tcpevents[i].tcpevent == tcpevent) {
      Com_Error(ERR_FATAL, "NET_TCPAddEventType: Attempt to add an already "
  ...
  Com_Error(ERR_FATAL, "NET_TCPAddEventType: Out of redirect handles. Increase "
                       "MAX_TCPEVENTS to add more redirect destinations");
}
