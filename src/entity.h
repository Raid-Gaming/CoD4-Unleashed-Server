...
typedef struct entityState_s { // Confirmed names and offsets but not types
  ...
  int solid; // 0x98 for client side prediction, trap_linkentity sets this
             // properly	0x98
...
typedef struct {
  // entityState_t	s;				//Duplicated struct is
  // removed
  ...
  vec3_t absmin,
      absmax; // 0x124  //0x130 derived from mins/maxs and origin + rotation

  // currentOrigin will be used for all collision detection and world linking.
  // it will not necessarily be the same as the trajectory evaluation for the
  // current time, because each entity must be moved one at a time after time is
  // advanced to avoid simultanious collision issues
...
struct gentity_s {
  ...
  /*
          ...
          qboolean neverFree;             // if true, FreeEntity will only
     unlink
                                                                          //
     bodyque uses this
          ...
          char        *model;
          char        *model2;
          int freetime;                   // level.time when the object was
     freed

          int eventTime;                  // events will be cleared
     EVENT_VALID_MSEC after set qboolean freeAfterEvent; qboolean
     unlinkAfterEvent;

          qboolean physicsObject;         // if true, it can be pushed by movers
     and fall off edges
                                                                          // all
     game items are physicsObjects, float physicsBounce;            // 1.0 =
     continuous bounce, 0.0 = no bounce int clipmask;                   //
     ...
     players, for instance
          ...
          float speed;
          float closespeed;           // for movers that close at a different
     speed than they open vec3_t movedir;
     ...
     hitting endpoint void ( *blocked )( gentity_t *self, gentity_t *other );
          void ( *touch )( gentity_t *self, gentity_t *other, trace_t *trace );
          void ( *use )( gentity_t *self, gentity_t *other, gentity_t *activator
     ); void ( *pain )( gentity_t *self, gentity_t *attacker, int damage, vec3_t
     point ); void ( *die )( gentity_t *self, gentity_t *inflictor, gentity_t
     *attacker, int damage, int mod );
          ...
          int damage;
          int splashDamage;           // quad will increase this without
     increasing radius int splashRadius; int methodOfDeath; int
     splashMethodOfDeath;
          ...
          qboolean aiInactive;
          int aiCharacter;            // the index of the type of character we
     are (from aicast_soldier.c)
          // done.
          ...
          // Ridah
          int missionLevel;               // mission we are currently trying to
     complete
                                                                          //
     gets reset each new level
          ...
          int grenadeExplodeTime;         // we've caught a grenade, which was
     due to explode at this time int grenadeFired;               // the grenade
     entity we last fired

          int mg42ClampTime;              // time to wait before an AI decides
     to ditch the mg42
     ...
     perform for each event type g_script_status_t scriptStatus;     // current
     status of scripting
          ...
          //
     -------------------------------------------------------------------------------------------
          // if working on a post release patch, new variables should ONLY be
     inserted after this point
          // DHM - Nerve :: the above warning does not really apply to MP, but
     I'll follow it for good measure

          int voiceChatSquelch;                   // DHM - Nerve
          int voiceChatPreviousTime;              // DHM - Nerve
          int lastBurnedFrameNumber;              // JPW - Nerve   : to fix FT
     instant-kill exploit*/
}; // Size: 0x274

/***************** Verified *******************************/

#define GENTITYNUM_BITS                                                        \
  10 // JPW NERVE put q3ta default back for testing	// don't need to send
     // any more
...
gentity_t* G_Spawn();
void G_SpawnHelicopter(gentity_t* vehent, gentity_t* ownerent, const char* type,
                       const char* model);
