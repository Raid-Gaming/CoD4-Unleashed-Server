...
void SV_ReliableSendRedirect(char* sendbuf, qboolean lastcommand) {
  ...
  for (; remaining > 0;) { // We have to split the string into smaller packages
                           // of max 240 bytes
    // This function tries to ensure that every package ends on the last
    // possible linebreak for better formating
    maxlength = remaining;
    ...
    for (lastlinebreak = 0, i = 0; i < maxlength; i++) {
      ...
      if (outputbuf[i] == '"')
        outputbuf[i] = 0x27; // replace all " with ' because no " are accepted
      if (outputbuf[i] == 0x0a)
        lastlinebreak = i; // search for the last linebreak
    }
    if (lastlinebreak > 0) {
      i = lastlinebreak; // found a linebreak and send everything until that
                         // position
      remaining -= i - 1;
      sendbuf += i + 1;
      outputbuf[i + 1] = 0x00;
...
qboolean SV_ExecuteRemoteCmd(int clientnum, const char* msg) {
  ...
  if (powercmd == -1) {
    SV_SendServerCommand(
        redirectClient,
        "e \"^5Command^2: %s\n^3Command execution failed - Invalid command "
        "invoked - Type ^2$cmdlist ^3to get a list of all available commands\"",
        printPtr);
    return qfalse;
  }
  if (powercmd > power) {
    SV_SendServerCommand(
        redirectClient,
        "e \"^5Command^2: %s\n^3Command execution failed - Insufficient power "
        "to execute this command.\n^3You need at least ^6%i ^3powerpoints to "
        "invoke this command.\n^3Type ^2$cmdlist ^3to get a list of all "
        "available commands\"",
        printPtr, powercmd);
    return qtrue;
  }
  Com_Printf(
      "Command execution: %s   Invoked by: %s   InvokerUID: %i Power: %i\n",
...
void QDECL SV_PrintAdministrativeLog(const char* fmt, ...) {
  ...
  Com_sprintf(msg, sizeof(msg), "%s - Admin %i with %i power %s\n", ltime,
              Cmd_GetInvokerUID(), Cmd_GetInvokerPower(), inputmsg);
