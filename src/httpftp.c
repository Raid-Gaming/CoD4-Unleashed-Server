...
static int FT_SendData(ftRequest_t* request) {
  ...
  bytes = NET_TcpSendData(request->socket, request->sendmsg.data,
                          request->sendmsg.cursize);
  ...
  memmove(request->sendmsg.data, &request->sendmsg.data[bytes],
          request->sendmsg.cursize);
  return 1;
}
...
void HTTP_BuildNewRequest(ftRequest_t* request, const char* method, msg_t* msg,
  ...
  if (msg != NULL) {
    Com_sprintf(extheaderfields, sizeof(extheaderfields),
                "Content-Length: %d\r\n", msg->cursize);
...
static void HTTPSplitURL(const char* url, char* address, int lenaddress,
                         char* wwwpath, int lenwwwpath) {
  char* charloc;
...
int HTTP_SendReceiveData(ftRequest_t* request) {
  ...
  if (request->finallen == -1) {
    ...
    while ((line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf,
                                      sizeof(stringlinebuf))) &&
           line[0] != '\0') {
      if (line[0] == '\r') {
        gotheader = qtrue;
        break;
    ...
    line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf,
                              sizeof(stringlinebuf));
    if (Q_stricmpn(line, "HTTP/1.", 7) || isInteger(line + 7, 2) == qfalse ||
        isInteger(line + 9, 4) == qfalse) {
    ...
    if (request->version < 0 || request->version > 9) {
      Com_PrintError(
          "HTTP_ReceiveData: Packet has unknown HTTP version 1.%d !\n",
          request->version);
    ...
    while ((line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf,
      ...
      if (!Q_stricmpn("Content-Length:", line, 15)) {
        ...
        if (strlen(line + 9) > 8) {
          ...
          HTTPSplitURL(line, request->address, sizeof(request->address),
...
ftRequest_t* HTTPRequest(const char* url, const char* method, msg_t* msg,
                         const char* addheaderlines) {
...
static void FTP_SplitURL(const char* url, char* address, int lenaddress,
                         char* path, int lenpath, char* loginname,
                         int loginnamelen, char* password, int passwdlen) {
  char* charloc;
...
static ftRequest_t* FTP_DLRequest(const char* url) {
  ...
  FTP_SplitURL(url, address, sizeof(address), ftppath, sizeof(ftppath), user,
...
static void FTP_GetExtendedPassiveAddress(const char* line, const char* address,
                                          netadr_t* adr) {
  int i, j, port;
  ...
  if (NET_StringToAdr(address, adr, NA_UNSPEC) ==
      0) { /* No valid address string. Must not happen.  */
    return;
  }
...
static int FTP_SendReceiveData(ftRequest_t* request) {
  ...
  while ((line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf,
    ...
    case 202:
      if (request->stage < 21 && request->stage > 10) {
        ...
        Com_Printf("\nFTP_SendReceiveData: Received unexpected response: %s - "
                   "Will abort!\n",
                   line);
        request->stage = -20;
      }
    ...
    case 331:
      if (request->stage == 10) {
        ...
        Com_Printf("FTP_SendReceiveData: Received unexpected response: %s - "
                   "Will abort!\n",
                   line);
        request->stage = -20;
      }
      ...
      if (request->stage == 36) {
        ...
        if (pasvadr.type == NA_IP) {
          ...
          request->transfersocket =
              NET_TcpClientConnect(NET_AdrToString(&pasvadr));
          if (request->transfersocket < 0) {
            request->transfersocket = -1;
            return -1;
          }
          ...
          Com_PrintWarning("FTP_SendReceiveData: Couldn't read the "
                           "address/port of passive mode response\n");
          return -1;
        }
    ...
    case 229:
      if (request->stage == 32) {
        ...
        if (pasvadr.type == NA_IP || pasvadr.type == NA_IP6) {
          Com_DPrintf(
              "FTP_SendReceiveData: Entering Extended Passive Mode at %s OK\n",
              NET_AdrToString(&pasvadr));
          request->transfersocket =
              NET_TcpClientConnect(NET_AdrToString(&pasvadr));
          if (request->transfersocket < 0) {
    ...
    case 213:
      if (request->stage == 50) {
        ...
        if (bytes < 1 || bytes > 1024 * 1024 * 1024) {
          Com_PrintWarning(
              "FTP_SendReceiveData: Requested file exceeds %d bytes.\n",
              1024 * 1024 * 1024);
        }
        ...
        buf = Z_Malloc(INITIAL_BUFFERLEN + 1);
        if (buf == NULL) {
          Com_PrintWarning("FTP_SendReceiveData: Failed to allocate %d bytes "
                           "for download file!\n",
                           bytes);
          return -1;
        }
    ...
    case 221:
      if (request->stage < 10000) {
        Com_Printf("\nThe FTP server closed the control connection before the "
    ...
    case 231:
      if (request->stage < 10000) {
        Com_Printf("\nThe FTP server logged us out before the transfer was "
                   "completed!\n");
        request->stage = -20;
      }
      break;
    case 350:
      if (request->stage < 10000) {
        Com_Printf("\nThe FTP server returned \'%s\' before the transfer was "
    ...
    case 503:
      if (request->stage == 32) {
        request->stage = 35;
        Com_DPrintf("FTP_SendReceiveData: Command EPSV is not implemented on "
                    "FTP server. Trying PASV...\n");
        break;
      } else if (request->stage == 36) {
        Com_Printf("FTP_SendReceiveData: FTP Server does not support passive "
                   "mode. Request failed!\n");
        request->stage = -10;
      }
    default:
  ...
  case 9999:
    ...
    if (request->transfertotalreceivedbytes == request->finallen) {
      ...
      Com_Printf("\nThe FTP server closed the data connection before the "
                 "transfer was completed!\n");
      Com_Printf("Expected %d bytes but got %d bytes\n", request->finallen,
                 request->transfertotalreceivedbytes);
      request->stage = -20;
      break;
    }
  case -20:
...
int HTTPServer_ReadMessage(netadr_t* from, msg_t* msg, ftRequest_t* request) {
  ...
  Com_Memcpy(&request->recvmsg.data[request->recvmsg.cursize], msg->data,
             msg->cursize);
  ...
  if (request->finallen == -1) {
    ...
    while ((line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf,
      ...
      if (!Q_stricmpn("Content-Length:", line, 15)) {
        ...
        if (line[13] == ' ') {
          Q_strncpyz(request->contentType, &line[14],
                     sizeof(request->contentType));
        } else {
          Q_strncpyz(request->contentType, &line[13],
                     sizeof(request->contentType));
        }
  ...
  //	Com_Printf("^6Received: Version: HTTP/1.%d   Method: %d Content-Length:
  //%d   URL: %s\n", request->version, request->mode, request->contentLength,
  //request->url);
  return 1;
}

void HTTPServer_BuildMessage(ftRequest_t* request, char* status, char* message,
                             int len, char* sessionkey) {
  int headerlen;
  byte* newbuf;
...
qboolean HTTPServer_WriteMessage(ftRequest_t* request, netadr_t* from) {
  ...
  bytes =
      NET_TcpSendData(from->sock, request->sendmsg.data + request->sentBytes,
                      request->sendmsg.cursize - request->sentBytes);
...
void HTTPServer_BuildResponse(ftRequest_t* request, char* sessionkey,
  ...
  if (hasmessage) {
    HTTPServer_BuildMessage(request, "200 OK", (char*)msg.data, msg.cursize,
                            sessionkey);
    Z_Free(msg.data);
    return;
  }
  HTTPServer_BuildMessage(request, "403 FORBIDDEN", "Error: Forbidden",
                          strlen("Error: Forbidden"), sessionkey);
}
...
/* Detecting the clientside protocol and process the 1st chunk of data if it is
 * a http client */
tcpclientstate_t HTTPServer_AuthEvent(netadr_t* from, msg_t* msg,
                                      int* connectionId) {
  ftRequest_t* request;
  *connectionId = 0;
  ...
  if (line != NULL) {
    if (Q_strncmp(line, "GET", 3) && Q_strncmp(line, "POST", 4) &&
        Q_strncmp(line, "HEAD", 4)) {
...
void HTTPServer_Init() {
  ...
  /* Register the events */
  NET_TCPAddEventType(HTTPServer_Event, HTTPServer_AuthEvent,
...
/*
 * Function to open a new request by an valid URL starting with ftp:// or
 * http:// Return codes: NULL = failed, Otherwise a valid handle. A valid handle
 * must be always freed when action is completed. It also must be freed in error
 * cases
 */
...
/*
 * This function must get called periodically (For example a loop) as long as it
 * returns 0 to do the transfer actions Return codes: 0 = not complete yet; 1 =
 * complete; -1 = failed
 */
...
const char* FileDownloadGenerateProgress(ftRequest_t* request) {
  ...
  if (request->finallen > 0 && request->totalreceivedbytes > 0)
    percent = 100.0f *
              ((float)request->totalreceivedbytes / (float)request->finallen);
  else
    percent = 0.0f;
  ...
  if (elapsedmsec != 0) {
    rate = (((float)request->totalreceivedbytes / 1024.0f) * 1000.0f) /
           (float)elapsedmsec;
  } else {
    rate = 0;
  }
  ...
  Com_sprintf(line, sizeof(line),
              "\rReceiving: %s %.1f/%.1f kBytes @ %.1f kB/s", bar,
              (float)request->totalreceivedbytes / 1024.0f,
              (float)request->finallen / 1024.0f, rate);
...
void DL_Test_CB(const char* filename, qboolean success, ftRequest_t *handle)
{
        ...
        if(success == qfalse)
        {
                ...
                FS_WriteFile(filename, handle->recvmsg.data,
handle->recvmsg.cursize); Com_Printf("Filewrite took %d msec",
Sys_Milliseconds() - before);
        }
...
void DL_File(char* url)
{
        ftRequest_t* handle;
        //handle = DownloadFile(
"http://iceops.in/demos/hardcore_tdm/demo_215216_0000.dm_1" );
        //handle =
DownloadFile("ftp://ftp.idsoftware.com/idstuff/teamarena/q3ta_trailer.mpg");

        handle = DownloadFile(url);
        if(handle != NULL)
                Sys_SetupThreadCallback(DL_Test_CB, "dlfilename.gz", qtrue,
handle); else Sys_SetupThreadCallback(DL_Test_CB, "dlfilename.gz", qfalse,
handle);
...
{
        ftRequest_t* handle;
        void (*downloadfinishedcb)(const char* filename, qboolean success,
ftRequest_t* handle); char url[MAX_STRING_CHARS]; char path[MAX_QPATH];
}fileDownload_t;

fileDownload_t filedownloads[MAX_FILEDOWNLOAD_HANDLES];

qboolean Com_AddDownload(const char* url, const char* savepath, void
(*downloadfinishedcb)(const char* filename, qboolean success, ftRequest_t*
handle))
{
        int i;
        ...
        for( i = 0; i < MAX_FILEDOWNLOAD_HANDLES; i++ )
        {
                ...
                if(filedownloads[i].downloadfinishedcb == NULL)
                {
                        ...
                        filedownloads[i].downloadfinishedcb =
downloadfinishedcb; Q_strncpyz(filedownloads[i].path, savepath,
sizeof(filedownloads[i].path)); Q_strncpyz(filedownloads[i].url, url,
sizeof(filedownloads[i].url)); return qtrue;
                }
...
void Com_ProcessRunningDownloads()
{
        ...
        for( i = 0; i < MAX_FILEDOWNLOAD_HANDLES; i++)
        {
                ...
                if(filedownloads[i].handle == NULL)
                {
                        ...
                        if(handle == NULL)
                        {
                                filedownloads[i].downloadfinishedcb(filedownloads[i].path,
qfalse, filedownloads[i].handle); filedownloads[i].downloadfinishedcb = NULL;
                                memmove( &filedownloads[i], &filedownloads[i
+1], MAX_FILEDOWNLOAD_HANDLES - (i +1) ); }else { filedownloads[i].handle =
handle;
                        }
                        continue;
                }
                ...
                {
                        filedownloads[i].downloadfinishedcb(filedownloads[i].path,
qfalse, filedownloads[i].handle); }else {
                        filedownloads[i].downloadfinishedcb(filedownloads[i].path,
qtrue, filedownloads[i].handle);
                }
                ...
                memmove( &filedownloads[i], &filedownloads[i +1],
MAX_FILEDOWNLOAD_HANDLES - (i +1) );
