...
// Don't ban IPs for more than MAX_IPBAN_MINUTES minutes as they can be shared
// (Carrier-grade NAT)
...
typedef struct { // It is only for timelimited tempbans to prevent happy
                 // reconnecting and sitting in server for 2 minutes until player
                 // is detected or keep to advertise with playernick while
                 // reconnecting
  netadr_t remote;
  char banmsg[128];
...
qboolean SV_OversizeBanlistAlign() {
  ...
  if (current_banlist_size <=
      (current_banindex + 1) * sizeof(banList_t)) { // Memory extension
    ...
    if (new_blist) {
      ...
      Com_PrintError("Could not allocate enougth memory to extend the size of "
                     "banlist. Failed to add new bans\n");
      return qfalse;
    }
  }
  return qtrue;
  ...
  if (playeruid) {
    for (i = 0; i < current_banindex; this ++, i++) {
      if (this->playeruid == playeruid) {
        Com_Printf("Error: This player with UID: %i is already banned on this "
                   "server (line: %d)\n",
    ...
    for (i = 0; i < current_banindex; this ++, i++) {
      if (!Q_stricmp(this->pbguid, guid)) {
        Com_Printf("Error: This player with GUID: %s is already banned on this "
                   "server (line: %d)\n",
                   guid, linenumber);
        return qfalse;
      }
...
void SV_LoadBanlist() {
  ...
  for (i = 0, error = 0; error < 32; i++) {
    ...
    if (!SV_ParseBanlist(buf, aclock, i + 1))
      error++; // Executes the function given as argument in execute
  }
...
char* SV_PlayerBannedByip(netadr_t* netadr, char* message,
                          int len) { // Gets called in SV_DirectConnect
  ipBanList_t* this;
  int i;
  ...
  if (sv_banappealurl && sv_banappealurl->string[0]) {
    Com_sprintf(appealmsg, sizeof(appealmsg),
                "You can appeal this ban online at: %s",
  ...
  for (this = &ipBans[0], i = 0; i < MAX_IPBANS; this ++, i++) {
    ...
    if (NET_CompareBaseAdr(netadr, &this->remote)) {
      ...
      if (Com_GetRealtime() < this->timeout) {
        if (this->uid > 0) {
          Com_sprintf(uidguidmsg, sizeof(uidguidmsg), "Your UID is: %i",
                      this->uid);
        } else {
          Com_sprintf(uidguidmsg, sizeof(uidguidmsg), "Your GUID is: %s",
                      this->guid);
        }
        ...
        if (this->expire == -1) {
          Com_sprintf(message, len,
                      ...
                      "from this server\nYou will be never allowed to join "
                      "this gameserver again\n %s    Banning admin UID is: "
                      "%i\nReason for this ban:\n%s\n%s\n",
          ...
          int remaining = (int)(this->expire - Com_GetRealtime()) +
                          1; // in seconds (+1 for fixing up a display error
                             // when only some seconds are remaining)
          int srem = remaining;
          ...
          if (sv_banappealminhours &&
              srem < sv_banappealminhours->integer * 60 * 60) {
            appealmsg[0] = '\0';
          }
          ...
          Com_sprintf(message, len,
                      ...
                      "days %i hours %i minutes\n %s    Banning admin UID is: "
                      "%i\nReason for this ban:\n%s\n%s\n",
                      d, h, m, uidguidmsg, this->adminuid, this->banmsg,
                      appealmsg);
          return message;
        }
      }
...
void SV_PlayerAddBanByip(
  ...
  for (list = &ipBans[0], i = 0; i < MAX_IPBANS; list++,
      i++) { // At first check whether we have already an entry for this player
  ...
  if (duration > ipbantime->integer * 60 || expire == -1)
    duration = ipbantime->integer *
               60; // Don't ban IPs for more than MAX_IPBAN_MINUTES minutes as
                   // they can be shared (Carrier-grade NAT)
...
char* SV_PlayerIsBanned(int uid, char* pbguid, netadr_t* addr, char* message,
                        int len) {
    ...
    for (i = 0; i < current_banindex; this ++, i++) {
      ...
      if (this->playeruid == uid) {
        ...
        if (this->expire == (time_t)-1) {
          SV_PlayerAddBanByip(addr, this->reason, this->playeruid, pbguid,
                              this->adminuid, -1);
          Com_sprintf(message, len,
                      "\nEnforcing prior ban\nYou have been permanently banned "
      ...
      if (!Q_strncmp(this->pbguid, &pbguid[24], 8)) {
        ...
        if (this->expire == (time_t)-1) {
          Com_sprintf(message, len,
                      ...
                      "will be never allowed to join this gameserver again\n "
                      "Your GUID is: %s\nReason for this ban:\n%s\n%s\n",
...
void SV_InitBanlist() {
  ...
  banlistfile =
      Cvar_RegisterString("banlist_filename", "banlist.dat", CVAR_INIT,
                          "Name of the file which holds the banlist");
  ipbantime = Cvar_RegisterInt(
      "banlist_maxipbantime", MAX_DEFAULT_IPBAN_MINUTES, 0, 20160, 0,
      "Limit of minutes to keep a ban against an ip-address up");
  sv_banappealurl = Cvar_RegisterString("banlist_appealurl", "", 0,
                                        "Showing the url for ban appeal");
  sv_banappealminhours = Cvar_RegisterInt(
      "banlist_appealminhours", DEFAULT_APPEAL_MINHOURS, 0, 336, 0,
      "How much hours have to be left for showing an appeal url");
  ...
  if (banlist) {
    ...
    Com_PrintError(
        "Failed to allocate memory for the banlist. Banlist is disabled\n");
  }
}
...
qboolean SV_AddBan(int uid, int auid, char* guid, char* name, time_t expire,
                   char* banreason) {
...
qboolean SV_RemoveBan(int uid, char* guid, char* name) {
  ...
  for (i = 0; i < current_banindex; this ++, i++) {
    ...
    SV_PrintAdministrativeLog(
        "Removing ban for Nick: %s, UID: %i, GUID: %s, Banreason: %s\n",
        printnick, this->playeruid, printguid, banreason);
  }
  if (succ)
    SV_WriteBanlist();
...
void SV_DumpBanlist() {
  ...
  for (i = 0, k = 0; i < current_banindex; this ++, i++) {
    ...
    if (this->expire == (time_t)-1 || this->expire > aclock) {
      ...
      Com_Printf("%i uid: %i; nick: %s; guid: %s; adminuid: %i; expire: %s; "
                 "reason: %s\n",
                 i, this->playeruid, this->playername, this->pbguid,
                 this->adminuid, timestr, this->reason);
    }
  }
