...
qboolean __cdecl Sys_IsDatabaseThread(void) { return qfalse; }
...
void Sys_EnterCriticalSection(int section) {
  ...
  if (section > CRIT_ERRORCHECK)
    Sys_Print(
        va("^6Sys_EnterCriticalSection for Thread: %d Section: %d Depth: %d\n",
           Sys_GetCurrentThreadId(), section, mutex_depth[section]));
  mutex_depth[section]++;
#endif
  /*	if(Com_InError() && section != CRIT_ERROR && Sys_IsMainThread() ==
     qtrue)
          {
...
void Sys_RunThreadCallbacks() {
  ...
  for (tcb = thread_callbacks, i = 0; i < MAX_CALLBACKS; i++) {
    ...
    if (thread_callbacks[i].callbackMain != NULL)
      thread_callbacks[i].callbackMain(
          tcb->callback_args[0], tcb->callback_args[1], tcb->callback_args[2],
          tcb->callback_args[3], tcb->callback_args[4], tcb->callback_args[5],
          tcb->callback_args[6], tcb->callback_args[7]);
...
void* Sys_CbThreadStub(void* arg) {
  ...
  tcb->threadMain(tcb->thread_args[0], tcb->thread_args[1], tcb->thread_args[2],
                  tcb->thread_args[3], tcb->thread_args[4], tcb->thread_args[5],
                  tcb->thread_args[6], tcb->thread_args[7]); // real main-thread
  tcb->isActive = qtrue;
  return NULL;
}
...
qboolean Sys_SetupThreadCallback(void* callbackMain, ...) {
  ...
  if (Sys_IsMainThread() == qtrue) {
    Com_Error(ERR_FATAL, "Sys_SetupThreadCallback: Can not call this function "
...
qboolean Sys_CreateCallbackThread(void* threadMain, ...) {
  ...
  if (Sys_IsMainThread() == qfalse) {
    Com_Error(ERR_FATAL, "Sys_CreateCallbackThread: Can only call this "
