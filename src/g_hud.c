...
void G_HudSetColor(game_hudelem_t* element, ucolor_t color,
                   ucolor_t glowcolor) {
...
void G_HudSetFont(game_hudelem_t* element, float fontscale,
                  fonttype_t fonttype) {
...
void G_HudSetMovingOverTime(game_hudelem_t* element, int time, float newx,
                            float newy) {

  if (time > 60000 || time < 0) {
    Com_PrintWarning("G_HudSetMovingOverTime: time: %i is out of range. Range "
                     "is 0 to 60000\n",
                     time);
    time = 0;
  }
...
void G_HudSetFadingOverTime(game_hudelem_t* element, int time,
                            ucolor_t newcolor) {

  if (time > 60000 || time < 0) {
    Com_PrintWarning("G_HudSetFadeingOverTime: time: %i is out of range. Range "
                     "is 0 to 60000\n",
                     time);
    time = 0;
  }
