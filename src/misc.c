...
qboolean __cdecl Com_LoadDvarsFromBuffer(const char** inputbuf,
                                         unsigned int length,
                                         const char* data_p,
                                         const char* filename) {
  ...
  while ((cvar_name = Com_Parse(&data_p)) && cvar_name[0]) {
    ...
    if (i == length) {
      ...
      if (com_developer && com_developer->integer) {
        ...
        if ((!Q_strncmp(inputbuf[i], "bg_shock_viewKickPeriod", 23) ||
             !Q_strncmp(inputbuf[i], "bg_shock_viewKickFadeTime", 25)) &&
            (line[0] == '0' && line[1] == '\0')) {
          /* Quite this spam */
        } else {
          Cvar_Set(inputbuf[i], line);
