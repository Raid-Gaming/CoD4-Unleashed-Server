...
cvar_t	*sv_zombietime;			// seconds to sink messages after
disconnect
...
to 99999 (bug #491) #ifndef STANDALONE cvar_t	*sv_strictAuth; #endif
...
void SV_DumpReliableCommands( client_t *client, const char* cmd) {
        ...
        Com_sprintf(msg, sizeof(msg), "Cl: %i, Seq: %i, Time: %i, NotAck: %i,
Len: %i, Msg: %s\n", client - svs.clients, client->reliableSequence, svs.time
,client->reliableSequence - client->reliableAcknowledge, strlen(cmd), cmd);
    ...
    // TTimo: only open the qconsole.log if the filesystem is in an initialized
state
    //   also, avoid recursing in the qconsole.log opening (i.e. if fs_debug is
on) if ( !reliabledump && FS_Initialized()) { struct tm *newtime; time_t aclock;
...
void sub_5310E0(client_t* client) {
  ...
  for (i = client->reliableSent + 1; i <= client->reliableSequence; ++i) {
    if (client->reliableCommands[i & (MAX_RELIABLE_COMMANDS - 1)].cmdType) {
      if ((v1 & (MAX_RELIABLE_COMMANDS - 1)) !=
          (i & (MAX_RELIABLE_COMMANDS - 1))) {
        memcpy(&client->reliableCommands[v1 & (MAX_RELIABLE_COMMANDS - 1)],
...
int sub_530FC0(client_t* client, const char* command) {
  ...
  for (i = client->reliableSent + 1; i <= client->reliableSequence; ++i) {
    ...
    if (client->reliableCommands[i & (MAX_RELIABLE_COMMANDS - 1)].command[0] !=
        command[0])
      continue;
...
void __cdecl SV_AddServerCommand(client_t* client, int type, const char* cmd) {
  ...
  if (v4 < 0) {
    ...
    for (i = v4 + 1; i <= client->reliableSequence; ++v4) {
      memcpy(&client->reliableCommands[v4 & 0x7F],
             &client->reliableCommands[i++ & 0x7F], sizeof(reliableCommands_t));
    }
  }
  ...
  if (client->reliableSequence - client->reliableAcknowledge ==
    ...
    for (j = client->reliableAcknowledge + 1; j <= client->reliableSequence;
         ++j) {
      Com_PrintNoRedirect(
          "cmd %5d: %8d: %s\n", j,
          client->reliableCommands[j & (MAX_RELIABLE_COMMANDS - 1)].cmdTime,
          &client->reliableCommands[j & (MAX_RELIABLE_COMMANDS - 1)].command);
    }
    ...
    type = 1;
    Com_sprintf(string, sizeof(string), "%c \"EXE_SERVERCOMMANDOVERFLOW\"",
                119);
    cmd = string;
  }
  ...
  MSG_WriteReliableCommandToBuffer(
      cmd, client->reliableCommands[index].command,
      sizeof(client->reliableCommands[index].command));
...
static void SVC_RateLimitInit() {
  ...
  int totalsize = querylimit.max_buckets * sizeof(leakyBucket_t) +
                  querylimit.max_hashes * sizeof(leakyBucket_t*);

  querylimit.buckets = Z_Malloc(totalsize);

  if (!querylimit.buckets) {
    Com_PrintError(
        "QUERY LIMIT: System is out of memory. All queries are disabled\n");
    querylimit.queryLimitsEnabled = -1;
  }

  querylimit.bucketHashes =
      (leakyBucket_t**)&querylimit.buckets[querylimit.max_buckets];
...
================
*/
__optimize3 __regparm3 static leakyBucket_t*
...
================
*/
__optimize3 __regparm3 static qboolean
...
__optimize3 __regparm1 void SVC_Status(netadr_t* from) {
  ...
  if (SVC_RateLimit(&querylimit.statusBucket, 20, 20000)) {
    //	Com_DPrintf( "SVC_Status: overall rate limit exceeded, dropping
    //request\n" );
    return;
  }
  ...
  if (sv_authorizemode->integer == 1) // Backward compatibility
    ...
    Info_SetValueForKey(infostring, "type",
                        va("%i", sv_authorizemode->integer));
  // add "demo" to the sv_keywords if restricted

  status[0] = 0;
  statusLength = 0;

  for (i = 0, gclient = level.clients; i < sv_maxclients->integer;
       i++, gclient++) {
    cl = &svs.clients[i];
...
__optimize3 __regparm1 void SVC_Info(netadr_t* from) {
  ...
  if (SVC_RateLimitAddress(from, 4, sv_queryIgnoreTime->integer * 1000)) {
    //	Com_DPrintf( "SVC_Info: rate limit from %s exceeded, dropping
    //request\n", NET_AdrToString( *from ) );
    return;
  }
  infostring[0] = 0;
  ...
  Info_SetValueForKey(infostring, "ff",
                      va("%d", Cvar_VariableIntegerValue("scr_team_fftype")));
  ...
  if (sv_maxPing->integer) {
    Info_SetValueForKey(infostring, "sv_maxPing",
...
void SVC_SourceEngineQuery_Info(netadr_t* from, const char* challengeStr,
  ...
  if (!masterserver) {
    ...
    // excess outbound bandwidth usage when being flooded inbound
    if (SVC_RateLimit(&querylimit.infoBucket, 100, 100000)) {
      //	Com_DPrintf( "SVC_Info: overall rate limit exceeded, dropping
      //request\n" );
      return;
    }
...
void SVC_SourceEngineQuery_SendSplitMessage(netadr_t* from, msg_t* longmsg) {
  ...
  for (i = 0; i < numpackets; i++) {
    ...
    if (longmsg->cursize - longmsg->readcount > SPLIT_SIZE) {
      Com_Memcpy(&msg.data[msg.cursize], &longmsg->data[longmsg->readcount],
                 SPLIT_SIZE);
      longmsg->readcount += SPLIT_SIZE;
      msg.cursize += SPLIT_SIZE;
    } else {
      Com_Memcpy(&msg.data[msg.cursize], &longmsg->data[longmsg->readcount],
                 longmsg->cursize - longmsg->readcount);
...
void SVC_SourceEngineQuery_Player(netadr_t* from, msg_t* recvmsg) {
  ...
  for (i = 0, cl = svs.clients, gclient = level.clients, numClients = 0;
       i < sv_maxclients->integer; i++, gclient++, cl++) {
    ...
    if (cl->state >= CS_CONNECTED) {
      ...
      MSG_WriteFloat(&playermsg,
                     ((float)(svs.time - cl->connectedTime)) / 1000);
      numClients++;
    }
  }
...
===============
*/
__optimize3 __regparm2 static void SVC_RemoteCommand(netadr_t* from,
                                                     msg_t* msg) {
  ...
  if (strcmp(SV_Cmd_Argv(1), sv_rconPassword->string)) {
    ...
    if (SVC_RateLimit(&querylimit.rconBucket, 1, 100)) {
      //	Com_DPrintf( "SVC_RemoteCommand: rate limit exceeded for bad
      //rcon\n" );
      return;
    }
  ...
  if (strlen(sv_rconPassword->string) < 8) {
    Com_BeginRedirect(sv_outputbuf, SV_OUTPUTBUF_LENGTH, SV_FlushRedirect);
    Com_Printf("No rconpassword set on server or password is shorter than 8 "
               "characters.\n");
    Com_EndRedirect();
    return;
  }
...
typedef enum { UPDCONN_CHALLENGING, UPDCONN_CONNECT } update_connState_t;
...
void SV_UpdateProxyUpdateBadChallenge(netadr_t* from) {
  ...
  if (!NET_CompareAdr(from, &update_connection.updateserveradr)) {
    Com_Printf(
        "SV_UpdateProxyUpdateBadChallenge: Packet not from updateserver\n");
    return;
  }
...
void SV_UpdateProxyChallengeResponse(netadr_t* from) {
  ...
  if (!NET_CompareAdr(from, &update_connection.updateserveradr)) {
    Com_Printf(
        "SV_UpdateProxyChallengeResponse: Packet not from updateserver\n");
    return;
  }
...
void SV_UpdateProxyConnectResponse(netadr_t* from) {
  ...
  if (!NET_CompareAdr(from, &update_connection.updateserveradr)) {
    //        Com_Printf("SV_UpdateProxyConnectResponse: Packet not from
    //        updateserver\n");
    return;
  }
  ...
  for (cl = svs.clients, i = 0; i < sv_maxclients->integer; ++i, ++cl) {
    if (cl->state == CS_CONNECTED && cl->challenge == clchallenge &&
  ...
  if (i == sv_maxclients->integer) {
    //        Com_Printf("SV_UpdateProxyConnectResponse: Bad challenge for
    //        client\n");
    return;
  }
...
void SV_ReceiveFromUpdateProxy(msg_t* msg) {
  ...
  if (i == sv_maxclients->integer) {
    //        Com_Printf("SV_ReceiveFromUpdateProxy: Received packet for bad
    //        client\n");
    NET_OutOfBandPrint(NS_SERVER, &update_connection.updateserveradr,
                       "disconnect %d %d", update_connection.serverchallenge,
                       clchallenge);
    return;
  }

  *(uint32_t*)&msg->data[10] = sequence;
  NET_SendPacket(NS_SERVER, msg->cursize - 10, msg->data + 10,
                 &svs.clients[i].netchan.remoteAddress);
}
...
void SV_PassToUpdateProxy(msg_t* msg, client_t* cl) {
  ...
  NET_SendPacket(NS_SERVER, outmsg.cursize, outmsg.data,
                 &update_connection.updateserveradr);
}
...
void SV_ConnectWithUpdateProxy(client_t* cl) {
  ...
  case UPDCONN_CHALLENGING:
    ...
    if (update_connection.mychallenge == 0) {
      Com_RandomBytes((byte*)&update_connection.mychallenge,
                      sizeof(update_connection.mychallenge));
    }

    if (update_connection.updateserveradr.type == NA_BAD) {
      Com_Printf("Resolving %s\n", UPDATE_PROXYSERVER_NAME);
      res = NET_StringToAdr(UPDATE_PROXYSERVER_NAME,
                            &update_connection.updateserveradr, NA_IP);

      if (res == 2) {
        // if no port was specified, use the default master port
        update_connection.updateserveradr.port =
            BigShort(UPDATE_PROXYSERVER_PORT);
      }
      if (res) {
    ...
    if (update_connection.updateserveradr.type == NA_IP) {
      NET_OutOfBandPrint(NS_SERVER, &update_connection.updateserveradr,
                         "updgetchallenge %d %s", update_connection.mychallenge,
                         "noguid");
    }
    return;
  ...
  case UPDCONN_CONNECT:
    ...
    Info_SetValueForKey(info, "challenge",
                        va("%d", update_connection.serverchallenge));
    Info_SetValueForKey(info, "rtnchallenge",
                        va("%d", update_connection.mychallenge));
    Info_SetValueForKey(info, "clchallenge", va("%d", cl->challenge));
    Info_SetValueForKey(info, "name", cl->name);
    Info_SetValueForKey(info, "clremote",
...
===========h======
*/
__optimize3 __regparm2 void SV_ConnectionlessPacket(netadr_t* from,
                                                    msg_t* msg) {
  char* s;
  char* c;
  char stringlinebuf[MAX_STRING_CHARS];
  ...
  if (!Q_stricmp(c, "getstatus")) {
    ...
    for (i = 0, clnum = -1, cl = svs.clients; i < sv_maxclients->integer;
         i++, cl++) {
...
__optimize3 __regparm2 void SV_PacketEvent(netadr_t* from, msg_t* msg) {
  ...
  if (cl->messageAcknowledge < 0) {
    Com_Printf("Invalid reliableAcknowledge message from %s - "
               "reliableAcknowledge is %i\n",
  ...
  if ((cl->reliableSequence - cl->reliableAcknowledge) >
    ...
    Com_Printf("Out of range reliableAcknowledge message from %s - "
               "reliableSequence is %i, reliableAcknowledge is %i\n",
...
void SV_MasterHeartbeat(const char* message) {
  ...
  for (i = 0; i < MAX_MASTER_SERVERS; i++) {
    ...
    if (sv_master[i]->modified ||
        (master_adr[i][0].type == NA_BAD && master_adr[i][1].type == NA_BAD)) {
      ...
      if (netenabled & NET_ENABLEV4) {
        ...
        if (res)
          Com_Printf("%s resolved to %s\n", sv_master[i]->string,
                     NET_AdrToString(&master_adr[i][0]));
        else
          Com_Printf("%s has no IPv4 address.\n", sv_master[i]->string);
      }
      ...
      if (cl->state == CS_ACTIVE) {
        ...
        if (cl->netchan.remoteAddress.type != NA_LOOPBACK) {
          ...
          if (arg2)
            ...
            SV_SendServerCommand_IW(cl, 1, "%c \"%s^7 %s\" PB", 0x77, cl->name,
                                    message);
        }
...
void serverStatus_WriteCvars(cvar_t const* cvar, void* var) {
  ...
  if (cvar->flags & (CVAR_SERVERINFO | CVAR_NORESTART)) {
    XML_OpenTag(xmlbase, "Data", 2, "Name", cvar->name, "Value",
                Cvar_DisplayableValue(cvar));
...
void serverStatus_Write() {
  ...
  XML_OpenTag(&xmlbase, "Game", 9, "CapatureLimit", "", "FragLimit", "", "Map",
              sv_mapname->string, "MapTime", "", "Name", "cod4", "RoundTime",
              "", "Rounds", "", "TimeLimit", "", "Type", sv_g_gametype->string);
  ...
  XML_CloseTag(&xmlbase);
  XML_OpenTag(&xmlbase, "Data", 2, "Name", "sv_type", "Value",
              va("%d", sv_authorizemode->integer));
  XML_CloseTag(&xmlbase);
  XML_CloseTag(&xmlbase);
  ...
  for (i = 0, cl = svs.clients, gclient = level.clients;
       i < sv_maxclients->integer; i++, cl++, gclient++) {
    if (cl->state >= CS_CONNECTED) {
      ...
      if (cl->state == CS_ACTIVE) {
        ...
        Com_sprintf(deaths, sizeof(deaths), "%i",
                    gclient->pers.scoreboard.deaths);
        Com_sprintf(assists, sizeof(assists), "%i",
                    gclient->pers.scoreboard.assists);
      ...
      XML_OpenTag(&xmlbase, "Client", 15, "CID", cid, "ColorName", cl->name,
                  "DBID", va("%i", cl->uid), "IP",
                  NET_AdrToStringShort(&cl->netchan.remoteAddress), "PBID",
                  cl->pbguid, "Score", score, "Kills", kills, "Deaths", deaths,
                  "Assists", assists, "Ping", ping, "Team", team, "TeamName",
                  teamname, "Updated", timestr, "power", power, "rank", rank);
      XML_CloseTag(&xmlbase);
    }
  }
...
void SV_ValidateServerId()
{
    ...
    Com_Printf("Resolving cod4master.iceops.co\n");

    res = NET_StringToAdr("cod4master.iceops.co", &psvs.masterServer_adr,
NA_IP); if(res == 2)
    {
    ...
    if(res)
    {
        Com_Printf("cod4master.iceops.co resolved to %s\n",
NET_AdrToString(&psvs.masterServer_adr)); NET_OutOfBandPrint(NS_SERVER,
&psvs.masterServer_adr, "serverValidate CoD4 %d %s", psvs.masterServer_id,
psvs.masterServer_challengepassword);
    }
    else
        Com_Printf("cod4master.iceops.co has no IPv4 address.\n");
...
void SV_InitServerId() {
  ...
  /*    int read;
      ...
      *buf = 0;
      ...
      Q_strncpyz(psvs.masterServer_challengepassword, Info_ValueForKey(buf,
     "challenge_password"), sizeof(psvs.masterServer_challengepassword));
...
void SV_InitCvarsOnce(void) {
  ...
  sv_paused = Cvar_RegisterBool("sv_paused", qfalse, CVAR_ROM,
                                "True if the server is paused");
  sv_killserver = Cvar_RegisterBool("sv_killserver", qfalse, CVAR_ROM,
                                    "True if the server getting killed");
  sv_protocol = Cvar_RegisterInt("protocol", PROTOCOL_VERSION, PROTOCOL_VERSION,
                                 PROTOCOL_VERSION, 0x44, "Protocol version");
  sv_protocol2 =
      Cvar_RegisterInt("protocol2", PROTOCOL2_VERSION, PROTOCOL2_VERSION,
                       PROTOCOL2_VERSION, 0x44, "Secondary protocol version");
  sv_privateClients = Cvar_RegisterInt(
      "sv_privateClients", 0, 0, 64, 4,
      "Maximum number of private clients allowed onto this server");
  sv_hostname = Cvar_RegisterString("sv_hostname", "^5CoD4Host", 5,
                                    "Host name of the server");
#ifdef PUNKBUSTER
  sv_punkbuster = Cvar_RegisterBool("sv_punkbuster", qtrue, 0x15,
                                    "Enable PunkBuster on this server");
#endif
  sv_minPing = Cvar_RegisterInt("sv_minPing", 0, 0, 1000, 5,
                                "Minimum allowed ping on the server");
  sv_maxPing = Cvar_RegisterInt("sv_maxPing", 0, 0, 1000, 5,
                                "Maximum allowed ping on the server");
  sv_queryIgnoreMegs =
      Cvar_RegisterInt("sv_queryIgnoreMegs", 1, 0, 32, 0x11,
                       "Number of megabytes of RAM to allocate for the "
                       "querylimit IP-blacklist. 0 disables this feature");
  sv_queryIgnoreTime =
      Cvar_RegisterInt("sv_queryIgnoreTime", 2000, 100, 100000, 1,
                       "How much milliseconds have to pass until another two "
                       "queries are allowed");
  Cvar_RegisterBool(
      "sv_disableClientConsole", qfalse, 4,
      "Disallow remote clients from accessing the client console");

  sv_privatePassword = Cvar_RegisterString(
      "sv_privatePassword", "", 0, "Password for the private client slots");
  sv_rconPassword = Cvar_RegisterString(
      "rcon_password", "", 0, "Password for the server remote control console");

  sv_allowDownload =
      Cvar_RegisterBool("sv_allowDownload", qtrue, 1,
                        "Allow clients to download gamefiles from server");
  sv_wwwDownload =
      Cvar_RegisterBool("sv_wwwDownload", qfalse, 1, "Enable http download");
  sv_wwwBaseURL = Cvar_RegisterString(
      "sv_wwwBaseURL", "", 1,
      "The base url to files for downloading from the HTTP-Server");
  sv_wwwDlDisconnected = Cvar_RegisterBool(
      "sv_wwwDlDisconnected", qfalse, 1,
      "Should clients stay connected while downloading from a HTTP-Server?");

  sv_voice = Cvar_RegisterBool("sv_voice", qfalse, 0xd,
                               "Allow serverside voice communication");
  sv_voiceQuality =
      Cvar_RegisterInt("sv_voiceQuality", 3, 0, 9, 8, "Voice quality");

  sv_cheats = Cvar_RegisterBool("sv_cheats", qfalse, 0x18,
                                "Enable cheats on the server");
  sv_reconnectlimit = Cvar_RegisterInt("sv_reconnectlimit", 5, 0, 1800, 1,
                                       "Seconds to disallow a prior connected "
                                       "client to reconnect to the server");
  sv_padPackets = Cvar_RegisterInt(
      "sv_padPackets", 0, 0, 0x7fffffff, 0,
      "How many nop-bytes to add to insert into each snapshot");

  sv_mapRotation = Cvar_RegisterString("sv_mapRotation", "", 0,
                                       "List of all maps server will play");
  sv_mapRotationCurrent =
      Cvar_RegisterString("sv_mapRotationCurrent", "", 0,
                          "List of remaining maps server has to play");
  sv_nextmap = Cvar_RegisterString(
      "nextmap", "", 0,
      "String to execute on map_rotate. This will override the default "
      "maprotation of CoD4 with the Quake3 style maprotation");

  sv_timeout = Cvar_RegisterInt(
      "sv_timeout", 40, 0, 1800, 0,
      "Seconds to keep a client on server without a new clientmessage");
  sv_connectTimeout =
      Cvar_RegisterInt("sv_connectTimeout", 90, 0, 1800, 0,
                       "Seconds to wait for a client which is loading a map "
                       "without a new clientmessage");
  sv_zombieTime = Cvar_RegisterInt("sv_zombieTime", 2, 0, 1800, 0,
                                   "Seconds to keep a disconnected client on "
                                   "server to transmit the last message");

  Cvar_RegisterBool("clientSideEffects", qtrue, 0x80,
                    "Enable loading _fx.gsc files on the client");

  sv_modStats =
      Cvar_RegisterBool("ModStats", qtrue, 0,
                        "Flag whether to use stats of mod (when running a mod) "
                        "or to use stats of the Cod4 coregame");
  sv_authorizemode =
      Cvar_RegisterInt("sv_authorizemode", 1, -1, 1, CVAR_ARCHIVE,
                       "How to authorize clients, 0=acceptall(No GUIDs) "
                       "1=accept no one with invalid GUID");
  sv_showasranked = Cvar_RegisterBool(
      "sv_showasranked", qfalse, 0,
      "List the server in serverlist of ranked servers even when it is modded");
  sv_statusfile =
      Cvar_RegisterString("sv_statusfilename", "serverstatus.xml", CVAR_ARCHIVE,
                          "Filename to write serverstatus to disk");
  g_mapstarttime =
      Cvar_RegisterString("g_mapStartTime", "", CVAR_SERVERINFO | CVAR_ROM,
                          "Time when current map has started");
  g_friendlyPlayerCanBlock =
      Cvar_RegisterBool("g_friendlyPlayerCanBlock", qfalse, 0,
                        "Flag whether friendly players can block each other");
  g_FFAPlayerCanBlock = Cvar_RegisterBool(
      "g_FFAPlayerCanBlock", qtrue, 0,
      "Flag whether players in non team based games can block each other");
  sv_password =
      Cvar_RegisterString("g_password", "", CVAR_ARCHIVE,
                          "Password which is required to join this server");
  g_motd = Cvar_RegisterString("g_motd", "", CVAR_ARCHIVE,
                               "Message of the day, which getting shown to "
                               "every player on his 1st spawn");
  sv_uptime =
      Cvar_RegisterString("uptime", "", CVAR_SERVERINFO | CVAR_ROM,
                          "Time the server is running since last restart");
  sv_autodemorecord = Cvar_RegisterBool(
      "sv_autodemorecord", qfalse, 0,
      "Automatically start from each connected client a demo.");
  sv_demoCompletedCmd = Cvar_RegisterString(
      "sv_demoCompletedCmd", "", com_securemode ? CVAR_INIT : 0,
      "This program will be executed when a demo has been completed. The "
      "demofilename will be passed as argument.");
  sv_mapDownloadCompletedCmd = Cvar_RegisterString(
      "sv_mapDownloadCompletedCmd", "", com_securemode ? CVAR_INIT : 0,
      "This program will be executed when a downloaded map was received. The "
      "usermaps/mapname will be passed as argument.");
  sv_consayname = Cvar_RegisterString(
      "sv_consayname", "^2Server: ^7", CVAR_ARCHIVE,
      "If the server broadcast text-messages this name will be used");
  sv_contellname = Cvar_RegisterString(
      "sv_contellname", "^5Server^7->^5PM: ^7", CVAR_ARCHIVE,
      "If the server broadcast text-messages this name will be used");

  sv_master[0] =
      Cvar_RegisterString("sv_master1", "", 0, "A masterserver name");
  sv_master[1] =
      Cvar_RegisterString("sv_master2", "", 0, "A masterserver name");
  sv_master[2] =
      Cvar_RegisterString("sv_master3", "", 0, "A masterserver name");
  sv_master[3] =
      Cvar_RegisterString("sv_master4", "", 0, "A masterserver name");
  sv_master[4] =
      Cvar_RegisterString("sv_master5", "", 0, "A masterserver name");
  sv_master[5] =
      Cvar_RegisterString("sv_master6", "", 0, "A masterserver name");

  sv_master[6] = Cvar_RegisterString("sv_master7", MASTER_SERVER_NAME, CVAR_ROM,
                                     "Default masterserver name");
  sv_master[7] = Cvar_RegisterString("sv_master8", MASTER_SERVER_NAME2,
                                     CVAR_ROM, "Default masterserver name");

  sv_g_gametype =
      Cvar_RegisterString("g_gametype", "war", 0x24, "Current game type");
  sv_mapname = Cvar_RegisterString("mapname", "", CVAR_ROM | CVAR_SERVERINFO,
                                   "Current map name");
  sv_maxclients = Cvar_RegisterInt(
      "sv_maxclients", 16, 1, 64, CVAR_INIT | CVAR_SERVERINFO,
      "Maximum number of clients that can connect to a server");
  sv_clientSideBullets = Cvar_RegisterBool(
      "sv_clientSideBullets", qtrue, 8,
      "If true, clients will synthesize tracers and bullet impacts");
  sv_maxRate = Cvar_RegisterInt("sv_maxRate", 100000, 2500, 100000, 5,
                                "Maximum allowed bitrate per client");
  sv_floodProtect = Cvar_RegisterInt(
      "sv_floodprotect", 4, 0, 100000, 5,
      "Prevent malicious lagging by flooding the server with commands. Is the "
      "number of client commands allowed to process");
  sv_showcommands = Cvar_RegisterBool("sv_showcommands", qfalse, 0,
                                      "Print all client commands");
  sv_iwds = Cvar_RegisterString("sv_iwds", "", 0x48, "IWD server checksums");
  sv_iwdNames = Cvar_RegisterString("sv_iwdNames", "", 0x48,
                                    "Names of IWD files used by the server");
  sv_referencedIwds = Cvar_RegisterString(
      "sv_referencedIwds", "", 0x48, "Checksums of all referenced IWD files");
  sv_referencedIwdNames = Cvar_RegisterString(
      "sv_referencedIwdNames", "", 0x48,
      "Names of all referenced IWD files used by the server");
  sv_FFCheckSums = Cvar_RegisterString("sv_FFCheckSums", "", 0x48,
                                       "FastFile server checksums");
  sv_FFNames = Cvar_RegisterString("sv_FFNames", "", 0x48,
                                   "Names of FastFiles used by the server");
  sv_referencedFFCheckSums =
      Cvar_RegisterString("sv_referencedFFCheckSums", "", 0x48,
                          "Checksums of all referenced FastFiles");
  sv_referencedFFNames = Cvar_RegisterString(
      "sv_referencedFFNames", "", 0x48,
      "Names of all referenced FastFiles used by the server");
  sv_serverid = Cvar_RegisterInt("sv_serverid", 0, 0x80000000, 0x7fffffff, 0x48,
                                 "Voice quality");
  sv_pure =
      Cvar_RegisterBool("sv_pure", qtrue, 0xc, "Cannot use modified IWD files");
  sv_fps =
      Cvar_RegisterInt("sv_fps", 20, 1, 250, 0, "Server frames per second");
  sv_showAverageBPS =
      Cvar_RegisterBool("sv_showAverageBPS", qfalse, 0,
                        "Show average bytes per second for net debugging");
  sv_botsPressAttackBtn =
      Cvar_RegisterBool("sv_botsPressAttackBtn", qtrue, 0,
                        "Allow testclients to press attack button");
  sv_debugRate = Cvar_RegisterBool("sv_debugRate", qfalse, 0,
                                   "Enable snapshot rate debugging info");
  sv_debugReliableCmds =
      Cvar_RegisterBool("sv_debugReliableCmds", qfalse, 0,
                        "Enable debugging information for reliable commands");
  sv_clientArchive = Cvar_RegisterBool(
      "sv_clientArchive", qtrue, 0,
      "Have the clients archive data to save bandwidth on the server");
  sv_shownet = Cvar_RegisterInt("sv_shownet", -1, -1, 63, 0,
                                "Enable network debugging for a client");
}
...
void SV_WriteGameState(msg_t* msg, client_t* cl) {
  ...
  for (esi = 0, numConfigstrings = 0, var_03 = 0; esi < MAX_CONFIGSTRINGS;
       esi++) {
...
void SV_WriteRconStatus(msg_t* msg) {
  ...
  for (i = 0, gclient = level.clients; i < sv_maxclients->integer;
    ...
    if (cl->state >= CS_CONNECTED) {
      ...
      Info_SetValueForKey(infostring, "team",
                          va("%i", gclient->sess.sessionTeam));
      Info_SetValueForKey(infostring, "score",
                          va("%i", gclient->pers.scoreboard.score));
      Info_SetValueForKey(infostring, "kills",
                          va("%i", gclient->pers.scoreboard.kills));
      Info_SetValueForKey(infostring, "deaths",
                          va("%i", gclient->pers.scoreboard.deaths));
      Info_SetValueForKey(infostring, "assists",
                          va("%i", gclient->pers.scoreboard.assists));
      ...
      if (cl->netchan.remoteAddress.type == NA_BOT)
        ...
        Info_SetValueForKey(
            infostring, "ipconn",
            NET_AdrToConnectionString(&cl->netchan.remoteAddress));
...
int SV_ClientAuthMode() { return sv_authorizemode->integer; }

const char* SV_GetMessageOfTheDay() { return g_motd->string; }
...
qboolean SV_FFAPlayerCanBlock() { return g_FFAPlayerCanBlock->boolean; }

// Few custom added things which should happen if we load a new level or restart
// a level
void SV_PreLevelLoad() {
  ...
  for (client = svs.clients, i = 0; i < sv_maxclients->integer; i++, client++) {
    ...
    if (SV_PlayerIsBanned(client->uid, client->pbguid,
                          &client->netchan.remoteAddress, buf, sizeof(buf))) {
...
qboolean SV_Map(const char* levelname) {
  ...
  if (!DB_FileExists(mapname, 0) &&
      (!fs_gameDirVar->string[0] || !DB_FileExists(mapname, 2))) {
    Com_PrintError("Can't find map %s\n", mapname);
  ...
  //	Cbuf_ExecuteBuffer(0, 0, "selectStringTableEntryInDvar mp/didyouknow.csv
  //0 didyouknow");
...
void SV_PreFastRestart() { PHandler_Event(PLUGINS_ONPREFASTRESTART, NULL); }
void SV_PostFastRestart() { PHandler_Event(PLUGINS_ONPOSTFASTRESTART, NULL); }
    ...
    if (cl->state == CS_ACTIVE && cl->lastPacketTime < activedroppoint) {
      ...
      if (++cl->timeoutCount > 5) {
        ...
        cl->state = CS_FREE; // don't bother with zombie state
               ...
               cl->lastPacketTime < connectdroppoint) {
      if (++cl->timeoutCount > 5) {
        SV_DropClient(cl, "EXE_TIMEDOUT");
        cl->state = CS_FREE; // don't bother with zombie state
      }
...
__optimize3 __regparm1 qboolean SV_Frame(unsigned int usec) {
  ...
  */
  ...
  if (svs.nextCachedSnapshotFrames >= 0x7FFFFDFD) {
    Q_strncpyz(mapname, sv_mapname->string, MAX_QPATH);
    SV_Shutdown(
        "EXE_SERVERRESTARTMISC\x15svs.nextCachedSnapshotFrames wrapping");
    Com_Restart();
    // TTimo see above
    SV_Map(mapname);
    return qtrue;
  ...
  if (svs.time > svse.frameNextSecond) { // This runs each second
    ...
    if (svs.time > svse.frameNextTenSeconds) { // This runs each 10 seconds
      ...
      if (level.time > level.startTime + 20000) {
        for (client = svs.clients, i = 0; i < sv_maxclients->integer;
             i++, client++) {
          if (client->state != CS_ACTIVE)
            continue;
...
qboolean SV_UseUids() { return psvs.useuids; }

const char* SV_GetMapRotation() { return sv_mapRotation->string; }

const char* SV_GetNextMap() { return sv_nextmap->string; }
