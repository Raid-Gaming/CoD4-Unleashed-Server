...
int DB_GetXAssetStdCount(XAssetType_t type) { return XAssetStdCount[type]; }
...
void DB_ParseRequestedXAssetNum() {
  ...
  r_xassetnum = Cvar_RegisterString("r_xassetnum", "", CVAR_INIT,
                                    "The list of xassets with their count in "
                                    "the key=value key=value... format");

  Com_Memcpy(XAssetRequestedCount, XAssetStdCount,
             sizeof(XAssetRequestedCount));
  Com_sprintf(toparse, sizeof(toparse), " %s", r_xassetnum->string);

  for (i = 0; i < NumXAssets; ++i) {

    if (DB_XAssetNoAlloc(i) || i == menuDef_t || i == WeaponDef ||
        i == StringTable) {
      continue;
    }
...
void XAssets_PatchLimits() {
  ...
  if (!Sys_MemoryProtectWrite(ptrpool, size) ||
      !Sys_MemoryProtectWrite(ptrsize, size)) {
    Com_Error(ERR_FATAL,
              "XAssets_PatchLimits: Failed to change memory to writeable\n");
  }
  ...
  if (!Sys_MemoryProtectReadonly(ptrpool, size) ||
      !Sys_MemoryProtectReadonly(ptrsize, size)) {
    Com_Error(ERR_FATAL,
              "XAssets_PatchLimits: Failed to change memory to read only\n");
  }
}
#endif
...
void _InterlockedExchangeAdd(int* lockedvar, int num) { *lockedvar += num; }
...
void __cdecl DB_ReferencedFastFiles(char* g_zoneSumList, char* g_zoneNameList,
int maxsize)
{
        int i;
        g_zones_struct *zone;
        ...
        {
                if ( zone->fastfilename[0] == '\0' ||
!Q_strncmp(zone->fastfilename, "localized_", 10) )
                {
                        continue;
                }
                if ( g_zoneSumList[0] )
          ...
          Q_strncat(g_zoneNameList, maxsize, zone->fastfilename);
                ...
                Com_sprintf(fileName, sizeof(fileName), "%s_load",
zone->fastfilename); DB_BuildOSPath(fileName, 2, sizeof(ospath), ospath); v4 =
CreateFileA(ospath);
...
void DB_EnumXAssets(XAssetType_t type,
                    void(__cdecl* callback)(XAssetHeader_t*, void*),
                    void* cbargs, qboolean a4) {
  int index, sindex, i;
  ...
  */
  for (i = 0; i < 32768; ++i) {
    for (index = db_hashTable[i]; index; index = listselector->nextListIndex) {
      ...
      for (sindex = listselector->field_C; sindex;
           sindex = slistselect->field_C) {
...
void DB_BuildOverallocatedXAssetList(char* configstring, int len) {
  ...
  for (i = 0; i < NumXAssets; ++i) {
    ...
    Com_sprintf(cstring, sizeof(cstring), "%s=%d ", DB_GetXAssetTypeName[i],
                countlist[i]);
...
void XAssetUsage_f() {
  ...
  for (assettype = 0; assettype < NumXAssets; assettype++) {
    ...
    Com_Printf(" %5d %5d\n", countlist[assettype],
               DB_XAssetPoolSize[assettype] - countlist[assettype]);
  }
  Com_Printf("\n");
}
