...
void Netchan_Init(int port) {
  ...
  port &= 0xffff;
  showpackets = Cvar_RegisterBool("showpackets", qfalse, CVAR_TEMP,
                                  "Show all sent and received packets");
  showdrop =
      Cvar_RegisterBool("showdrop", qfalse, CVAR_TEMP, "Show dropped packets");
  qport = Cvar_RegisterInt("net_qport", port, 1, 65535, CVAR_INIT,
                           "The net_chan qport");
  NET_CookieInit();
}
...
void Netchan_Setup(netsrc_t sock, netchan_t* chan, netadr_t adr,
                   unsigned int qport, byte* unsentBuffer, int unsentBufferSize,
                   byte* fragmentBuffer, int fragmentBufferSize) {
...
qboolean Netchan_Process(netchan_t* chan, msg_t* msg) {
  ...
  if (fragmented) {
    ...
    if (fragmentLength < 0 || msg->readcount + fragmentLength > msg->cursize ||
      ...
      if (showdrop->boolean || showpackets->boolean) {
        Com_Printf("%s:illegal fragment length: Current %i Fragment length %i "
                   "Max %i\n",
...
qboolean NET_SendPacket(netsrc_t sock, int length, const void* data,
                        netadr_t* to) {
...
__cdecl void QDECL NET_OutOfBandPrint(netsrc_t sock, netadr_t* adr,
                                      const char* format, ...) {
  va_list argptr;
...
void NET_OutOfBandData(netsrc_t sock, netadr_t* adr, byte* format, int len) {
  ...
  /*	if(len +5 >= MAX_MSGLEN){
              Com_PrintWarning("Buffer Overflow in NET_OutOfBandData %i
     bytes\n", len); return;
          }
  */
  for (i = 0; i < len; i++) {
