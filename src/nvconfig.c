...
qboolean NV_ParseConfigLine(char* line, int linenumber) {
  ...
  if (!Q_stricmp(Info_ValueForKey(line, "type"), "cmdMinPower")) {
    ...
    if (!Cmd_InfoSetPower(line)) {
      ...
      return qtrue; // Don't rise errors here! This can happen for commands we
                    // add at a later stage
    }
    return qtrue;

  } else if (!Q_stricmp(Info_ValueForKey(line, "type"), "rconAdmin")) {

    Com_PrintWarning("HL2 Rcon admin is unsupported. Please use the general "
                     "admin system instead\n");
    return qtrue;
...
void NV_WriteConfig() {
  ...
  Q_strcat(buffer, MAX_NVCONFIG_SIZE,
           "//Autogenerated non volatile config settings\n");
