...
qboolean G_ShouldEntitiesClip(moveclip_t* clip, int touchNum,
  ...
  if (clip->passEntityNum < 64 && clip->contentmask & CONTENTS_PLAYERCLIP) {
    ...
    if (g_entities[clip->passEntityNum].client->sess.sessionTeam == TEAM_FREE) {
      ...
      if (!SV_FFAPlayerCanBlock()) {
        ...
        } else if (touch->r.ownerNum - 1 < 64 &&
                   touch->r.contents & CONTENTS_PLAYERCLIP) {
          return qfalse;
        }
      }
      ...
      if (touchNum < 64) {
        if (OnSameTeam(&g_entities[clip->passEntityNum],
                       &g_entities[touchNum])) {
          return qfalse;
        }
      } else if (touch->r.ownerNum && touch->r.ownerNum <= 64 &&
                 touch->r.contents & CONTENTS_PLAYERCLIP) {
        if (OnSameTeam(&g_entities[clip->passEntityNum],
                       &g_entities[touch->r.ownerNum - 1])) {
          return qfalse;
        }
      }
    }
...
__cdecl void SV_ClipMoveToEntity(moveclip_t* clip, svEntity_t* entity,
                                 trace_t* trace) {
  ...
  CM_TransformedBoxTrace(trace, clip->start, clip->end, clip->mins, clip->maxs,
