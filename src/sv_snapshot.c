...
__cdecl void SV_UpdateServerCommandsToClient(client_t* client, msg_t* msg) {
  ...
  for (i = client->reliableAcknowledge + 1; i <= client->reliableSequence;
    ...
    MSG_WriteLong(msg, i);
    // MSG_WriteString( msg, extcl->reliableCommands[ i & (
    // MAX_RELIABLE_COMMANDS - 1 ) ].command );
    MSG_WriteString(
        msg, client->reliableCommands[i & (MAX_RELIABLE_COMMANDS - 1)].command);
  }
...
__cdecl void SV_WriteSnapshotToClient(client_t* client, msg_t* msg) {
  ...
  if (client->deltaMessage <= 0 || client->state != CS_ACTIVE) {
    ...
    if (oldframe->first_entity <
        svsHeader.nextSnapshotEntities - svsHeader.numSnapshotEntities) {
      Com_PrintWarning("%s: Delta request from out of date entities - delta "
                       "against entity %i, oldest is %i, current is %i.  Their "
                       "old snapshot had %i entities in it\n",
                       client->name, oldframe->first_entity,
                       svs.nextSnapshotEntities - svs.numSnapshotEntities,
                       svs.nextSnapshotEntities, oldframe->num_entities);
      oldframe = NULL;
      lastframe = 0;
      var_x = 0;

    } else if (oldframe->first_client <
               svsHeader.nextSnapshotClients - svsHeader.numSnapshotClients) {

      Com_PrintWarning("%s: Delta request from out of date clients - delta "
                       "against client %i, oldest is %i, current is %i.  Their "
                       "old snapshot had %i clients in it\n",
                       client->name, oldframe->first_client,
                       svs.nextSnapshotClients - svs.numSnapshotClients,
                       svs.nextSnapshotClients, oldframe->num_clients);
      oldframe = NULL;
      lastframe = 0;
      var_x = 0;
    }
  ...
  if (oldframe) {
    MSG_WriteDeltaPlayerstate(&snapInfo, msg, svsHeader.time, &oldframe->ps,
                              &frame->ps);
  ...
  while (newindex < frame->num_entities || oldindex < from_num_entities) {
    if (newindex >= frame->num_entities) {
      ...
      newent = &svsHeader.snapshotEntities[(frame->first_entity + newindex) %
                                           svsHeader.numSnapshotEntities];
    ...
    if (oldindex >= from_num_entities) {
      ...
      oldent = &svsHeader.snapshotEntities[(from_first_entity + oldindex) %
                                           svsHeader.numSnapshotEntities];
    ...
    if (newnum == oldnum) {
      ...
      //			Com_Printf("   Delta Update Entity - New delta:
      //%i, %x  Old delta: %i, %x\n", newent->number, newent, oldent->number,
      //oldent);
      MSG_WriteDeltaEntity(&snapInfo, msg, svsHeader.time, oldent, newent,
                           qfalse);
      oldindex++;
      newindex++;
      continue;
    ...
    if (newnum < oldnum) {
      ...
      MSG_WriteDeltaEntity(&snapInfo, msg, svsHeader.time,
                           &svsHeader.svEntities[newnum].baseline, newent,
                           qtrue);
      snapInfo.var_02 = 0;
      newindex++;
      continue;
  ...
  while (newindex < frame->num_clients || oldindex < from_num_clients) {
    if (newindex >= frame->num_clients) {
      ...
      newcs = &svsHeader.snapshotClients[(frame->first_client + newindex) %
    ...
    if (oldindex >= from_num_clients) {
      ...
      oldcs = &svsHeader.snapshotClients[(from_first_client + oldindex) %
                                         svsHeader.numSnapshotClients];
      oldnum = oldcs->number;
    }
    ...
    if (newnum == oldnum) {
      ...
      // because the force parm is qfalse, this will not result
      // in any bytes being emited if the entity has not changed at all
      MSG_WriteDeltaClient(&snapInfo, msg, svsHeader.time, oldcs, newcs,
                           qfalse);
      oldindex++;
      newindex++;
      continue;
...
int irand() { return svs.time ^ rand(); }
...
__cdecl void SV_SendMessageToClient(msg_t* msg, client_t* client) {
  ...
  *(int32_t*)0x13f39080 = *(int32_t*)msg->data;
  len = MSG_WriteBitsCompress(0, msg->data + 4, (byte*)0x13f39084,
                              msg->cursize - 4);
  ...
  if (client->state == CS_ACTIVE && client->deltaMessage >= 0 &&
      client->netchan.outgoingSequence - client->deltaMessage > 28) {
  ...
  // download can reach 30 kb/s on a regular server, we will still top at 20
  // kb/s because of sv_fps 20
...
void SV_SendClientMessages(void) {
  ...
  if (sv_showAverageBPS->integer && numclients > 0) {
    ...
    if (sv.bpsWindowSteps >= MAX_BPS_WINDOW) {
      ...
      Com_DPrintf("bpspc(%2.0f) bps(%2.0f) pk(%i) ubps(%2.0f) upk(%i) "
                  "cr(%2.2f) acr(%2.2f)\n",
                  ave / (float)numclients, ave, sv.bpsMaxBytes, uave,
                  sv.ubpsMaxBytes, comp_ratio, sv.ucompAve / sv.ucompNum);
    }
  }
  // -NERVE - SMF
  ...
  svs.archiveSnaps[svs.nextArchivedSnapshotFrames % 1200].buffer =
  ...
  if (msg.cursize > freeBytes) {
    ...
    memcpy(svs.archiveSnapBuffer, &msg.data[freeBytes],
           msg.cursize - freeBytes);
