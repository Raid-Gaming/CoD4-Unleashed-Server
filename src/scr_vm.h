...
typedef enum { SCR_CB_SAY, SCR_CB_SCRIPTCOMMAND } script_CallBacks_new_t;
...
/*Not working :(  */
void __cdecl Scr_PrintPrevCodePos(int printDest, const char* unk,
                                  qboolean unk2);
int __cdecl Scr_GetFunctionHandle(const char* scriptName,
                                  const char* labelName);
short __cdecl Scr_ExecEntThread(gentity_t* ent, int callbackHook,
...
void __cdecl Scr_SetString(unsigned short* strindexptr,
...
tGScr_AddFieldsForEntity GScr_AddFieldsForEntity =
(tGScr_AddFieldsForEntity(0x80c7808);
*/
...
void __cdecl Scr_ConstructMessageString(int, int, const char*, char*,
                                        unsigned int);
...
unsigned int Scr_LoadScript(const char* scriptname, PrecacheEntry* precache,
                            int iarg_02);
...
qboolean Scr_AddFunction(const char* cmd_name, xfunction_t function,
                         qboolean developer);
qboolean Scr_RemoveFunction(const char* cmd_name);
void Scr_ClearFunctions(void);
__cdecl void* Scr_GetFunction(const char** v_functionName,
                              qboolean* v_developer);
qboolean Scr_AddMethod(const char* cmd_name, xfunction_t function,
                       qboolean developer);
...
qboolean Scr_FS_FOpenFile(const char* filename, fsMode_t mode,
                          scr_fileHandle_t* f);
fileHandle_t Scr_OpenScriptFile(char* qpath, scr_fileHandleType_t ft,
                                fsMode_t mode);
