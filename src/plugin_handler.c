...
void PHandler_Init() // Initialize the Plugin Handler's data structures and add
                     // commands
{
...
qboolean PHandler_VerifyPlugin(void* buf, int len) {
  ...
  for (i = 0, curstring = strings[i]; curstring != NULL;
       i++, curstring = strings[i]) {
    /*
            if(strcmp(curstring,"malloc")==0 || strcmp(curstring,"calloc")==0 ||
    strcmp(curstring,"realloc")==0 || strcmp(curstring,"free")==0 ||
    strcmp(curstring,"printf")==0 || strcmp(curstring,"scanf")==0 ||
           strcmp(curstring,"asprintf")==0 || strcmp(curstring,"free")==0){ //
    malloc, calloc, realloc, free, printf, , asprintf, scanf Com_Printf("The
    plugin file contains one of the disallowed functions! Disallowed function
    name: \"%s\".\nPlease refer to the documentation for details.\nPlugin load
    failed.\n", curstring); free(strings); return qfalse;
    }
    if(strncmp(curstring,"_Zna",4)==0 || strncmp(curstring,"_Znw",4)==0){ // new
    and new[] Com_Printf("The plugin file contains C++'s new operator which is
    forbidden.\nPlease refer to the documentation for details.\nPlugin load
    failed.\n"); free(strings); return qfalse;
    }
            */
  }
  free(strings);
...
const char* PHandler_OpenTempFile(char* name, char* fullfilepath,
                                  int fplen) { // Load a plugin, safe for use
  ...
  if (PHandler_VerifyPlugin(buf, len) == qfalse) {
    Com_Printf("%s is not a plugin file or is corrupt or contains disallowed "
               "functions.\n",
               filepath);
    FS_FreeFile(buf);
    return NULL;
...
void PHandler_Load(char* name) // Load a plugin, safe for use
{
  ...
  realpath = (char*)PHandler_OpenTempFile(
      name, filepathbuf, sizeof(filepathbuf)); // Load a plugin, safe for use
  if (realpath == NULL) {
    return;
  }
  ...
  if (com_securemode) {
    ...
    if (Q_stricmp(hash, PLUGINGAMERANGER_HASH) &&
        Q_stricmp(hash, PLUGINCENSOR_HASH) &&
        Q_stricmp(hash, PLUGINANTISPAM_HASH)) {
      Com_Printf("Tiger = %s\n", hash);
      Com_PrintError("%s checksum missmatch! This plugin will not be loaded in "
                     "securemode.\n",
                     realpath);
      return;
    }
  ...
  for (j = 0; j < PLUGINS_ITEMCOUNT; ++j) {
    pluginFunctions.plugins[i].OnEvent[j] =
        Sys_GetProcedure(PHandler_Events[j]);
  }
  pluginFunctions.plugins[i].OnInfoRequest =
      pluginFunctions.plugins[i].OnEvent[PLUGINS_ONINFOREQUEST];
  ...
  Q_strncpyz(pluginFunctions.plugins[i].name, name,
             sizeof(pluginFunctions.plugins[i].name));
  pluginFunctions.initializing_plugin = qtrue;

  if (pluginFunctions.plugins[i].OnInit == NULL) {
    Com_Printf(
        "Error loading plugin's OnInit function.\nPlugin load failed.\n");
    pluginFunctions.initializing_plugin = qfalse;
    memset(
        pluginFunctions.plugins + i, 0x00,
        sizeof(
            plugin_t)); // We need to remove all references so we can dlclose.
  ...
  if (pluginFunctions.plugins[i].OnInfoRequest) {
    ...
    if (info.handlerVersion.major != PLUGIN_HANDLER_VERSION_MAJOR ||
        (info.handlerVersion.minor - info.handlerVersion.minor % 100) !=
            (PLUGIN_HANDLER_VERSION_MINOR -
             PLUGIN_HANDLER_VERSION_MINOR % 100)) {
      Com_PrintError(
          "This plugin might not be compatible with this server version! "
          "Requested plugin handler version: %d.%d, server's plugin handler "
          "version: %d.%d. Unloading the plugin...\n",
          info.handlerVersion.major, info.handlerVersion.minor,
    ...
    Com_PrintError(
        "function OnInfoRequest not found in the plugin file. Unloading...\n");
    PHandler_Unload(i);
    return;
  }
  ...
  Com_Printf("Plugin %s loaded successfully. Server is currently running %d "
             "plugins.\n",
...
void PHandler_Unload(int id) // Unload a plugin, safe for use.
{
  ...
  if (unloading) {
    Com_PrintError(
        "PHandler_Unload: tried to unload plugin #%d from it's destructor!\n",
        id);
    return;
  }

  if (pluginFunctions.plugins[id].loaded) {
    if (pluginFunctions.plugins[id].exports !=
        0) { // Library-plugins cannot be unloaded, see plugins/readme.txt
      Com_PrintError("PHandler_Unload: Cannot unload a library plugin!\n");
      return;
    }
    if (pluginFunctions.plugins[id].scriptfunctions != 0 ||
        pluginFunctions.plugins[id].scriptmethods != 0) {
    ...
    Com_Memset(&(pluginFunctions.plugins[id]), 0x00,
               sizeof(plugin_t)); // Wipe out all the data
    Sys_CloseLibrary(
        lib_handle); // Close the dll as there are no more references to it
    --pluginFunctions.loadedPlugins;
  } else {
...
void PHandler_Event(int eventID, ...) // Fire a plugin event, safe for use
{
  ...
  for (i = 0; i < pluginFunctions.loadedPlugins; i++) {
    if (pluginFunctions.plugins[i].OnEvent[eventID] != NULL) {
      pluginFunctions.hasControl = i;
      (*pluginFunctions.plugins[i].OnEvent[eventID])(arg_0, arg_1, arg_2, arg_3,
                                                     arg_4, arg_5);
      pluginFunctions.hasControl = PLUGIN_UNKNOWN;
    }
  }
