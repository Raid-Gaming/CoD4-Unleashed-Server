...
P_P_F int Plugin_ClientToSlot(client_t* client) { return client - svs.clients; }

P_P_F clientScoreboard_t Plugin_GetClientScoreboard(int clientNum) {
  return level.clients[clientNum].pers.scoreboard;
}
P_P_F int Plugin_Cmd_GetInvokerUid() { return Cmd_GetInvokerUID(); }
P_P_F int Plugin_Cmd_GetInvokerSlot() { return Cmd_GetInvokerClnum(); }
P_P_F int Plugin_GetPlayerUid(int slot) { return svs.clients[slot].uid; }
P_P_F int Plugin_GetSlotCount() { return sv_maxclients->integer; }
...
P_P_F char* Plugin_GetPlayerName(int slot) { return svs.clients[slot].name; }
P_P_F void Plugin_AddCommand(char* name, xcommand_t xcommand, int power) {
  volatile int pID;
  pID = PHandler_CallerID();
  if (pID >= MAX_PLUGINS) {
    Com_PrintError("Tried adding a command for a plugin with non existent pID. "
                   "pID supplied: %d.\n",
                   pID);
    return;
  } else if (pID < 0) {
    Com_PrintError("Plugin_AddCommand called from not within a plugin or from "
                   "a disabled plugin!\n");
    return;
  }
  ...
  pluginFunctions.plugins[pID].cmd[pluginFunctions.plugins[pID].cmds].xcommand =
      xcommand;
  strcpy(pluginFunctions.plugins[pID]
             .cmd[pluginFunctions.plugins[pID].cmds++]
             .name,
         name);
...
P_P_F qboolean Plugin_TcpConnect(int connection, const char* remote) {
  ...
  if (pID < 0) {
    Com_Printf(
        "Plugins: Error! Tried open a TCP-Connection for unknown plugin!\n");
    return qfalse;
  }
  if (pluginFunctions.plugins[pID].enabled == qfalse) {
    Com_Printf(
        "^1WARNING^7: Tried open a TCP-Connection for a disabled plugin!\n");
    return qfalse;
  }
  if (connection >= PLUGIN_MAX_SOCKETS || connection < 0) {
    Com_PrintError("Plugin_TcpConnect: Second argument can only be a value "
                   "inside the range: 0...%d plugin ID: #%d\n",
                   PLUGIN_MAX_SOCKETS);
    return qfalse;
  }
...
P_P_F int Plugin_TcpGetData(int connection, void* buf, int size) {
  ...
  if (pID < 0) {
    Com_Printf(
        "Plugin_TcpGetData: Error! Tried get TCP data for unknown plugin!\n");
    return -1;
  }
  if (pluginFunctions.plugins[pID].enabled == qfalse) {
    Com_Printf("^1WARNING^7: Plugin_TcpGetData: Tried get TCP data for a "
               "disabled plugin!\n");
    return -1;
  }
  if (connection >= PLUGIN_MAX_SOCKETS || connection < 0) {
    Com_PrintError("Plugin_TcpGetData: First argument can only be a value "
                   "inside the range: 0...%d plugin ID: #%d\n",
                   PLUGIN_MAX_SOCKETS, pID);
    return -1;
  }
  if (buf == NULL) {
    Com_PrintError("Plugin_TcpGetData: Third argument can not be a "
                   "NULL-Pointer for plugin ID: #%d\n",
                   pID);
    return -1;
  }
...
P_P_F qboolean Plugin_TcpSendData(int connection, void* data, int len) {
  ...
  if (pID < 0) {
    Com_Printf(
        "Plugin_TcpSendData: Error! Tried get TCP data for unknown plugin!\n");
    return qfalse;
  }
  if (pluginFunctions.plugins[pID].enabled == qfalse) {
    Com_Printf("^1WARNING^7: Plugin_TcpSendData: Tried get TCP data for a "
               "disabled plugin!\n");
    return qfalse;
  }
  if (connection >= PLUGIN_MAX_SOCKETS || connection < 0) {
    Com_PrintError("Plugin_TcpSendData: Second argument can only be a value "
                   "inside the range: 0...%d plugin ID: #%d\n",
                   PLUGIN_MAX_SOCKETS, pID);
    return qfalse;
  }
  if (data == NULL) {
    Com_PrintError("Plugin_TcpSendData: Second argument can not be a "
                   "NULL-Pointer for plugin ID: #%d\n",
                   pID);
    return qfalse;
  }
...
P_P_F void Plugin_TcpCloseConnection(int connection) {
  ...
  pID = PHandler_CallerID();
  if (pID < 0) {
    Com_Printf("Plugin_TcpCloseConnection: Error! Tried get close a connection "
               "for unknown plugin!\n");
    return;
  }
  if (pluginFunctions.plugins[pID].enabled == qfalse) {
    Com_Printf("^1WARNING^7: Plugin_TcpCloseConnection: Tried to close a "
               "connection for a disabled plugin!\n");
    return;
  }
  if (connection >= PLUGIN_MAX_SOCKETS || connection < 0) {
    Com_PrintError("Plugin_TcpCloseConnection: Second argument can only be a "
                   "value inside the range: 0...%d plugin ID: #%d\n",
                   PLUGIN_MAX_SOCKETS, pID);
    return;
  }
...
P_P_F qboolean Plugin_UdpSendData(netadr_t* to, void* data, int len) {
  ...
  if (to == NULL) {
    pID = PHandler_CallerID();
    Com_PrintError("Plugin_UdpSendData: First argument can not be a "
                   "NULL-Pointer for plugin ID: #%d\n",
                   pID);
    return qfalse;
  }
...
P_P_F void Plugin_SetPlayerUID(unsigned int clientslot, unsigned int uid) {
  ...
  if (clientslot > sv_maxclients->integer) {
    PHandler_Error(
        PID, P_ERROR_DISABLE,
        va("Plugin tried to set UID for bad client: %d\n", clientslot));
  }
...
P_P_F unsigned int Plugin_GetPlayerUID(unsigned int clientslot) {
  ...
  if (clientslot > sv_maxclients->integer) {
    PHandler_Error(
        PID, P_ERROR_DISABLE,
        va("Plugin tried to get UID for bad client: %d\n", clientslot));
  }
  cl = &svs.clients[clientslot];
  return cl->uid;
}
...
P_P_F const char* Plugin_GetPlayerGUID(unsigned int clientslot) {
  ...
  if (clientslot > sv_maxclients->integer) {
    PHandler_Error(
        PID, P_ERROR_DISABLE,
        va("Plugin tried to get GUID for bad client: %d\n", clientslot));
  }
  cl = &svs.clients[clientslot];
  return cl->pbguid;
}
...
P_P_F void Plugin_SetPlayerNoPB(unsigned int clientslot) {
  ...
  if (clientslot > sv_maxclients->integer) {
    PHandler_Error(
        PID, P_ERROR_DISABLE,
        va("Plugin tried to set No PunkBuster flag for bad client: %d\n",
           clientslot));
  }
  cl = &svs.clients[clientslot];
  cl->noPb = qtrue;
...
P_P_F int Plugin_DoesServerUseUids(void) { return psvs.useuids; }

P_P_F void Plugin_SetServerToUseUids(int useuids) { psvs.useuids = useuids; }

P_P_F int Plugin_GetLevelTime(void) { return level.time; }

P_P_F int Plugin_GetServerTime(void) { return svs.time; }
...
P_P_F int Plugin_Cvar_GetInteger(void* cvar) {
  ...
  if (var == NULL) {
    PHandler_Error(PID, P_ERROR_DISABLE,
                   "Plugin to get Cvar of NULL-Pointer\n");
    return 0;
  }

  if (var->type != CVAR_INT) {
    PHandler_Error(PID, P_ERROR_DISABLE,
                   "Plugin tried to get Cvar of different type\n");
    return 0;
  }
...
P_P_F void Plugin_BanClient(unsigned int clientnum, int duration, int invokerid,
                            char* banreason) {
  ...
  if (strlen(cl->pbguid) == 32) {
    ...
    SV_PlayerAddBanByip(&cl->netchan.remoteAddress, "INVALID USER", 0,
                        "INVALID", 0, expire);
    return;
  }
  ...
  if (cl->uid > 0) {
    ...
    SV_PrintAdministrativeLog(
        "Banned player: %s uid: %i until %s with the following reason: %s",
        cl->name, cl->uid, endtime, banreason);
    ...
    Com_Printf("Banrecord added for player: %s guid: %s\n", cl->name,
               cl->pbguid);
    SV_PrintAdministrativeLog(
        "Banned player: %s guid: %s until %s with the following reason: %s",
        cl->name, cl->pbguid, endtime, banreason);
    Com_sprintf(dropmsg, sizeof(dropmsg),
                "You have been banned from this server\nYour ban will expire "
    ...
    if (cl->authentication < 1) {
      SV_PlayerAddBanByip(&cl->netchan.remoteAddress, banreason, 0, cl->pbguid,
                          0, expire);
    }
  }
...
P_P_F int Plugin_FS_SV_WriteFile(const char* qpath, const void* buffer,
                                 int size) {
